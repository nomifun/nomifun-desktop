...
- 注释/文案随上下文用中文；交流用中文。提交署名 `NomiFun Contributor <nomifun@users.noreply.github.com>`，**不**加 `Co-Authored-By`：`git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m ...`。
