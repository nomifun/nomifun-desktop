...
- 提交作者：`git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit`（禁止 Claude 署名 / Co-Authored-By）。
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(nomi-tools): ToolRegistry::retain_named 工具白名单（空=不限制）"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(nomi-config): ToolsConfig 增 in_process_spawn 门控与 builtin_allowlist 白名单（默认=现状）"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(nomi-agent): bootstrap 接入 in_process_spawn 门控与 builtin_allowlist 白名单"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(ai-agent): 工厂计算进程内 Spawn 门控 + per-session 工具白名单灌入引擎 config"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(orchestrator): plan_flat 扁平规划（跳过 planner LLM）+ spawn_plan_flat_and_start"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(orchestrator): 受限角色 worker（per-node 工具白名单 + 网关收缩），run_restricted 默认方法零 mock 翻修"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(gateway): nomi_spawn 扁平并行扇出能力（画布可视化、supervised 即跑、deny Remote）"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "feat(prompt): lead/伙伴提示引导独立并行小任务用 nomi_spawn"
```
...
git -c user.name=nomifun -c user.email=nomifun@users.noreply.github.com commit -m "docs: 子 agent 可视化统一 v1 实施状态与真机冒烟清单"
```
