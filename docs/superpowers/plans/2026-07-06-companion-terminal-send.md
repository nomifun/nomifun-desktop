...
- **提交作者**：所有 commit 用 `--author="NomiFun Contributor <nomifun@users.noreply.github.com>"`，不加 Co-Authored-By。
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "feat(terminal): 共享 PTY 提交编码器（bracketed-paste + 分离CR）"
```
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "feat(terminal): TerminalService::submit_text 按 agent 家族路由提交序列"
```
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "feat(terminal): await_turn_settle（lifecycle TurnEnd / idle 静默 双策略）"
```
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "feat(terminal): read_output_tail（去 ANSI 的 scrollback 尾部读回）"
```
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "feat(gateway): nomi_terminal_send 能力（发送并执行，可选等待回执）"
```
...
```bash
git add crates/backend/nomifun-gateway/src/caps_terminal_ext.rs
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "feat(gateway): nomi_terminal_read_output 能力 + 伙伴可见性守卫测试"
```
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "feat(companion): 伙伴提示词补终端操作指引（本地会话）"
```
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "refactor(requirement): AutoWork 终端提交改用共享编码器（行为不变）"
```
...
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "refactor(idmm): 终端注入改用共享编码器（单行不变，多行→agent 修正为分离CR）"
```
...
```bash
git add -A
git commit --author="NomiFun Contributor <nomifun@users.noreply.github.com>" -m "chore: 伙伴终端操作闭环 验证收尾"
```
