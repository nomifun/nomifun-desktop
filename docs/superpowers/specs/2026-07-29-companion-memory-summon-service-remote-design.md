# 桌面伙伴记忆与召唤、客服改名、远程控制硬化 —— 综合设计

日期：2026-07-29
状态：已与用户逐节确认（A/B/C/D 四节均已批准；B 节按用户意见修订为"确认式记忆回写"）
...
- **C. 对外伙伴改名"客服"**：侧边栏分组"对外服务"→"服务"，整组移到"增强工具"之后；只改用户可见文案，不动任何代码标识符。
- **D. 远程控制稳定性硬化**：忙时排队、结果回推、远程解锁、错误结构化四项全做，分两批实施。

**非目标（本轮不做）**：物理世界 I/O 扩展（视频输入、TTS 语音输出）另起一轮；`public_agent`/`public_companion` 等标识符、路由、DB 字段、落盘格式一律不改名；记忆半衰期参数与注入预算不变；Remote 前门（/mcp）的 MCP 会话过期 401、REST busy→422 等外部 MCP 客户端体验问题记为已知问题跟进（本轮远控场景以 IM 渠道为主）。
...
### 命名面（C 的边界）
- 「对外伙伴」全部用户可见文案位于：`locales/zh-CN/publicCompanion.json`（约 20 处值）、`zh-CN/settings.json:171,959`、`zh-CN/common.json:4`（siderSection.publicService="对外服务"）及 en-US 对应文件；组件 defaultValue 兜底散布在 `pages/publicCompanion/` 各节、`SiderPublicServiceEntry.tsx:31`、`PlatformConfigBody.tsx:245`、`PresetEditDrawer.tsx:279`。
- 侧边栏分组唯一定义处 `ui/src/renderer/components/layout/Sider/index.tsx`（"对外服务"组在 226-235 行，"增强工具"组头在 274 行）。
- 兼容红线（绝不改）：side-store 目录 `public-agents/` 与 `PublicAgentConfig` 字段（`deny_unknown_fields`）、审计 JSONL 字段、DB `channel_plugins.public_agent_id` 与 `conversations.extra.$.public_agent_id`、preset 落库串 `"public_companion"`、`ExposureMode` 序列化值 `"public_service"`、REST `/api/public-agents/*` 与 wire 值 `"publicCompanion"`。
...
8. 客服**实体改名评估后否决**（2026-07-29 追加）：曾考虑把 `public_agent` 族代码命名一并改掉，评估后放弃——`extra.public_agent_id` 是 nomi 工厂执行期安全钳制的触发键，改名的失败模式是 fail-open（存量会话钳制失效）；渠道层 `send_to_agent` 中客服与桌面伙伴是同函数互斥分支，改名热区与 D 节硬化热区重叠；落盘迁移面 6 处而代码词根（companion vs public_agent）本不撞名。维持 C 节"只改用户可见文案"的设计。可选低风险补充：仅前端 i18n namespace `publicCompanion` 与目录 `pages/publicCompanion/` 改名（无持久化），实施时按需决定。
...
- zh：「对外伙伴」→「客服」；分组「对外服务」→「服务」。en：Public Companions → Customer Service；Public Service → Services。
- 改动面（值改、键不动，无需 gen:i18n）：`locales/{zh-CN,en-US}/publicCompanion.json` 全部含旧名的值、`settings.json`（presetTargetPublicCompanion、providerInUse.publicCompanion）、`common.json`（siderSection.publicService）；组件 defaultValue 兜底同步（`SiderPublicServiceEntry.tsx:31`、`pages/publicCompanion/` 各文件、`PlatformConfigBody.tsx:245`、`PresetEditDrawer.tsx:279`）；实现时以 `grep -rn "对外伙伴\|Public Companion" ui/ docs/` 全量核对收尾，保证无新旧名混用；用户文档 `docs/guides/channels{,.zh}.md`。

### C2. 侧边栏

`Sider/index.tsx`：将「对外服务」组块（226-235 行）移到「增强工具」组之后，组名用改后的「服务」；更新组件 doc 注释。分组保留独立灰体标识字（用户明确后续要在"服务"下加其他入口）。

### C3. 红线与回归

所有 `public_agent`/`public_companion`/`public_service` 形态的标识符、路由 `/public-companions`、REST 路径、DB 字段、落盘格式一律不动 → 零持久化兼容风险。回归：重跑 `capabilityHubNav.test.ts`、`siderOverflowScroll.test.ts`（直读源码断言）；`bun run check:i18n`。
...
1. **C**（半天级，立即可交付）→ 2. **D 第一批**（D0/D3/D4）→ 3. **A**（存储→工具→UI）→ 4. **B**（依赖 A 的搜索接口与选择器组件）→ 5. **D 第二批**（D1/D2）。

每个子项独立成 PR/分支，互不阻塞；B6 的 `nomi_create_conversation` 扩展与 D2 的 `nomi_send_to_conversation` 扩展都在 `caps_conversation.rs`，注意合并顺序。
