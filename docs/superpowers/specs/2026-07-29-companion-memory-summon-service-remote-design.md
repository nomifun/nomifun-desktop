...
9. 客服渠道所有权分域（2026-07-30 交付后修正）：C1 首版让客服从与桌面伙伴共享的渠道 bot 池中挑选绑定（且可跨域抢绑），用户指出这违背"客服自闭环"的设计意图 → `channel_plugins` 加 `owner_domain`（'companion'|'customer_service'）分域：客服在自己页面创建/管理/绑定自己的 bot；两域互斥双保险（DB 触发器：客服域 bot 不得携带 companion_id；应用层：跨域绑定一律 400，废除抢绑语义）；存量已被客服绑定的无主 bot 归客服域、双挂 bot 保伙伴清客服；渠道运行时基建（IM 插件、配对、消息循环）保持单一共用，分域的只是所有权（迁移 020——与远程 provider 迁移 016 合并重排前为 019，实施计划 2026-07-30-cs-channel-ownership.md）。
