...
但是 `daef16c9b` 的 11 个 renderer 文件是在 Mac signed/notarized artifact source `6ab013f68` 之后进入
分支，旧 App/DMG 因而不包含当前 Agent 选择范围和 Computer 设置入口。按 UARC-063 的“current source +
artifact”合同，不能把旧哈希冒充当前源码的 package 证据。UARC initiative 已重新标为 `active`；Windows
当前源码 verified，macOS 只剩一次外部 Mac 的 shared UI + current arm64 App/DMG 增量复验。
...
| `UARC-050` | Official Agents/Workbench/module UI 完整可用 | verified：880×600 + UI 3541 | implemented_unverified：late Agent selector 需 Mac 880×600/宽窗口复验 |
...
| `bun test --cwd ui` | pass；3,541/3,541 after `f08acea45` |
| late UI focused tests | pass；31/31 |
| `bun run check` | pass；包括 1,918-source 880×600 desktop boundary 与所有静态 scanner |
| production renderer build | pass；7,562 transformed modules |
| real 880×600 + wide preview | pass；完整列表、客服空态、Escape focus return、无 overflow |
...
### macOS 既有冻结证据（native 可复用，artifact 不再代表当前 renderer）
...
上述 CEF、Process/PTY、Computer physical/TCC 和 Runtime evidence 的源文件/合同未被 `daef16c9b` 改动，
可以按结果复用规则保留。App/DMG 和 shared UI 不能复用为 current-source 证据，必须在 Mac 重建并增量复验。

## 4. 生产可达性、删除与 retained 审计

- 普通 UARC scanner：13 个 legacy group 全部 0；open baseline anomaly 0；当前 Mac gap 1。
- `bun scripts/check-uarc-boundary.mjs --completion` 现在按设计失败，精确阻止 UARC-042/050/053/063/070 和
...
- UARC 初始 source 至当前 worktree 的 changed path 全部落入 manifest write set；late UI 的 Computer picker
  归 UARC-042，Conversation Agent selector/catalog 归 UARC-050，replacement cleanup 归 UARC-053。
...
- 24/29 task 保持 `complete`；`UARC-042/050/053/063` 回到 `integrated`，`UARC-070` 为 `active`。
- Windows required platform 全部 verified；上述五项的 macOS 状态为 `implemented_unverified`。
- `TASK-MANIFEST.json` initiative status 为 `active`，completion scanner fail closed。
- 唯一剩余工作是外部 Mac current-source UI/package 增量复验；若 Mac 不修改代码，无需重跑不受影响的
  CEF 34/34、Process 240、Terminal 146、Computer physical/TCC 或真实 StepFun gate。
