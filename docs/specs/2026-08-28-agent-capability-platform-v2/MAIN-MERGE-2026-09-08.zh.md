...
2. 数据库生命周期断言继续使用重构分支的统一迁移头常量，并更新到 74。
...
- 只在可写迁移阶段、一个外层事务内，把这两条 ledger 记录的版本号映射到 073/074，然后补齐重构分支 059–072；原校验和、描述和安装时间保留。升级失败时连同版本号移动一起回滚。正常启动后即为唯一的 001–074 正式链，不保留第二套运行时 schema。
- 只读启动探测只报告 `UpgradeRequired`；不改 ledger、不退役目录。未知版本、失败记录和校验和篡改继续拒绝。
- 已验证 main 059、main 060、refactor 072/073、重复打开、失败回滚、篡改拒绝，以及素材历史/待删除内容保留。
