...
6. **首次 clean cutover 固定 whole-root atomic rename，clean install 复用同一 parent-marker 协议。** 停止 app/sidecars 后，校验精确 canonical legacy root、同父目录 timestamp sibling archive target、target 不存在且同文件系统；只调用原子 directory rename，绝不 copy fallback。任何创建 v4 root 的操作都必须先在 canonical root 的 **parent** 独占写一个 immutable bootstrap marker；cutover 在 rename 前写，clean install 在 mkdir 前写。marker exact-set 只引用 canonical Rust contract：`operation_id/operation_kind/canonical_normalized_relative_basename/cutover_archive_sibling_relative_basename?/target_data_generation/canonical_schema_manifest_digest`，不含绝对路径、旧内容或 mutable stage。marker durable 后发生 rename、target race 或 cross-volume 失败时，legacy root 必须原样保留、不得创建 v4 root且必须保留同一 marker供确定性恢复。rename 成功后才在原 canonical path 创建空 v4 root，按 final migrations/`schema_metadata` → bundled Package materialization → G0-frozen `OfficialPresetSeedManifest` seven-template authoring seed 的顺序执行；在 data generation、migration/seed/projection versions 与 canonical schema/seed manifest digest 完全匹配后写 ready marker并删除 parent marker。初始化失败只清理或重试这个无 ready marker 的不完整 v4 root，archive 永不触碰。用户随后重新配置。无 converter、legacy import、ID mapping、冲突合并或旧 binary rollback bundle。
...
6. **Activation gate**：activation 只允许在无 Tool/Effect 正执行的 model-turn
   boundary。唯一持久 Event 是 `capability/active-set-committed` generation `N+1`；
   requested/failed 仅为 transient diagnostic。`active_N ⊆ active_N+1 ⊆ ceiling`，
   commit 后下一次模型请求才见新 descriptor。重复/并发 request 使用稳定
   idempotency key，只能得到完整 N 或 N+1。
...
- Runtime activation 只能满足 `active_N ⊆ active_N+1 ⊆ ceiling`；集合外请求稳定
  返回 `CAPABILITY_NOT_IN_PRESET`，未激活调用返回 `CAPABILITY_NOT_ACTIVE`，均不
  产生 approval/grant/lease；
...
3. **Immutable parent marker（两种 kind 共用）**：在 canonical root 的 parent 独占创建单个最小 marker，发生在 cutover rename 或 clean-install mkdir **之前**。schema 固定引用 canonical Rust exact-set：`{operation_id,operation_kind,canonical_normalized_relative_basename,cutover_archive_sibling_relative_basename?,target_data_generation,canonical_schema_manifest_digest}`；`operation_kind` 仅为 `fresh|cutover`，archive basename 只允许在 cutover 存在。marker 不含绝对 canonical/target path、旧内容、credential、可变 intent/phase/stage，不建立 state table/DB record/API/产品对象。marker 创建失败不得 rename 或 mkdir。
4. **Atomic rename（仅 cutover）**：调用 OS 原子 whole-directory rename。禁止逐文件
   enumerate/read/copy、recursive move、copy+delete 或跨卷 fallback。marker 一旦
   durable，rename 的确定性失败与 crash 都必须保留该不可变 marker；legacy root
   仍在 canonical basename、archive basename 不存在且不得创建 v4。恢复只能从
   marker 与 exact filesystem facts 重新校验后重试或 fail-stop。
5. **Fresh canonical root（两种 kind 共用）**：cutover rename 成功后 archive 保持
   opaque 且应用不访问；clean install 无 archive。两者都在 marker 已 durable 后才于
   canonical basename 创建空 v4 root，按固定顺序运行正式 migration chain并写
   `schema_metadata{data_generation=4,root_instance_id,migration_head,seed_manifest_digest,canonical_schema_manifest_digest,projection_schema_version}`，
   再物化 compiled bundled Packages，最后只消费 G0-frozen
   `OfficialPresetSeedManifest` 幂等 seed 七模板。`schema_metadata` 是数据库内部
   lineage 元数据，不属于 D-015 Session 事实表/Projection exact-set，也不是业务对象
   或 bootstrap 状态表。只有 metadata 与 marker/installed build/migration/
   OfficialPresetSeedManifest exact match、schema/materialization/seed/composition
   均通过后才能写 ready marker。
...
- capability ceiling 外调用稳定返回 `CAPABILITY_NOT_IN_PRESET`，未激活 on-demand
  调用稳定返回 `CAPABILITY_NOT_ACTIVE`；所有场景均无
  approval/grant/lease/release/install/preset mutation/ceiling expansion runtime path；
