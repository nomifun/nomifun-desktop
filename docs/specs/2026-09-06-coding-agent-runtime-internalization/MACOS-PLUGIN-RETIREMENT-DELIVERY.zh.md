...
最终远程核对发现新提交 `71b5b16430e4d655f0297f5ce4afc730b0f79d5f`
（输入框、资产与多模态草稿，55 个 UI 文件）。已 fetch；它晚于本次候选构建，不在上述制品内，
也未自动合入本地已验收版本。双方提交均保留，不用强推消除分叉。后续整合应使用普通 merge，
复核其 renderer 改动；不得把本包的检查结果直接归属该新 UI 提交。
