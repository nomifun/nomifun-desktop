...
### CAR-D-021：Engine 仅允许编译期集成，随应用打包（2026-09-13）

用户收敛第一阶段范围：社区通过二次开发添加 Engine 实现和构建依赖，在组合入口
注册后重新打包 NomiFun。发布后的应用不允许安装、挂载、卸载或热替换 Engine。

- 官方发行版预置 Nomi 通用 Engine 与 Coding Engine；社区实现使用同一注册契约。
- Engine 的差异是任务执行循环、规划或上下文机制，不是另一套 Agent 身份。
- 使用受信任进程内 Rust 接口及编译期检查；不建设动态库 ABI、运行期插件安装器
  或 Engine 热更新系统。HTTP 目录只读；应用组装后关闭注册。
- 描述符、工厂及兼容性策略一起注册。平台按绑定查找策略，不在会话业务路由中
  判断具体 Engine family。能力支持声明不授予权限，实际调用仍由 Kernel 准入。
- 应用升级仍需处理旧 exact binding 和私有状态兼容；本决定不授权静默替换构建，
  不把 Fork 当作迁移接口，也不要求先建设独立 Engine 更新器。
- 公共模型/工具/历史/状态宿主接口、异常重启恢复仍是后续必需工作，不能因为
  注册回调可调用就宣布社区完整接入完成。

本轮实现通用注册准入、Coding 活跃能力状态及真实循环的有界上下文选择。
自动 compaction、AGENTS.md 和 checkpoint 恢复不在本轮完成范围内。
