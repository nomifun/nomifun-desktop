...
2. 先自由填写模型 ID，或明确选择一条实时/维护目录建议；目录只是候选来源，不能成为录入白名单；
3. 只有明确选择目录项这一动作才会用该条目的模型级能力元数据初始化建议任务，并把结果完整显示出来供用户复核和调整；手填模型先通过单选任务添加器加入一个主任务，额外任务只能通过“添加其他任务”逐项加入；
...
`MODEL_TASK_ORDER` 只是 UI 任务添加器的完整选项顺序，不表示把供应商全部已适配任务展示成模型能力，也不应呈现为九项自由多选。目录模型从用户明确选择的目录项读取模型级任务建议；手填模型每次只添加一个任务。一个模型仍可保存多个任务，且每个已添加任务都可独立删除和配置，不会因为模型同时支持另一任务而自动继承协议或 endpoint。
...
2. 先自由填写模型 ID，或从实时/维护目录中明确选择一个具体模型；目录加载失败不得阻止手工输入和保存。
3. 明确选择目录项时，用该目录项携带的模型级能力元数据初始化建议任务和已核验 traits，并立即把全部结果显示出来供用户复核、删除或继续配置；仅输入相同 ID、后台目录刷新或供应商 adapter 状态都不得静默改变任务集合。
4. 手填模型通过单选任务添加器先加入一个主任务；需要多任务时，用户再通过“添加其他任务”逐项加入，已添加任务不再出现在添加器选项中。正常新建流程不得把全部九种任务呈现为无差别多选。
...
单任务添加器只是新建交互约束，不改变底层多 task 契约。编辑已有多任务模型时必须完整加载并保留其 capability 集合；运行时每次请求仍只选择一个 task，并只按精确的 `(provider_id, model, task)` 行取得协议和 transport 配置。
...
| Ctyun | `wishub-x1` 所属文档已停止维护；`wishub-x6` 也已被现行接口表替换 | 新增只用 `ai.ctaigw.cn/v1`；精确命中旧 x6 官方预置根的已有配置由一次性迁移 031 更新，自定义网关不改。 |
...
- UI 的“供应商 → 模型 ID / 明确选择目录项 → 支持任务”顺序、自由模型 ID 输入和目录失败不阻塞保存测试；目录选择必须用具体模型的能力元数据显式初始化建议任务并可见供复核，手填模型必须先单选添加主任务、再通过“添加其他任务”逐项扩展；还需覆盖“已适配 / 官网已知待适配 / 自定义需覆盖”三态，且供应商 adapter 状态不得被当作模型能力，只有 `supported` 能自动命中预置路由。
