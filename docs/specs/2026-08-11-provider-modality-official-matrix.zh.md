...
`MODEL_TASK_ORDER` 只是 UI 主类型和“添加其他任务”的完整选项顺序，不表示把供应商全部已适配任务展示成模型能力，也不应呈现为九项自由多选。新建模型先单选主类型，目录候选只显示明确包含该任务的模型；手填模型也只初始化这个主任务。一个模型仍可保存多个任务，且每个额外任务都可独立删除和配置，不会因为目录声明了另一任务而自动继承协议或 endpoint。
