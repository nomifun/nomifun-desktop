...
| Ark / 火山语音 | Ark `/chat/completions` / Responses；Bearer | Ark `/images/generations`；Seedream 4.0/4.5/5.0 的文生图与图片编辑共用该路径，编辑输入为有序 `image` 字符串/数组，`ark.images` 已覆盖 `image_generation` 与 `image_edit` ✅ | Ark 内容生成 task submit/query；异步 | `openspeech` 独立域名；TTS v1/v3、ASR submit/query/WS 的路径、header 和 client request id 各不相同 | 仅对官方已确认的 Seedream 4.0/4.5/5.0 目录 ID推导双任务；不从不透明 `ep-*` 名称猜测模型，但旧版已配置的 `ark.images` 行会迁移出同传输的编辑能力并重置健康状态。 |
