...
当前唯一会阻止“直接搬代码”的问题是 L0 许可证/来源证明。架构壳、Rust 端口和原创实施不被它阻塞。

下一步顺序：

1. 确认采用“提供完整授权后直接移植”还是“按行为规格独立实现并接受更长工期”。
2. 在不复制参考代码的前提下完成 P1：拆分应用级受保护运行时与普通工作台布局。
3. 完成 P2：建立全屏 Focus Shell，并进行第一次真实 Tauri UI 验收。
4. L0 通过后，按本计划进入产品 UI 和领域能力实施。
