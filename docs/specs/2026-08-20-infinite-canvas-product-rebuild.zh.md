...
截至 2026-08-20，P1 与 P2 已按原创实现路径完成：

- 应用级认证、主题、深链、通知和桌面伴侣运行时已从普通工作台布局中拆出，Focus Shell 与普通工作台共用同一受保护运行时；
- `/workshop` 已成为独立全屏产品入口，主侧栏入口不再携带旧 Beta 标记，返回动作固定回到 `/guid`；
- Focus Shell 拥有独立标题栏、拖拽区和 Windows 窗口控制，不依赖普通工作台的 `LayoutContext`；
- WebUI 已验证“工作台进入 -> 全屏工作区 -> 返回工作台”、直接访问、刷新、主题保留和无旧侧栏；
- 真实 Tauri 窗口已在隔离数据目录中验证同一关键路径，刷新后仍保留 Focus Shell，返回后恢复普通工作台和原生窗口控制；
- P1/P2 目标测试、`bun run typecheck`、`bun run check` 与 `git diff --check` 均通过。首次空数据初始化期间观察到既有 `/api/preset-tags` 并发请求短暂返回 409，随后恢复 200；它不来自 Focus Shell，本阶段未以补丁方式顺带修改。

当前唯一会阻止“直接搬代码”的问题仍是 L0 许可证/来源证明。架构壳、Rust 端口和原创实施不被它阻塞，但在产品主体开工前必须明确选择实现边界，避免后续提交混入来源不清的代码或资产。

下一步顺序：

1. 确认采用“提供完整授权后直接移植”还是“按行为规格独立实现并接受更长工期”；推荐后者。
2. 选择独立实现后进入 P3：项目中心、`nomifun.creative-studio/v1` 数据契约与新建/打开/删除项目闭环。
3. 之后按 P4-P8 逐批实现画布、节点、资产和模型能力，每个提交都执行目标测试、WebUI 截图与里程碑 Tauri 验收。
4. P9-P11 完成代理/导演台、旧实现删除、恢复性/性能/打包验证与最终切换。
