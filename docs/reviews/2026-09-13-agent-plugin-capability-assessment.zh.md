...
最新批准的第一/二/三批清理与支持矩阵见 [架构收敛记录](2026-09-15-plugin-architecture-convergence.zh.md)。系统能力组合/替换与生命周期插入分别验收；本轮撤掉旧 Skill 链、Service 流式前置及 UI 专用事件协议，简化页面参考实现与宿主装配，不恢复 Rust 插件或补齐延期领域。下文相关旧实现描述只保留历史意义。
...
- 最新决定为彻底删除 native Service 后端、Rust 作者 SDK、启用入口及专属示例/打包/测试路径，替代此前默认关闭但保留的决定。已移除，Windows 删除专项回归通过；历史测试不代表现行支持。插件 Agent 页面决策独立，正式默认仍为内置 UI。
- 模型插件、整个 Agent runtime、Shell 和深层基础设施替换延期；Node 普通 Service、存储/取消/生命周期和生产模型合同保留；没有生产消费者的 Service 流式前置按本轮审查决定删除。
...
状态：历史上曾实现 Rust 原生 Service 后端与 SDK，后按范围收敛决定删除；代码删除已完成，已通过检查及待补回归见发布台账。完整领域开放目标尚未完成。

**开发安排阅读入口：** 当前决定及验证状态以发布台账和剩余工作文档为准；§27.17 / 实施台账 §2.40 仅保留原生插件曾实现后按范围收敛删除的历史记录。模型功能卡看 §27.16，29 行长期状态看 [实施台账 §3](2026-09-14-agent-plugin-openness-implementation.zh.md#3-原始-29-个环节不得缩减的跟踪清单)。不为已删除后端强做临时 JS 包装；早期“下一批”及旧人周估计不是执行指令。

**2026-09-14 历史实施边界（不构成恢复原生插件的安排）：** 用户当时要求，若某项在 JS 插件支持下不合理、更适合原生实现，则延期，避免临时方案和历史债务。29 行保留为长期能力清单，不要求每行强行交付 JS 第二实现；适配性与延期条件见 §27.13。不为延期项预建 Rust stub 或无人消费的协议。
...
§1～10、§12～13 描述原始基线；§11 总结架构判断；§14～21 为目标设计与研究证据；§22～26 保留非 native 工作包及必要历史说明；§27 核对原始需求与遗漏范围。原生插件详细设计、开发与打包方案已移除，§22/§27.17 仅作历史追溯。§20 的 A～G 只是主题路线，不是另一套执行包。历史“建议/应当”不代表当前产品能力或排期，实际代码与验证以发布台账为准。
...
5. **原生插件历史取舍**：曾评估并实现 Rust 进程外后端及作者 SDK，后按范围收敛决定删除；不提供现行开发路线或启用承诺。语言选择不替代真实消费接口与授权边界工作。
...
## 17. Rust 主机与 JS 插件：公共边界与历史取舍

原 Rust/native 插件执行形态、SDK 和协议方案已移除：后端曾实现，后按范围收敛决定删除，已移除，Windows 删除专项回归通过。历史原生测试不代表现行支持；未来是否重开需独立产品决定，不保留开发步骤。

### 17.1 首要问题是接口边界，不是语言对立

不能跨语言直接传递 Rust 的 trait object、借用、`Arc`、异步 Future 或内部服务引用，但可以传递请求、结构化结果、流、取消通知和资源 handle。大部分 Agent 扩展是粗粒度 I/O 与策略调用，不需要把每次 Rust 函数调用都 RPC 化。

现有 [`JsKernelPluginAdapter`](../../crates/backend/nomifun-js-kernel-adapter/src/lib.rs) 把 JS 调用适配成 Kernel handler/context/resource 接口；宿主准入和实际消费仍须逐项接线。公共契约继续用语言无关 DTO、已有 Rust canonical contracts 与 schema/TS 生成设施，不另建能力目录、Preset 或授权系统。Rust trait 是主机内部 port，不是用户原生插件装载能力。

### 17.2 保留的隔离边界

普通 Node `import()` 和 stdio 进程通信不构成强沙箱；共享 host 的异常或无限循环可能影响同 host 其他插件。进程隔离不自动限制文件、网络、凭据或同权限进程，SDK 的 grant 校验只保护宿主接口。不能把尚未实现的 OS sandbox 或受控 Wasm imports 宣传为现有能力。

Node 隔离、资源限制和故障回收属于自身产品边界，不因原生插件删除而撤销。Wasm、其他执行形态与强隔离只保留为有具体需求时另行评估的长期议题，不预建新后端或 SDK。

本节保留的技术依据（2026-09-13 查阅，非本仓库运行验证）：[Node.js VM](https://nodejs.org/api/vm.html) 明确不是安全机制；[Wasmtime Security](https://docs.wasmtime.dev/security.html) 说明隔离及 WASI 授权边界。两者均不是本项目支持新后端的承诺。
...
5. 保留 JS 与内置 Rust 的公共数据边界；原生插件已移除，Wasm 仅为未排期的长期议题；
6. 把最小信任根与部署级替换讲清楚，不把必要安全不变量扩大成产品功能封锁。

可概括为：**同一能力体系、多种实现语言、显式组装、单一执行真相；默认内置组件也是可以被用户替换的实现。**

## 22. 原生插件方案历史记录（详细设计已移除）

曾评估并实现与 Node 共用 Product 平台的原生 Service 后端和 Rust 作者 SDK，后按范围收敛决定删除；已移除，Windows 删除专项回归通过。原执行架构、目标制品、SDK 和作者开发方案不再保留为可执行计划。历史测试不代表现行支持，亦不证明删除后的候选通过。

Node 运行环境、插件执行与 Agent Runtime 是不同职责；共享 Service 流式、模型合同、授权与生命周期能力保留，不随原生后端删除。完整模型/UI/Runtime 的真实消费者缺口仍须独立评估，不能由一种执行后端代替。
...
|---|---|---|---|
| R1：可替换能力与 Agent UI 的开发者预览（历史目标） | 一个真实系统契约有内置/用户可选实现；Tool、Context、插件 Skill 真正消费；插件完整 Agent 页面；用户 JS 实现可安装 | P0～P4、P6；P5 原生插件方案已移除 | 不叫“任意主机部件全栈完成”；不含整个 Loop 替换、完整 Shell、任意资源 Provider 或强沙箱 |
...
R1 历史上以一个“研究助手”插件为纵向验收样例：选择自己的搜索/检索实现，加载包内 Skill 与 Context，在插件 Agent 页面完成一次真实会话。原额外 Rust 实现验收方案已移除，不作为现行交付要求。
...
原 R1-JS 保留公共边界与内置 Rust/用户 JS 契约验证，不执行 P5。该分期仅供历史追踪，当前上线范围以两份 2026-09-15 决策/发布文档为准；原生插件已决定删除，不是等待开启的可选首发包。JS Provider、Skill/Context 与页面的非 native 工作保留。
...
|---|---|---|---|---|
| P0. 基线与两条验证 | 固定候选基线；选定真实替换部件；内置 Rust/用户 JS 同契约验证；UI 走现有 API 完成发送/观察；据结果重估 | 报告、现有示例/测试、contracts/runtime/UI port 的最小验证 | 无；不等待所有长期功能设计 | 1～2（历史估算） |
...
| P5. 原生插件历史项 | 后端与 SDK 曾实现后按范围收敛决定删除；不保留开发/验收工作包 | 删除实现及验证以发布台账为准 | 不再作为其他包前置 | 不再排期 |
| P6. R1 集成与开发者预览 | 参考插件全链路、故障恢复、升级不漂移、Windows Web/桌面验证、可复现打包；文档和诊断可交给外部开发者 | 集成测试、发布脚本/示例、各包 owner 联合修复 | P3/P4，不依赖 P5 | 2～3（历史估算） |
| **估算边界** | **原合计含已移除 P5，不再沿用；非 native 工作需按实际剩余范围重估** | — | — | — |

P1 不是要求把统一工程再重做一次。已有 contracts/DB/app/UI 改动应按实际证据区分“已验收”“仍需集成”“尚未完成”，只估剩余工作，不把 Git 修改文件数量当进度。发现基础功能损坏需大范围修复时应重估，不隐藏在其他预算中。

具体 crate 名称只是责任定位，不预先规定新增十几个 crate；优先在现有模块内建立清楚接口，不为已移除原生插件方案预建 adapter 或 SDK。

### 24.2 原生插件预算历史记录

原 P5 的子任务、人日拆分和开发方案已移除；后端与 SDK 曾实现后按范围收敛决定删除，不再以旧预算安排开发。共享协议与非 native 集成工作不随该项移除。
...
| P12. 目标平台与发布收口 | 所选 UI/runtime/Node 组合的打包、依赖、签名/公证和发布说明；当前首发仅 Windows x64 + macOS arm64 | 可提前做单平台验证；正式收口依赖所发布范围稳定 | 4～7（历史估算，需重估） |
...
完成上述工作代表所列主干部件具有真实替换路径。原五项需求及 §27.2 的 29 个环节作为长期评估保留，旧估算未拆细的部分需补估。历史实现及全部策略组合的穷举验证不由粗估表承诺；不能把主干预算包装成完整目标的固定总价。
...
P1/P2 是历史首期关键路径，不宜让多人同时改同一份 contracts/compiler。P3/P4 在接口稳定后可并行，共享 app composition 接线由一个集成 owner 管理。阶段依赖满足即可合流，不要求先完成无关未来 SPI。
...
3. 原 R1 估算以 Windows x64 的 Web 与桌面路径为基线，不含强沙箱；当前双平台首发与删除后范围须按发布台账重估。
...
含原生后端的总量与成本对比已移除，不再用于排期。非 native 的原 R1-JS 基础估计为 14～24 人周，R2 增量 19～34、R3 增量 12～21；均是当时未校准的主干预算，不是当前剩余量或交付承诺。共享协议/Provider 工作对 JS 本身仍有价值，不随原生后端删除。
...
| 后端 B / 运行集成 | Context/Skill、Node 与共享进程/资源接入、故障测试 | 与 A 协调公共接口和集成职责，不并行争写共享模块 |
...
原日历含 native 小样、SDK 与联合验证，现移除该安排，不换算为删除后的承诺工期。保留的协作原则是：契约稳定后再并行，单人不能在多个包同时投入 100%，以真实剩余量、共享文件和目标平台条件重估。
...
| 附加范围 | 是否已经包含 | 处理方式 |
|---|---|---|
| 额外桌面发布平台 | 当前只确认 Windows x64 + macOS arm64 | 其他架构与 Linux 另行决定，不保留原生插件平台验收项 |
| Wasm 首个受限执行后端 | 不含 | 可先占 4～8 人周基础探索/接入预算；正式支持按 imports、异步与工具链 spike 重估 |
| 不可信 Node 插件的跨平台强沙箱 | 不含 | 独立安全工程，未选定威胁模型和 OS 机制前不沿用旧成本占位 |
| 多后端同包编排、远程插件、在线构建服务 | 不含 | 独立长期议题，需产品决定与预算，不构成原生插件恢复计划 |
...
3. 验证内置 Rust adapter 与用户 JS Provider 的相同输入/输出、错误和取消契约，不开发或测试原生插件入口。
...
原 Rust 小样步骤已移除；公共契约与 UI 的非 native 验证保留为历史方法，不作为最新执行指令。

这些短验证通过后，历史安排为 P1/P2，再于接口稳定后并行 P3/P4；不再安排 P5。小样只在有实际消费者或测试价值时保留，不留下额外生产路径。
...
1. 一次真实产品调用分别绑定内置/用户 JS 实现，调用记录能确认选中者；不选择的实现不被暗中调用。
...
原生插件安装及故障验收要求已移除；其他历史退出条件仅说明各工作包的目标，当前正式支持范围仍以发布台账为准。

验证按风险选择：contracts/compiler 变更跑相关 Rust 契约/编译测试，Skill 跑精确装载测试，UI 跑交互与 Bridge 测试，Node/共享 Service 跑适用生命周期测试。跨模块集成或候选发布再跑相应更宽检查，不为局部修改要求全仓全平台检查。
...
含 Rust 插件小样、P5 和原生 SDK 的实施安排已移除。非 native 工作仍按公共基线、真实消费者、定向验收逐步推进，不同时展开 Wasm、强沙箱、全部内部策略、完整 Shell 和深层存储替换。当前首发范围及执行入口以两份 2026-09-15 文档为准。
...
### 27.17 原生插件曾实现后按范围收敛删除（历史记录）

曾按当时授权实现 Rust 原生 Service 后端和作者 SDK，后按用户最新范围收敛决定删除。详细执行架构、制品格式、启用、SDK、打包和测试方案已移除，不保留为开发指南。代码删除已完成，已通过检查与待完成回归统一见发布台账；历史 Windows 原生测试不代表现行支持或删除后的候选通过。

同步历史保留：当时分支从 `2ff029512` 更新到 `e617feb2b`，吸收 7 个上游提交、35 个文件变更；备份并恢复既有工作，无冲突，当轮没有提交或推送。此同步事实不因删除原生插件而撤销。

共享 Service 流式、存储 IPC、授权与生命周期、模型纯数据合同及 Node 工作保留，详见实施台账 §2.35～2.39。模型 Provider、会话资源、完整 UI/Shell/Runtime 的真实消费缺口仍属长期清单，不因曾增加或现删除一种后端而自动完成，也不在本轮继续扩展。
