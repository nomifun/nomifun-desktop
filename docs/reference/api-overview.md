...
| Mini-apps | `GET/POST /api/miniapps`, `GET/PUT/DELETE /api/miniapps/{miniapp_id}` | instance owner only | [`nomifun-miniapp/src/routes.rs`](../../crates/backend/nomifun-miniapp/src/routes.rs) |
| Mini-app runtime (serve the stored HTML) | `GET /api/miniapps/{miniapp_id}/serve` | public — auth-exempt on purpose: an iframe subresource load carries no trust header, so the runner and the in-conversation preview could not render at all behind auth; the guard is the unguessable UUIDv7 (same precedent as the workshop public file route) | same as above |
