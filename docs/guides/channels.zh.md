...
`channels.<platform>.masterAgent`（无值 = 开启）。关闭后该平台回退
...
伙伴的渠道行回退到平台级偏好 `channels.<platform>.companionId`，再回退
...
每个平台都在它的配置表单里有 **默认 agent** 和 **默认模型** 选择器。平台把它们存为客户端配置中的 `channels.<platform>.defaultModel`，因此：
