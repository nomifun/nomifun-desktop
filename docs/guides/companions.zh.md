...
4. **删除伙伴**会级联清理：它的**记忆**、技能、陪伴会话、运行时状态（XP 等）、`('companion', companionId)` 知识库绑定一并移除；若删的是默认伙伴，默认资格自动顺延给下一个。由于每条记忆都属于某一个伙伴，删掉伙伴就等于永久删掉记在它名下的记忆，且没有任何迁移包能救回来（伙伴包只带设定与成长，也没有按伙伴导出记忆的入口）。删除允许删到零个——采集与学习照常运行，但没有伙伴可归属时，学习 run 会提前结束而不写入记忆。
...
- **记忆与聊天历史都不随伙伴包迁移**：记忆存在 `shared/memory.db`、陪伴会话记录存在主数据库里，伙伴包只带人格、设置与成长。记忆只能走记忆包（而记忆包是整机一份，不按伙伴切分）；聊天记录留在原机。
