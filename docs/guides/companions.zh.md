...
NomiFun 的数字伙伴从「单个 nomi」升级为**多伙伴家庭**：你可以创建多个伙伴并同时使用、分别培养、自定义名称/形象/人格，每个伙伴可以使用自己的聊天模型、绑定自己的**专属知识库**（演进出金融/文学/coding/情感等专业伙伴）；而**每条记忆都只属于一个伙伴**——一个伙伴学到的东西是它自己的，别的伙伴看不到。采集与学习仍是一条全局链路，但它产出的每条记忆都记在某一个伙伴名下。记忆、伙伴、知识库都可以打包成 `.zip` 导出/导入，换机平滑迁移。

> 入口是侧边栏的 **桌面伙伴** 页（即 `/nomi` 路由）；任意桌面伙伴窗口右键菜单的「打开聊天」也会深链到这里。

## 页面结构：伙伴切换条 + 双域 Tab

「桌面伙伴」页顶部是**伙伴切换条**：每个伙伴一张卡片（形象缩略图 + 名字 + 等级），加一个「新建伙伴」按钮。当前选中的伙伴驱动下面的**伙伴域** Tab；少数仍然全局唯一的设置归**install 级** Tab：
...
| | 设置 | 名称 / 形象 / 人格 / 勿扰 / 删除伙伴 |
| install 级（全局唯一） | 数据采集 · 学习 | 唯一的采集 + 学习链路：整机一套事件来源、一个间隔、一个学习模型。在任何一个伙伴的 Tab 里改，都是给所有伙伴一起改 |
| | 迁移 | 导出 / 导入迁移包（见下文） |
...
4. **删除伙伴**会级联清理：它的**记忆**、技能、陪伴会话、运行时状态（XP 等）、`('companion', companionId)` 知识库绑定一并移除；若删的是默认伙伴，默认资格自动顺延给下一个。由于每条记忆都属于某一个伙伴，删掉伙伴就等于永久删掉记在它名下的记忆——想留住就先把它导出（伙伴包默认带走它的记忆）。删除允许删到零个——采集与学习照常运行，但没有伙伴可归属时，学习 run 会提前结束而不写入记忆。
...
`{data_dir}/companion/shared/` 下的那套**设施**仍然全局一份（一份配置、一棵事件目录、一个 `memory.db` 文件把所有伙伴的记忆行放在一起，每行标着自己的主人）——但链路**产出**始终落在唯一的主人名下：
...
- **学习**：单个学习器按设定间隔增量蒸馏事件为长期记忆，存入 `shared/memory.db`。学习链路使用**共享配置里的学习模型**（与每个伙伴的聊天模型相互独立，单链路单预算）。它的间隔、模型与事件来源都是 **install 级**的：整机只有一条循环，在任何一个伙伴的 Tab 里改就是给所有伙伴一起改。
- **学习产出归谁**：学习 run 写出的记忆记在**主人伙伴**名下——若显式默认伙伴仍在名册里就是它，否则是最早创建的那个；也只有这个伙伴看得到。聊天中保存的记忆则属于该对话所属的伙伴。
...
**mood 是全局的**：由学习 run 产出、存共享状态，所有伙伴同一 mood（按伙伴分化的人格化 mood 留待后续版本）。XP 与 mood 是全家仅剩的共享物——记忆不是。
