...
| [`nomifun-workshop`](../../crates/backend/nomifun-workshop/) | Canonical 创意工坊域：持有带版本项目文档、素材、提示词、模板/运行、严格 one-shot 模板草稿、Canvas ZIP 归档，以及大部分 owner-only `/api/creative-studio/*` 路由。项目文档、模板状态与素材 metadata 在 SQLite；二进制原件/缩略图在 `{data_dir}/workshop/assets/`。唯一公开面是供浏览器媒体元素使用的只读 `GET /api/creative-studio/files/{asset_id}`。 |
| [`nomifun-plugin-platform`](../../crates/backend/nomifun-plugin-platform/) | Plugin 平台与 Product 运行域：持有项目、源码、测试、不可变 Release、UI Surface、Service 生命周期、Capability、传输、备份、回滚和 owner-scoped Plugin 状态。活动 HTTP 路由统一为 `/api/plugins/**`（包括 `/api/plugins/runtimes/**`）；运行资源通过受 Capability 约束的 Surface descriptor 交付，不再使用公开的旧 serve 路由。 |
