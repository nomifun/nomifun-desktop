...
| [`nomifun-miniapp`](../../crates/backend/nomifun-miniapp/) | 小程序域：用户固化的 AI 生成单文件网页小工具。持有 `miniapps` 表（HTML 内联入库）、属主隔离的 `/api/miniapps` CRUD 路由面，以及供 iframe 渲染的免认证 `GET /api/miniapps/{id}/serve` 路由。 |
