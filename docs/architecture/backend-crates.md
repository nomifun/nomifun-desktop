...
| [`nomifun-plugin-platform`](../../crates/backend/nomifun-plugin-platform/) | Plugin platform and Product runtime: projects, source, tests, immutable Releases, UI Surfaces, Service lifecycle, capabilities, transfer, backup, rollback, and owner-scoped Plugin state. The active HTTP surface is `/api/plugins/**`, including `/api/plugins/runtimes/**`; runtime assets are delivered through capability-fenced Surface descriptors rather than a public legacy serve route. |
