...
| [`nomifun-workshop`](../../crates/backend/nomifun-workshop/) | Canonical Creative Studio domain: versioned project documents, assets, prompts, workflows/runs, strict one-shot workflow drafts, Director-aware project ZIP archives, and most owner-only `/api/creative-studio/*` routes. Project documents, workflow state, and asset metadata live in SQLite; binary originals/thumbnails live under `{data_dir}/workshop/assets/`. The only public surface is read-only `GET /api/creative-studio/files/{asset_id}` for browser media elements. |
