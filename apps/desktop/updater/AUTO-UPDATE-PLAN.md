...
> 在 Mac 上 `git pull` 即可获得本文件。当前进度：**版本号联动已完成并上线；自动更新仅完成调研与方案设计，尚未写任何更新相关代码。**

---

## 0. 一句话现状

- ✅ **版本号单一真源 + `bun run bump` 一键改版**：已实现、已推送 main（commit `ecc0bda`），当前版本 `0.1.10`。
- ✅ **Windows 单一 NSIS 安装包 + `build:win` 兼容 PS 5.1**：已上线（`830e88a` / `6c1646d`）。
- 🔜 **应用内自动更新**：方案已定（见下），**代码未开始**。当前 Tauri 下更新 UI 是"死"的（详见 §3）。
...
- main 最新相关提交：`ecc0bda`（版本号单一真源）、`830e88a`（NSIS 单包）。
- 当前版本：`0.1.10`（`Cargo.toml [workspace.package].version`；`tauri.conf.json` 无 version 字段、靠继承）。
- `bun run help --check` 通过；`bun run build:win` 已验证产出单一 NSIS 安装包。
- 尚未改动任何"更新"相关代码 / 未生成任何签名密钥（本项目仅停留在调研 + 本设计文档）。
