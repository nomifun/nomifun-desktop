...
3. **托管：GitHub Releases**（`latest.json` 与各平台安装包都作为 Release 资产）。仓库 = `redacted@example.invalid:nomifun/nomifun-tauri.git`。
...
**剩余手动步骤（需要真机 + 你的发布账号）：**
1. 各平台构建机：`export TAURI_SIGNING_PRIVATE_KEY="$(cat apps/desktop/signing/nomifun-updater.key)"` → 出签名更新产物（统一用叠加配置文件：`bun run build:<mac|win|linux> --config apps/desktop/tauri.updater.conf.json`；**不要内联 JSON**，Windows PowerShell 5.1 会剥引号）。
...
> 现状：阶段一（脚本 + 配置文件 + 文档）已落地；阶段二（在本机真打签名产物 → 合并 `latest.json`）随后执行。

**关键调研结论 —— Windows PowerShell 5.1 的原生参数怪癖：**
- 本机只有 Windows PowerShell 5.1（无 pwsh 7+）。实测：经 PS 5.1 传给原生程序（`bun`）的**含双引号 JSON 会被剥引号**——`--config '{"bundle":{"targets":["nsis"]}}'` 到 `tauri` 手里变成非法的 `{bundle:{targets:[nsis]}}`（内联 splat 同样会坏）。
- **结论**：凡需要叠加 config 的地方一律改用**文件路径** `--config <path>`（无引号、各 shell 都稳）。这也意味着旧文档里 `build:win -- --config '{...}'` 在 5.1 上其实是坏的（此前从没在 Windows 真打过 updater，所以未暴露）。

**脚本/配置改动（已落地）：**
- 新增叠加配置文件 `apps/desktop/tauri.win.conf.json`（`bundle.targets=["nsis"]`，`build:win` 始终叠加，锁定 Windows 仅产单一 NSIS）与 `apps/desktop/tauri.updater.conf.json`（`createUpdaterArtifacts=true`，按需 `--config` 传入，**跨平台通用**，mac/linux 也用它）。
- `scripts/desktop-build-win.ps1`：① 未知 `--xxx`（及其后续值）一律透传给 `tauri build`（对齐 `build:mac`，`build:win --config <path>` 不再需要 `--` 分隔符）；② `dist/desktop` 收集列表加 `*.sig`；③ 构建追加 `--config tauri.win.conf.json`；④ 汇总尺寸兼容小文件（KB）；⑤ 头部注释补 updater 用法 + 5.1 内联 JSON 告警。`--signed`（Authenticode）本轮未启用，其内联指纹注入在 5.1 下仍会坏，已加注释提示需 pwsh 7+。
- `scripts/make-latest-json.mjs` 报错提示统一到文件形态命令。

**私钥（本机）：** 已把 `apps/desktop/signing/nomifun-updater.key`（+`.key.pub`、`.env.signing`）放到本机，均被 gitignore（不入库）。已校验本机公钥与 `tauri.conf.json` 内嵌 `pubkey` 完全一致（keyID `F3AA272E60AA7952`）——签出的包客户端能验签通过。

**文档已统一（zh/en）：** `apps/desktop/updater/README.md(.zh-CN)`、根 `RELEASING.md(.zh-CN)` —— 全部改用 `--config apps/desktop/tauri.updater.conf.json`，并补「新构建机需先从密钥库拷私钥」前置步骤与 5.1 告警。

**阶段二命令（在本机执行）：**
```powershell
$env:TAURI_SIGNING_PRIVATE_KEY = Get-Content apps/desktop/signing/nomifun-updater.key -Raw
$env:TAURI_SIGNING_PRIVATE_KEY_PASSWORD = ""
bun run build:win --config apps/desktop/tauri.updater.conf.json   # 产 NomiFun_<ver>_x64-setup.exe + .sig
bun run make:latest                                               # 合并 windows-x86_64 进 latest.json（保留 darwin 两条）
```
之后建/补 `v<ver>` GitHub Release，上传 `setup.exe` + `.sig` + `latest.json`（已存在则 `--clobber` 替换 `latest.json`）。
