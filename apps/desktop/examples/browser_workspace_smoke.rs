...
#[cfg(windows)]
async fn verify_frame_sessions(
    view: &tauri::Webview,
    url: &str,
) -> Result<serde_json::Value, String> {
    use serde_json::json;
    let mut sessions = windows::frames::FrameSessions::connect(view).await?;
    if windows::frames::FrameSessions::connect(view).await.is_ok() {
        return Err("A second frame owner acquired the same native view.".into());
    }
    windows::protocol_call(view, "Page.navigate", json!({"url":url})).await?;
    let deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(5);
    let trees = loop {
        let trees = sessions.trees().await?;
        if trees.children.len() >= 2
            && trees.children.iter().all(|(_, tree)| {
                tree["frame"]["url"].as_str().is_some_and(|url| {
                    url.ends_with("/frame-child") || url.ends_with("/frame-nested")
                })
            })
        {
            break trees;
        ...
        if tokio::time::Instant::now() >= deadline {
            return Err(format!(
                "Real OOPIF sessions were not discovered: root={}, children={}",
                trees.root,
                trees.children.len()
            ));
        }
    ...
    if trees.descendant_count()? != 3 {
        return Err(
            "Frame discovery did not include both OOPIFs and the same-process child.".into(),
        );
    }
    if evaluate(
        view,
        "document.getElementById('foreign').contentDocument === null",
    )
    .await?
        != true
    {
        return Err("Cross-site fixture did not enforce the same-origin boundary.".into());
    }
    let mut evidence = vec![];
    for (frame, tree) in &trees.children {
        let context = sessions
            .command(
                frame,
                "Page.createIsolatedWorld",
                json!({
                    "frameId":frame.frame_id,"worldName":"nomi-frame-route-smoke"
                }),
            )
            .await?;
        let result = sessions.command(frame, "Runtime.evaluate", json!({
            "contextId":context["executionContextId"],"expression":"({url:location.href,title:document.title})",
            "returnByValue":true
        })).await?;
        let observed = &result["result"]["value"];
        if observed["url"] != tree["frame"]["url"] {
            return Err("Frame session reached a different document.".into());
        }
        evidence.push(observed.clone());
    }
    evaluate(view, "document.getElementById('foreign').remove(); true").await?;
    loop {
        let current = sessions.trees().await?;
        if current.children.is_empty() {
            break;
        }
        if tokio::time::Instant::now() >= deadline {
            return Err("Detached OOPIF routes remained live.".into());
        }
        tokio::time::sleep(std::time::Duration::from_millis(25)).await;
    }
    for (frame, _) in &trees.children {
        if sessions
            ...
            .is_ok()
        {
            return Err("Detached frame session was accepted.".into());
        }
    }
    Ok(
        json!({"isolated_worlds":evidence,"descendants":3,"detached_routes_rejected":true,"duplicate_owner_rejected":true}),
    )
}
...
#[cfg(windows)]
async fn verify_frame_input(view: &tauri::Webview, url: &str) -> Result<serde_json::Value, String> {
    ...
    driver
        ...
        .await
        .map_err(|error| error.to_string())?;
    windows::protocol_call(view, "Page.navigate", json!({"url":format!("{url}?input")})).await?;
    let deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(5);
    loop {
        if evaluate(
            view,
    ...
    for level in ["Cross-site", "Nested", "Same-process"] {
        ...
        driver
            ...
            .await
            .map_err(|error| format!("{level} native select: {error}"))?;
    }
...
#[cfg(windows)]
async fn wait_trusted_frame_event(view:&tauri::Webview,level:&str,kind:&str,target:&str)->Result<(),String> {
