...
impl ProcessSupervisor {
    ...
    async fn register_reserved(
        ...
        match commit {
            ...
            CommitResult::Retiring(retirement) => {
                ...
                Err(ProcessError::SupervisorShuttingDown)
            }
        }
    }
    ...
    pub fn terminal_outcome_if_ready(
        ...
        let session = match self.registry.inspect(session_id, owner) {
            ...
            Err(LookupError::NotFound) => {
                return Err(ProcessError::SessionNotFound {
    ...
    /// Stop a session because its hard process deadline elapsed. Unlike
    ...
    pub async fn shutdown(&self) -> ShutdownReport {
        ...
        loop {
            ...
            reports
                .changed()
                .await
                .expect("process shutdown report sender unexpectedly closed");
        }
    }
    ...
    async fn reserve_start_capacity(
        self: &Arc<Self>,
    ) -> Result<StartReservation, ProcessError> {
        loop {
            match self.registry.reserve_start() {
                Ok(reservation) => return Ok(reservation),
                Err(ReserveError::ShuttingDown) => {
                    return Err(ProcessError::SupervisorShuttingDown);
                }
...
async fn retire_session(session: Arc<Session>) -> ProcessOutcome {
    if let Some(terminal) = session.terminal() {
...
fn outcome_reaped(outcome: &ProcessOutcome) -> bool {
    match outcome {
        ProcessOutcome::Exited { cleanup, .. }
        | ProcessOutcome::Cancelled { cleanup, .. }
        | ProcessOutcome::TimedOut { cleanup, .. }
        | ProcessOutcome::Lost { cleanup, .. } => cleanup.reaped,
        ProcessOutcome::SpawnFailed(_) => true,
    }
}
...
async fn run_shutdown(registry: Arc<Registry>, shutdown: Arc<ShutdownState>) {
    ...
    loop {
        ...
        changes
            .changed()
            .await
            .expect("process registry change sender unexpectedly closed");
    }
}
...
impl Session {
    ...
    pub(crate) fn touch_at(&self, now: Instant) {
        *self
            .last_activity_at
            .lock()
            .expect("process session activity lock is poisoned") = now;
    }
    ...
    fn snapshot(&self) -> ProcessSnapshot {
        let process_state = {
            let state = self
                .state
                .lock()
                .expect("process session state lock is poisoned");
        ...
        ProcessSnapshot {
            ...
            last_activity_at: *self
                .last_activity_at
                .lock()
                .expect("process session activity lock is poisoned"),
        }
    }
    ...
    async fn wait_for_exit(&self) -> ExitObservation {
        ...
        loop {
            ...
            exits
                .changed()
                .await
                .expect("process exit watch closed while session is alive");
        }
    }
    ...
    fn force_lost(&self, error: String) -> ProcessOutcome {
    ...
    fn outcome(&self, terminal: &TerminalRecord, cursor: OutputCursor) -> ProcessOutcome {
        match &terminal.kind {
            ...
            TerminalKind::Cancelled { output } => ProcessOutcome::Cancelled {
                output: output.snapshot_from(cursor),
                cleanup: terminal.cleanup.clone(),
            },
            TerminalKind::TimedOut { output } => ProcessOutcome::TimedOut {
                output: output.snapshot_from(cursor),
                cleanup: terminal.cleanup.clone(),
            },
            TerminalKind::Lost { last_known, output } => ProcessOutcome::Lost {
    ...
    async fn wait_for_terminal(&self) -> TerminalRecord {
        ...
        loop {
            ...
            lifecycle
                .changed()
                .await
                .expect("process lifecycle watch closed while session is alive");
        }
    }
...
fn start_process_deadline(session: Arc<Session>) {
...
impl SignalStage {
    ...
    fn grace(self, policy: &ProcessPolicy) -> Duration {
        match self {
    ...
    async fn send(self, owner: &dyn PlatformProcess) -> std::io::Result<()> {
        match self {
...
fn owner_io_error(action: &'static str, error: std::io::Error) -> ProcessError {
    ProcessError::Io {
        operation: action,
...
mod tests {
    ...
    #[async_trait]
    impl PlatformProcess for FakeOwner {
        fn pid(&self) -> u32 {
            4_242
        }
    ...
    async fn dropping_the_handle_does_not_remove_the_owned_process() {
        ...
        let request = fake_request(ProcessPolicy::default());
    ...
    async fn hard_deadline_classifies_a_just_after_deadline_exit_as_timed_out() {
        ...
        assert!(matches!(
            &result,
            PollResult::Finished(ProcessOutcome::TimedOut { .. })
    ...
    async fn pre_deadline_exit_stays_exited_when_final_output_drain_crosses_deadline() {
        ...
        assert!(matches!(
            &result,
            PollResult::Finished(ProcessOutcome::Exited { code: Some(0), .. })
    ...
    async fn shutdown_waits_for_and_reports_a_reserved_session_that_commits_late() {
        let supervisor = ProcessSupervisor::new(SupervisorConfig::default());
        let request = fake_request(ProcessPolicy {
            interrupt_grace: Duration::from_millis(10),
            terminate_grace: Duration::from_millis(10),
            reap_grace: Duration::from_millis(10),
            ..ProcessPolicy::default()
    ...
    async fn terminal_inspection_waits_for_final_output_freeze() {
        ...
        let ProcessOutcome::Exited { output, .. } = outcome else {
    ...
    async fn retirement_signal_panic_becomes_lost_without_releasing_capacity() {
        ...
        let ProcessOutcome::Lost { cleanup, .. } = outcome else {
    ...
    async fn repeated_waiter_panic_publishes_lost_instead_of_stranding_waiters() {
        ...
        let PollResult::Finished(ProcessOutcome::Lost { cleanup, .. }) = result else {
    ...
    async fn unknown_session_and_wrong_owner_fail_closed_with_stable_codes() {
        ...
        let wrong_owner = ProcessOwner::new(uuid::Uuid::now_v7(), uuid::Uuid::now_v7());
    ...
    async fn natural_exit_preserves_nonfatal_platform_cleanup_diagnostics() {
        ...
        let ProcessOutcome::Exited { code, cleanup, .. } = outcome else {
    ...
    async fn unproven_reap_lost_wakes_a_far_future_poll_without_an_exit_fact() {
        ...
        assert!(matches!(cancelled, ProcessOutcome::Lost { .. }));
        let polled = poll
            .await
        ...
        assert!(matches!(
            polled,
            PollResult::Finished(ProcessOutcome::Lost { .. })
        ));
        assert_eq!(
            tokio::time::Instant::now(),
            observed_start + Duration::from_secs(5)
    ...
    async fn final_output_drain_never_extends_cancellation_past_five_seconds() {
        ...
        assert!(
            matches!(outcome, ProcessOutcome::Cancelled { .. }),
    ...
    async fn reap_published_after_lost_commit_does_not_rewrite_terminal_cleanup() {
        ...
        let first = supervisor
            ...
            .await
            .expect("cancel should return a terminal outcome");
        let ProcessOutcome::Lost { cleanup, .. } = &first else {
        ...
        let ProcessOutcome::Lost { cleanup, .. } = repeated else {
    ...
    fn finished(result: PollResult) -> ProcessOutcome {
        match result {
    ...
    async fn register_fake(
        ...
        register_fake_with_policy(fake, ProcessPolicy::default()).await
    }
    ...
    fn fake_request(policy: ProcessPolicy) -> NormalizedProcessRequest {
