use nomi_execution::{ExecutionEvent, OutputBuffer, OutputCursor, OutputStream};

const MAX_DECODED_TEXT_BYTES_PER_SOURCE_BYTE: usize = 4;

fn event_seq(event: &ExecutionEvent) -> u64 {
    match event {
        ExecutionEvent::Output { seq, .. }
        | ExecutionEvent::StateChanged { seq, .. }
        | ExecutionEvent::OutputDropped { seq, .. } => *seq,
    }
}

fn dropped_delta(events: &[ExecutionEvent]) -> u64 {
    events
        .iter()
        .map(|event| match event {
            ExecutionEvent::OutputDropped { bytes, .. } => *bytes,
            _ => 0,
        })
        .sum()
}

fn output_encoding(events: &[ExecutionEvent]) -> &nomi_execution::EncodingMetadata {
    match &events[0] {
        ExecutionEvent::Output { encoding, .. } => encoding,
...
fn active_windows_code_page_decodes_split_native_text() {
    ...
    for byte in bytes {
        ...
        if let ExecutionEvent::Output { text, .. } = &events[0] {
...
fn event_sequences_and_absolute_byte_offsets_are_monotonic() {
    ...
    assert!(matches!(events[2], ExecutionEvent::OutputDropped { bytes: 2, .. }));
    assert!(matches!(events[4], ExecutionEvent::OutputDropped { bytes: 1, .. }));
...
fn incremental_decoder_state_is_independent_per_stream() {
    ...
    assert!(matches!(
        &stdout_pending[0],
        ExecutionEvent::Output { text, .. } if text.is_empty()
    ));
    assert!(matches!(
        &stderr_pending[0],
        ExecutionEvent::Output { text, .. } if text.is_empty()
    ...
    assert!(matches!(
        &stdout_complete[0],
        ExecutionEvent::Output { text, .. } if text == "\u{4e2d}"
    ));
    assert!(matches!(
        &stderr_complete[0],
        ExecutionEvent::Output { text, .. } if text == "\u{1f642}"
    ));
