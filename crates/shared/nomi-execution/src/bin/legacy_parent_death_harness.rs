...
#[cfg(any(unix, windows))]
use nomi_execution::CommandBuilder;

#[cfg(unix)]
#[tokio::main(flavor = "current_thread")]
async fn main() {
    if let Err(error) = run().await {
        eprintln!("legacy parent-death harness failed: {error}");
        process::exit(2);
    }
...
async fn run() -> io::Result<()> {
    ...
    let mut builder = CommandBuilder::new(helper);
    builder
        .args([
    ...
    let leader = child
        .id()
        .ok_or_else(|| io::Error::other("legacy child exited before publishing its PID"))?;
