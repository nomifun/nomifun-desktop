...
#[cfg(unix)]
use nomi_execution::{
    CapabilityPolicy, CommandSpec, ExecutionOutcome, ExecutionOwner, ExecutionPolicy,
    NormalizedExecutionRequest, OutputCursor, PollResult, ProcessSupervisor, SupervisorConfig,
    Transport,
};
...
fn run() -> Result<(), String> {
    let helper = env::args_os()
        .nth(1)
        .ok_or_else(|| "expected the execution_test_helper path".to_owned())?;
...
#[cfg(unix)]
fn helper_request(helper: OsString, expected: i32) -> NormalizedExecutionRequest {
