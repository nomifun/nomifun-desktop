...
/// Compatibility command builder for legacy backend callers.
///
/// New Agent execution must use [`crate::ProcessSupervisor`]. This builder
/// exists only to keep non-Agent process adapters source-compatible while
/// centralizing environment hygiene and platform spawn setup in this crate.
...
impl CommandBuilder {
    ...
    pub fn spawn(self) -> io::Result<Child> {
        ...
        spawn_legacy(this.inner, this.hand_off)
    }
...
#[cfg(unix)]
fn spawn_legacy(command: Command, hand_off: bool) -> io::Result<Child> {
    crate::platform::unix::spawn_legacy(command, hand_off)
}

#[cfg(windows)]
fn spawn_legacy(command: Command, hand_off: bool) -> io::Result<Child> {
    crate::platform::windows::spawn_legacy(command, hand_off)
}

#[cfg(not(any(unix, windows)))]
fn spawn_legacy(mut command: Command, _hand_off: bool) -> io::Result<Child> {
    command.spawn()
}

pub async fn kill_legacy_process_tree(child: &mut Child) -> io::Result<()> {
    #[cfg(unix)]
    {
        return crate::platform::unix::kill_legacy_process_tree(child).await;
    }
    #[cfg(windows)]
    {
        return crate::platform::windows::kill_legacy_process_tree(child).await;
    }
    #[cfg(not(any(unix, windows)))]
    {
        child.kill().await?;
...
mod tests {
    ...
    fn clean_builder_strips_polluting_environment() {
        #[cfg(unix)]
        {
            let builder = CommandBuilder::clean_cli("example");
