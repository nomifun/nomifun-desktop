//! Minimal, stateless Workflow draft completion contract.
//!
//! This surface deliberately owns no Conversation, Skill, MCP, workflow, or
...
pub const MAX_WORKFLOW_DRAFT_PROMPT_UTF16: usize = 20_000;
/// Provider model ids are managed natural keys, not free-form prompts.
pub const MAX_WORKFLOW_DRAFT_MODEL_UTF16: usize = 512;
...
pub const WORKFLOW_DRAFT_TIMEOUT_SECS: u64 = 120;
/// A workflow draft is intentionally small and structurally bounded.
pub const WORKFLOW_DRAFT_MAX_TOKENS: u32 = 4_096;
/// Renderer limit for the JSON payload inside the canonical response fence.
pub const MAX_WORKFLOW_DRAFT_JSON_BYTES: usize = 262_144;
...
pub const MAX_WORKFLOW_DRAFT_RESPONSE_BYTES: usize = MAX_WORKFLOW_DRAFT_JSON_BYTES + 12;
...
pub const WORKFLOW_DRAFT_SYSTEM_PROMPT: &str = r#"You design one minimal NomiFun Creative Studio workflow draft for manual review.
...
Never include ids, revisions, timestamps, variables, visibility, tags, attachments, assets, or model bindings. Never save or run a workflow, call tools or another model, generate media, or claim that anything was persisted. The product will create private defaults only after the user reviews and explicitly applies the draft."#;

/// Validated request handed to the app-owned stateless completion runner.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct WorkflowDraftRunRequest {
...
pub trait WorkflowDraftRunner: Send + Sync {
...
pub fn workflow_draft_run_request(
    prompt: String,
    provider_id: String,
    model: String,
) -> Result<WorkflowDraftRunRequest, AppError> {
    ...
    if prompt.encode_utf16().count() > MAX_WORKFLOW_DRAFT_PROMPT_UTF16 {
        return Err(AppError::BadRequest(format!(
            "prompt is too long (max {MAX_WORKFLOW_DRAFT_PROMPT_UTF16} UTF-16 code units)"
        )));
    }
    ...
    if model.is_empty()
        || model.trim() != model
        || model.encode_utf16().count() > MAX_WORKFLOW_DRAFT_MODEL_UTF16
    {
        return Err(AppError::BadRequest(format!(
            "model.model must be a non-empty trimmed string no longer than {MAX_WORKFLOW_DRAFT_MODEL_UTF16} UTF-16 code units"
        )));
    }

    Ok(WorkflowDraftRunRequest {
        provider_id,
        model,
        system_prompt: WORKFLOW_DRAFT_SYSTEM_PROMPT,
...
mod tests {
    ...
    fn request_validation_is_strict_and_uses_utf16_limits() {
        ...
        let request = workflow_draft_run_request(
            "  设计一组社交海报  ".into(),
            provider_id.clone(),
            "chat-model".into(),
        )
        ...
        assert_eq!(request.system_prompt, WORKFLOW_DRAFT_SYSTEM_PROMPT);
        assert_eq!(WORKFLOW_DRAFT_TIMEOUT_SECS, 120);
        assert_eq!(MAX_WORKFLOW_DRAFT_JSON_BYTES, 262_144);
        assert_eq!(MAX_WORKFLOW_DRAFT_RESPONSE_BYTES, 262_156);
        assert_eq!(
            b"```json\n".len() + MAX_WORKFLOW_DRAFT_JSON_BYTES + b"\n```".len(),
            MAX_WORKFLOW_DRAFT_RESPONSE_BYTES
        );
        assert!(
            b"```json\n".len() + MAX_WORKFLOW_DRAFT_JSON_BYTES + 1 + b"\n```".len()
                > MAX_WORKFLOW_DRAFT_RESPONSE_BYTES
        );

        assert!(
            workflow_draft_run_request(" \r\n ".into(), ProviderId::new().into_string(), "m".into())
                .is_err()
        );
        assert!(
            workflow_draft_run_request(
                "😀".repeat(MAX_WORKFLOW_DRAFT_PROMPT_UTF16 / 2 + 1),
        ...
        assert!(
            workflow_draft_run_request("x".into(), "not-a-provider".into(), "m".into())
                .is_err()
        );
        assert!(
            workflow_draft_run_request(
        ...
        assert!(
            workflow_draft_run_request(
                "x".into(),
                ProviderId::new().into_string(),
                "m".repeat(MAX_WORKFLOW_DRAFT_MODEL_UTF16 + 1),
            )
            .is_err()
        );
    }
