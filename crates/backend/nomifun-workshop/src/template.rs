//! Canonical Creative Studio template definition v1.
//!
//! This is a closed server-owned persistence boundary.  It mirrors the
//! renderer's typed template domain, rejects unknown fields through Serde, and
//! validates graph/reference invariants before any definition reaches SQLite.

use std::collections::{BTreeSet, HashMap, VecDeque};

use nomifun_common::validate_uuidv7;
use nomifun_db::CreativeStudioTemplateRow;
use serde::{Deserialize, Serialize};

pub const MAX_TEMPLATE_DEFINITION_BYTES: usize = 8 * 1024 * 1024;
...
pub struct CreativeTemplateDefinitionV1 {
    pub id: String,
    pub revision: i64,
    pub metadata: CreativeTemplateMetadata,
    pub output: CreativeTemplateOutputPlan,
    pub variables: Vec<CreativeTemplateVariable>,
    pub templates: Vec<CreativePromptTemplate>,
    pub steps: Vec<CreativeTemplateStep>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct CreativeTemplateMetadata {
    pub name: String,
    pub description: String,
    pub category: String,
    pub visibility: CreativeTemplateVisibility,
    pub tags: Vec<String>,
    pub created_at: i64,
    pub updated_at: i64,
}
...
pub enum CreativeTemplateVisibility {
    Private,
    Public,
}

impl CreativeTemplateVisibility {
...
pub enum CreativeTemplateOutputPlan {
    SingleImage,
    MultiImageSeries {
        target_count: usize,
        concurrency: usize,
...
pub enum CreativeTemplateVariable {
    Text {
        id: String,
        key: String,
        label: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct CreativePromptTemplate {
    pub id: String,
    pub name: String,
    pub segments: Vec<CreativePromptTemplateSegment>,
}
...
pub enum CreativePromptTemplateSegment {
    Text { text: String },
...
pub enum CreativeTemplatePromptSource {
...
pub enum CreativeTemplateImageTask {
    ImageGeneration,
    ImageEdit,
}

impl CreativeTemplateImageTask {
    pub fn as_str(self) -> &'static str {
        match self {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct CreativeTemplateImageModelBinding {
    pub provider_id: String,
    pub model: String,
    pub task: CreativeTemplateImageTask,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
pub enum CreativeTemplateImageQuality {
    Auto,
    High,
    Medium,
    Low,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct CreativeTemplateImageGenerationSettings {
    pub model: Option<CreativeTemplateImageModelBinding>,
    pub quality: CreativeTemplateImageQuality,
    pub width: usize,
    pub height: usize,
...
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
pub enum CreativeTemplateTextTask {
    Chat,
}

impl CreativeTemplateTextTask {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::Chat => "chat",
        }
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct CreativeTemplateTextModelBinding {
    pub provider_id: String,
    pub model: String,
    pub task: CreativeTemplateTextTask,
}
...
impl CreativeTemplateImageGenerationSettings {
    fn validate(&self, path: &str) -> Result<(), String> {
        if let Some(model) = self.model.as_ref() {
            validate_id(&format!("{path}.model.providerId"), &model.provider_id)?;
            validate_text(&format!("{path}.model.model"), &model.model, 512, false)?;
...
pub enum CreativeTemplateStep {
    ...
    GenerateImages {
        ...
        depends_on: Vec<String>,
        enabled: bool,
        prompt_source: CreativeTemplatePromptSource,
...
impl CreativeTemplateDefinitionV1 {
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.metadata.created_at < 0 || self.metadata.updated_at < self.metadata.created_at {
            return Err("template metadata timestamps are invalid".into());
        }
        if self.variables.len() > MAX_VARIABLES
            || self.templates.len() > MAX_TEMPLATES
            || self.steps.len() > MAX_STEPS
        {
            return Err("template collection limit exceeded".into());
        }
        if self.templates.is_empty()
            || self.steps.is_empty()
            || !self.steps.iter().any(CreativeTemplateStep::is_generate_images)
        {
            return Err("template requires a prompt template and image-generation step".into());
        }
        if let CreativeTemplateOutputPlan::MultiImageSeries {
            target_count,
            concurrency,
            ..
        } = self.output
        ...
        for (index, variable) in self.variables.iter().enumerate() {
            ...
            if !owned_ids.insert(id) {
                return Err("template identifiers must be globally unique".into());
            }
        ...
        let variables: HashMap<&str, &CreativeTemplateVariable> = self
            .variables
            .iter()
        ...
        for (index, template) in self.templates.iter().enumerate() {
            ...
            for (segment_index, segment) in template.segments.iter().enumerate() {
                match segment {
                    CreativePromptTemplateSegment::Text { text } => validate_text(
    ...
    pub fn image_model_bindings(
        &self,
    ) -> impl Iterator<Item = &CreativeTemplateImageModelBinding> {
        self.steps.iter().filter_map(|step| match step {
            CreativeTemplateStep::GenerateImages { generation, .. } => generation.model.as_ref(),
            _ => None,
        })
    }
    ...
    pub fn text_model_bindings(
        &self,
    ) -> impl Iterator<Item = &CreativeTemplateTextModelBinding> {
        self.steps.iter().filter_map(|step| match step {
            CreativeTemplateStep::DraftPrompts { planning, .. } => planning.model.as_ref(),
            _ => None,
        })
    }

    pub fn to_row(&self) -> Result<CreativeStudioTemplateRow, String> {
        self.validate()?;
        let definition_json = serde_json::to_string(self)
            .map_err(|error| format!("failed to serialize template definition: {error}"))?;
        if definition_json.len() > MAX_TEMPLATE_DEFINITION_BYTES {
            return Err("template definition exceeds the 8 MiB limit".into());
        }
        Ok(CreativeStudioTemplateRow {
            id: 0,
...
pub fn parse_template_row(
    row: &CreativeStudioTemplateRow,
) -> Result<CreativeTemplateDefinitionV1, String> {
    if row.definition_json.len() > MAX_TEMPLATE_DEFINITION_BYTES {
        return Err("stored template definition exceeds the 8 MiB limit".into());
    }
    let definition: CreativeTemplateDefinitionV1 = serde_json::from_str(&row.definition_json)
        .map_err(|error| format!("stored template JSON is invalid: {error}"))?;
    definition.validate()?;
    if definition.id != row.template_id
    ...
    {
        return Err("stored template row metadata does not match its canonical definition".into());
    }
    Ok(definition)
}

fn validate_variable(variable: &CreativeTemplateVariable, index: usize) -> Result<(), String> {
    match variable {
        CreativeTemplateVariable::Text {
            default_value,
            placeholder,
            min_length,
            max_length,
        ...
        | CreativeTemplateVariable::MultilineText {
            default_value,
            placeholder,
            min_length,
            max_length,
        ...
        CreativeTemplateVariable::Number { default_value, minimum, maximum, step, .. } => {
        ...
        CreativeTemplateVariable::Choice { default_value, options, .. } => {
        ...
        CreativeTemplateVariable::Image { default_asset_id, .. } => {
            if let Some(id) = default_asset_id {
                validate_id(&format!("variables[{index}].defaultAssetId"), id)?;
            }
        }
        CreativeTemplateVariable::ImageSeries {
            default_asset_ids,
            min_items,
            max_items,
            ..
...
fn validate_step_references<'a>(
    ...
    index: usize,
    step: &CreativeTemplateStep,
    variables: &HashMap<&'a str, &'a CreativeTemplateVariable>,
    templates: &HashMap<&'a str, &'a CreativePromptTemplate>,
    steps: &HashMap<&'a str, &'a CreativeTemplateStep>,
) -> Result<(), String> {
    match step {
        CreativeTemplateStep::RenderTemplate { template_id, .. } => {
        ...
        CreativeTemplateStep::DraftPrompts {
            ...
            if !matches!(definition.output, CreativeTemplateOutputPlan::MultiImageSeries { .. }) {
                return Err("draft-prompts is only valid for a multi-image template".into());
            }
        ...
        CreativeTemplateStep::GenerateImages {
            ...
            match prompt_source {
                CreativeTemplatePromptSource::Template { template_id } => {
                    if !templates.contains_key(template_id.as_str())
                        || !matches!(definition.output, CreativeTemplateOutputPlan::SingleImage)
                    {
                        return Err(format!("steps[{index}] has an invalid template prompt source"));
                    }
                }
                CreativeTemplatePromptSource::PromptDrafts { step_id } => {
                    let valid_source = steps
                        .get(step_id.as_str())
                        .is_some_and(|source| matches!(source, CreativeTemplateStep::DraftPrompts { .. }));
                    if !valid_source
                        || !depends_on.contains(step_id)
                        || !matches!(definition.output, CreativeTemplateOutputPlan::MultiImageSeries { .. })
                    {
        ...
        CreativeTemplateStep::RecordHistory { depends_on, source_step_ids, .. } => {
            validate_unique_ids(
...
fn validate_acyclic(
    ordered_steps: &[CreativeTemplateStep],
    steps: &HashMap<&str, &CreativeTemplateStep>,
) -> Result<(), String> {
    ...
    if visited != ordered_steps.len() {
        return Err("template step graph must be acyclic".into());
    }
    Ok(())
}
...
mod tests {
    ...
    const DEFINITION_ID: &str = "0190f5fe-7c00-7a00-8abc-000000000201";
    const VARIABLE: &str = "0190f5fe-7c00-7a00-8abc-000000000202";
    const PROMPT_TEMPLATE_ID: &str = "0190f5fe-7c00-7a00-8abc-000000000203";
        ...
        CreativeTemplateDefinitionV1 {
            ...
            templates: vec![CreativePromptTemplate {
                ...
                segments: vec![
                    ...
                    CreativePromptTemplateSegment::Variable { variable_id: VARIABLE.into() },
    ...
    fn validates_and_round_trips_a_closed_definition() {
        ...
        let parsed: CreativeTemplateDefinitionV1 = serde_json::from_str(&json).unwrap();
    ...
    fn rejects_unknown_fields_broken_references_and_cycles() {
        ...
        assert!(serde_json::from_value::<CreativeTemplateDefinitionV1>(value).is_err());

        let mut missing = definition();
        if let CreativeTemplateStep::GenerateImages { depends_on, .. } = &mut missing.steps[1] {
        ...
        if let CreativeTemplateStep::RenderTemplate { depends_on, .. } = &mut cyclic.steps[0] {
            depends_on.push(GENERATE.into());
        }
        assert!(cyclic.validate().unwrap_err().contains("acyclic"));

        let mut mismatched = definition();
        if let CreativeTemplateStep::GenerateImages { generation, .. } = &mut mismatched.steps[1] {
            generation.model = Some(CreativeTemplateImageModelBinding {
                provider_id: "0190f5fe-7c00-7a00-8abc-000000000206".into(),
                model: "image-model".into(),
                task: CreativeTemplateImageTask::ImageEdit,
            });
        }
        assert!(
            mismatched
