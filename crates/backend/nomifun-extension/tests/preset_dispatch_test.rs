//! Integration coverage for the `/api/skills/preset-rule/*` and
//! `/api/skills/preset-skill/*` source dispatch introduced by T1b.
//!
//! Exercises the three dispatch paths (builtin → assets, extension → empty,
//! user → writable dir) using a fake [`PresetRuleDispatcher`] that
...
use nomifun_api_types::{ApiResponse, PresetSource};
use nomifun_common::AppError;
use nomifun_extension::classifier::{PresetClassifier, PresetRuleDispatcher};
...
impl PresetClassifier for FakeDispatcher {
...
#[async_trait::async_trait]
impl PresetRuleDispatcher for FakeDispatcher {
    ...
    async fn write_rule(&self, id: &str, locale: Option<&str>, content: &str) -> Result<(), AppError> {
        if self.reject_writes_for.contains(id) {
            return Err(AppError::BadRequest("Cannot write rule for built-in preset".into()));
        }
        self.log
            .lock()
            .unwrap()
    ...
    async fn delete_rule(&self, id: &str) -> Result<bool, AppError> {
        if self.reject_writes_for.contains(id) {
            return Err(AppError::BadRequest("Cannot delete rule for built-in preset".into()));
        }
    ...
    async fn write_skill(&self, id: &str, locale: Option<&str>, content: &str) -> Result<(), AppError> {
        if self.reject_writes_for.contains(id) {
            return Err(AppError::BadRequest("Cannot write skill for built-in preset".into()));
        }
        self.log
            .lock()
            .unwrap()
    ...
    async fn delete_skill(&self, id: &str) -> Result<bool, AppError> {
        if self.reject_writes_for.contains(id) {
            return Err(AppError::BadRequest(
                "Cannot delete skill for built-in preset".into(),
            ));
        }
...
fn read_body(preset_id: &str, locale: Option<&str>) -> Vec<u8> {
    let body = match locale {
        Some(loc) => serde_json::json!({ "preset_id": preset_id, "locale": loc }),
        None => serde_json::json!({ "preset_id": preset_id }),
    };
    serde_json::to_vec(&body).unwrap()
}

fn write_body(preset_id: &str, content: &str, locale: Option<&str>) -> Vec<u8> {
    let body = match locale {
