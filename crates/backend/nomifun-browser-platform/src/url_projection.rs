//! Safe URL identifiers for model-visible and non-address-bar metadata.
//!
//! Navigation and the user's address bar keep the original URL. These functions
//! only project metadata; they never mutate a browser, profile or runtime.

use url::Url;

const REDACTED_URL: &str = "[REDACTED_URL]";

pub fn project_metadata_url(raw: &str) -> String {
    if raw.len() > 8192 {
    ...
    // Projected metadata is only for identifying ordinary web pages. Internal
...
fn is_local_or_private_host(host: Option<&str>) -> bool {
    ...
    let address_text = host
        ...
        .unwrap_or(&host);
    let Ok(address) = address_text.parse::<std::net::IpAddr>() else {
        return false;
    };
    match address {
...
mod tests {
    ...
    fn preserves_normal_http_https_host_and_path_only() {
        assert_eq!(
            project_metadata_url("https://example.test/catalog/items"),
            "https://example.test/catalog/items"
        );
        assert_eq!(
            project_metadata_url("http://example.test:8080/docs/page%20one?lang=zh#section"),
    ...
    fn removes_userinfo_query_and_fragment() {
        let projected = project_metadata_url(
    ...
    fn rejects_non_page_schemes() {
        for raw in [
            ...
            assert_eq!(project_metadata_url(raw), REDACTED_URL, "{raw}");
        }
    }
    ...
    fn preserves_non_debug_loopback_pages_without_metadata() {
        ...
        assert_eq!(
            project_metadata_url("http://127.0.0.1:8080/docs/api-key"),
    ...
    fn malformed_urls_fail_closed() {
        assert_eq!(project_metadata_url("not a url?token=secret"), REDACTED_URL);
        assert!(!project_metadata_url("https://[invalid?token=secret").contains("secret"));
    }

    #[test]
    fn ipv6_and_ipv4_mapped_private_bridges_are_not_exposed() {
        for host in [
            "[::1]",
            "[fc00::1]",
            "[fe80::1]",
            ...
            let raw = format!("http://{host}:40123/page?endpoint=ws%3A%2F%2F127.0.0.1%3A9222");
            assert_eq!(project_metadata_url(&raw), REDACTED_URL, "{host}");
        }
        assert_eq!(
            project_metadata_url("http://[::1]:5173/app?code=secret"),
            "http://[::1]:5173/app"
        );
    }

    #[test]
    fn oversized_identifiers_are_rejected_before_or_after_normalization() {
        assert_eq!(
            project_metadata_url(&format!("https://example.test/{}", "x".repeat(8192))),
            REDACTED_URL
        );
        assert_eq!(
            project_metadata_url(&format!("https://example.test/{}", "页".repeat(1000))),
            REDACTED_URL
        );
    }
}
