...
pub fn preset_routes(state: PresetRouterState) -> Router {
    Router::new()
        ...
        .route("/api/preset-tags/{key}", axum::routing::put(update_tag).delete(delete_tag))
        .with_state(state)
}
...
async fn update_tag(State(s):State<PresetRouterState>,Path(key):Path<String>,body:Result<Json<UpdatePresetTagRequest>,JsonRejection>)->Result<Json<ApiResponse<PresetTagResponse>>,AppError>{let Json(r)=body.map_err(|e|AppError::BadRequest(e.to_string()))?;Ok(Json(ApiResponse::ok(s.service.update_tag(&key,r).await?)))}
async fn delete_tag(State(s):State<PresetRouterState>,Path(key):Path<String>)->Result<Json<ApiResponse<()>>,AppError>{s.service.delete_tag(&key).await?;Ok(Json(ApiResponse::success()))}
