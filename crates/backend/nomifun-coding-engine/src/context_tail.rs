...
) -> Result<Option<RecentToolExchange<'_>>, CodingEngineError> {
...
pub(crate) fn selected<'a>(
    ...
    call_ids: &[ToolCallId],
) -> Result<Option<RecentToolExchange<'a>>, CodingEngineError> {
...
impl<'a> RecentToolExchange<'a> {
    ...
    pub fn earlier(&self) -> Result<Option<Self>, CodingEngineError> {
    ...
    pub fn fits_text_bound(&self, requirements: &[ChatMessage]) -> Result<bool, CodingEngineError> {
    ...
    fn required_positions(
        &self,
        requirements: &[ChatMessage],
    ) -> Result<(usize, BTreeSet<usize>), CodingEngineError> {
