...
use crate::{CodingEffectClass, CodingToolBinding, CodingToolResult};
...
pub struct CodingPatchRecoveryState {
    pub version: u8,
    pub targets: Vec<String>,
    pub target_budget_exceeded: bool,
}

impl Default for CodingPatchRecoveryState {
    fn default() -> Self {
        Self {
            version: 1,
            targets: Vec::new(),
...
impl CodingPatchRecoveryState {
    pub fn has_pending(&self) -> bool {
        self.target_budget_exceeded || !self.targets.is_empty()
    }

    pub fn validate(&self) -> Result<(), crate::CodingEngineError> {
        ...
        {
            return Err(crate::CodingEngineError::InvalidContract(
...
pub(crate) struct PatchRecovery {
    ...
    persisted: Option<CodingPatchRecoveryState>,
}

impl PatchRecovery {
    pub(crate) fn restore(
        state: &CodingPatchRecoveryState,
    ) -> Result<Self, crate::CodingEngineError> {
        state.validate()?;
    ...
    fn snapshot(&self) -> CodingPatchRecoveryState {
        CodingPatchRecoveryState {
    ...
    pub(crate) async fn persist(
        &mut self,
        sink: &dyn crate::CodingEventSink,
    ) -> Result<(), crate::CodingEngineError> {
        let state = self.snapshot();
        if self.persisted.as_ref() != Some(&state) {
            sink.emit(crate::CodingEngineEvent::PatchRecoveryUpdated {
                state: state.clone(),
            })
            .await?;
    ...
    pub(crate) fn arm(&mut self, call: &ChatToolCall) -> Result<(), crate::CodingEngineError> {
        if self.pending() {
            return Err(crate::CodingEngineError::InvalidContract(
        ...
        if candidate.unaddressable || candidate.targets.is_empty() {
            return Err(crate::CodingEngineError::InvalidContract("Patch needs valid targets within the recovery budget before dispatch; correct or split the request".into()));
        }
    ...
    pub(crate) fn observe_read(&mut self, call: &ChatToolCall, result: &CodingToolResult) {
