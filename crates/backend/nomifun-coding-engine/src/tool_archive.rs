...
impl ToolArchive {
    ...
    pub async fn load(
        &mut self,
        call: &ChatToolCall,
        port: &dyn crate::CodingHistoryPort,
        ...
        let page = tokio::select! {
            ...
            result = port.read_previous(causality, args.before_turn.as_deref()) => match result {
                ...
                Err(_) => return Ok(CodingToolResult::text(call.call_id.clone(), "Historical read unavailable or outside its scoped budget. No historical tools were executed and no archive was changed.", true)),
            },
        };
        ...
        Ok(match result {
            Ok(value) => CodingToolResult::text(call.call_id.clone(), value.to_string(), false),
            Err(_) => CodingToolResult::text(call.call_id.clone(), json!({
    ...
    fn import(
        &mut self,
        page: crate::CodingHistoryPage,
        binding: &crate::EngineBinding,
    ) -> Result<Value, CodingEngineError> {
        ...
        let Some(crate::CodingEngineEvent::TurnStarted {
            binding: recorded,
            turn_operation_id,
        }) = turn.events.first()
        else {
        ...
        for event in &turn.events {
            match event {
                crate::CodingEngineEvent::ToolCallCompleted { step, call } if *step > 0 => {
                    calls.insert(&call.call_id, call);
                }
                crate::CodingEngineEvent::ToolCompleted { step, result } if *step > 0 => {
                    let call = calls
    ...
    pub fn handle(&self, call: &ChatToolCall) -> CodingToolResult {
        ...
        match result {
            Ok(value) => CodingToolResult::text(call.call_id.clone(), value.to_string(), false),
            Err(message) => CodingToolResult::text(call.call_id.clone(), message, true),
        }
    }
...
fn invalid(message: &str) -> CodingEngineError {
    CodingEngineError::ContextAssembly(message.into())
}
