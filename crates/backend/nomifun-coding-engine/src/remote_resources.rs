...
pub(crate) fn prepare(
    call: &ChatToolCall,
    image_input: bool,
) -> Result<PreparedRead, CodingToolResult> {
    ...
    if image {
        if !image_input {
            return Err(CodingToolResult::text(
                call.call_id.clone(),
    ...
    let request = match prepared {
        ...
        PreparedRead::Image(request) => {
            ...
            return match result {
                ...
                Err(error) => Ok(CodingToolResult::text(
                    call.call_id.clone(),
                    format!(
    ...
    Ok(match result {
        Ok(value) => {
            let is_error = value
                ...
                .unwrap_or(true);
            CodingToolResult::text(call.call_id.clone(), value.to_string(), is_error)
        }
        Err(error) => CodingToolResult::text(
            call.call_id.clone(),
            format!(
