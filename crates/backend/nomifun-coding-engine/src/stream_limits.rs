...
impl StreamBudget {
    pub(crate) fn admit(&mut self, event: &ChatModelEvent) -> Result<(), CodingEngineError> {
...
pub(crate) fn identity(id: &ToolCallId, name: &str) -> Result<(), CodingEngineError> {
    let id = id.as_ref();
    if id.trim().is_empty()
        || id.len() > 256
        || id.chars().any(char::is_control)
        || id.starts_with("coding-instructions:")
        || name.len() > 128
...
pub(crate) fn completed(call: &ChatToolCall) -> Result<(), CodingEngineError> {
...
fn invalid(message: &str) -> CodingEngineError {
    CodingEngineError::InvalidModelEvent(message.into())
}

pub(crate) fn serialized_size(
    value: &impl Serialize,
    limit: usize,
) -> Result<usize, CodingEngineError> {
    struct Counter {
        bytes: usize,
        limit: usize,
    }
