...
impl IProviderModelRepository for SqliteProviderModelRepository {
    ...
    async fn delete_coordinated(
        ...
        let agent_revision_binding: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM agent_preset_revisions revision, \
             json_tree(revision.payload_json) route \
             WHERE route.type = 'object' \
               AND json_extract(route.value, '$.provider_id') = ? \
               AND json_extract(route.value, '$.model') = ?)",
        )
        .bind(provider_id.as_str())
        .bind(&plan.model)
        .fetch_one(&mut *transaction)
        .await?;
        if agent_revision_binding {
            return Err(DbError::Conflict(format!(
                "provider model '{}/{}' is referenced by an Agent revision",
                provider_id, plan.model
            )));
        }
