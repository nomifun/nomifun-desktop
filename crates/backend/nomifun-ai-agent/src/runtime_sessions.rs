//! Process-local lifecycle pool for the official Runtime's live Sessions.
//!
//! A slot is keyed by canonical Agent Session ID and owns the live Runtime
//! instance reused across turns. It is not a persisted execution step or DAG
//! task; turn admission remains with the Session owner.
...
#[path = "runtime_session_shutdown.rs"]
mod shutdown;
#[path = "runtime_session_acquisition.rs"]
mod acquisition;

/// Factory function that creates an [`AgentRuntimeHandle`] from build options.
///
/// Async so the source-installed Driver can resolve canonical host ports and
/// model configuration without blocking the Session lifecycle owner.
...
pub type OfficialRuntimeOpener =
...
pub type RuntimeModelConfigResolver = Arc<
    dyn Fn(ProviderWithModel) -> BoxFuture<'static, Result<RuntimeModelConfigBinding, AppError>>
        + Send
        + Sync,
>;

/// Manages the lifecycle of active official Runtime Sessions.
///
...
#[async_trait]
pub trait AgentRuntimeSessions: Send + Sync {
    ...
    fn has_owned_runtime(&self, conversation_id: &str) -> bool {
...
/// Default implementation of [`AgentRuntimeSessions`] using a concurrent hash map.
#[derive(Clone)]
pub struct InMemoryAgentRuntimeSessions {
    ...
    factory: OfficialRuntimeOpener,
    model_config_resolver: Option<RuntimeModelConfigResolver>,
...
impl InMemoryAgentRuntimeSessions {
    pub fn new(factory: OfficialRuntimeOpener) -> Self {
        Self {
    ...
    pub fn with_model_config_resolver(
        mut self,
        resolver: RuntimeModelConfigResolver,
    ) -> Self {
    ...
    async fn terminate_owned_runtime(
        &self,
        conversation_id: &str,
        reason: Option<AgentKillReason>,
    ) -> Result<(), AppError> {
    ...
    async fn get_or_create_runtime_inner(
        ...
        let build = slot.get_or_try_init(|| async move {
            ...
            let factory_result = factory(options).await;
        ...
        let cancelled = user_cancelled || self.shutdown.closed.is_cancelled();
...
impl AgentRuntimeSessions for InMemoryAgentRuntimeSessions {
    ...
    fn terminate_and_wait_result(
        ...
        // Governor accounting happens inside `terminate_owned_runtime`,
        ...
        Box::pin(async move {
            registry
                .terminate_owned_runtime(&conversation_id, reason)
                .await
        })
    }
...
mod tests {
    ...
    /// A minimal mock Agent for testing Runtime Session lifecycle behavior.
    struct MockAgent {
    ...
    impl crate::OfficialAgentRuntime for MockAgent {
    ...
    #[tokio::test]
    async fn official_runtime_uses_singleflight_and_retains_failed_teardown_quarantine() {
        let calls = Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let factory_calls = calls.clone();
        let factory: OfficialRuntimeOpener = Arc::new(move |options| {
            factory_calls.fetch_add(1, Ordering::SeqCst);
            async move {
                Ok(AgentRuntimeHandle::Official(Arc::new(
        ...
        match (&first, &second) {
            (AgentRuntimeHandle::Official(a), AgentRuntimeHandle::Official(b)) => {
                assert!(Arc::ptr_eq(a, b));
            }
            _ => panic!("production must use the official Runtime handle"),
        }
    ...
    #[tokio::test]
    async fn official_runtime_optional_mutations_fail_closed() {
        let runtime = AgentRuntimeHandle::Official(Arc::new(MockAgent::new("custom", None)));
    ...
    fn make_registry() -> InMemoryAgentRuntimeSessions {
        let factory: OfficialRuntimeOpener = Arc::new(|opts: AgentRuntimeBuildOptions| {
            async move { Ok(mock_runtime(MockAgent::new(&opts.conversation_id, None))) }.boxed()
        });
        InMemoryAgentRuntimeSessions::new(factory)
    }
    ...
    #[tokio::test]
    async fn provider_config_revision_change_recycles_official_runtime_before_next_turn() {
        ...
        let resolver: RuntimeModelConfigResolver = Arc::new(move |selection| {
    ...
    async fn knowledge_runtime_metadata_without_workspace_lease_is_rejected_before_factory() {
        ...
        assert!(
            !registry.has_owned_runtime("conv-unleased-knowledge"),
    ...
    async fn workspace_lease_for_different_physical_directory_is_rejected_before_factory() {
        ...
        assert!(
            !registry.has_owned_runtime("conv-mismatched-workspace-lease"),
    ...
    async fn failed_runtime_teardown_retains_workspace_binding_authority() {
        let workspace = tempfile::tempdir().unwrap();
        let factory: OfficialRuntimeOpener = Arc::new(|options| {
            async move {
                Ok(mock_runtime(
                    MockAgent::new(
                        &options.conversation_id,
    ...
    async fn get_or_create_is_single_flight_under_concurrency() {
        ...
        let factory: OfficialRuntimeOpener = Arc::new(move |opts: AgentRuntimeBuildOptions| {
        ...
        let registry = Arc::new(InMemoryAgentRuntimeSessions::new(factory));
    ...
    async fn synchronous_terminate_during_initialization_does_not_leak_runtime() {
        ...
        let factory: OfficialRuntimeOpener = {
    ...
    fn terminate_nonexistent_is_ok() {
        let factory: OfficialRuntimeOpener = Arc::new(|_| async { unreachable!() }.boxed());
        let registry = InMemoryAgentRuntimeSessions::new(factory);
    ...
    async fn shutdown_keeps_exact_runtime_owned_when_its_caller_drops() {
        ...
        let release=Arc::new(Semaphore::new(0));
        let registry=InMemoryAgentRuntimeSessions::new(Arc::new({
        ...
        drop(registry.shutdown_and_wait());
        entered.acquire().await.unwrap().forget();
        assert!(registry.has_owned_runtime("shutdown-owned"));
        ...
        tokio::time::timeout(std::time::Duration::from_secs(2),async {
            while registry.has_owned_runtime("shutdown-owned") {tokio::task::yield_now().await;}
        }).await.unwrap();
        registry.shutdown_and_wait().await.unwrap();
    }
    ...
    async fn shutdown_drains_a_cold_factory_without_publishing_its_runtime() {
        ...
        entered.acquire().await.unwrap().forget();
        let closing=tokio::spawn(registry.shutdown_and_wait());
        ...
        assert!(!registry.has_owned_runtime("shutdown-build"));
    ...
    async fn active_turn_generation_cannot_be_overwritten_and_stale_release_is_absorbing() {
        let registry = InMemoryAgentRuntimeSessions::new(Arc::new(|options| {
            async move {
                Ok(mock_runtime(MockAgent::new(
                    &options.conversation_id,
                    Some(ConversationStatus::Finished),
