...
//! live SSH connection. The trait lives here in `nomifun-ai-agent`; the implementation
//! lives in `crates/backend/nomifun-ssh` and is reached through the
//! `nomifun-ai-agent` re-export — so the Runtime never depends on russh or the
//! backend.
//!
