...
#[derive(Debug)]
pub struct ImageAttachmentError(String);
...
pub(crate) async fn load_image_blocks(
...
/// Shared host-side preparation for bundled engines. Callers must first bind
/// references to an accepted delivery and supply the Session's attachment root.
/// This reads attachments only; it does not grant general filesystem access.
pub async fn load_image_parts(
    file_references: &[String],
    trusted_root: Option<&Path>,
) -> Result<Vec<nomifun_chat_model_broker::ChatContentPart>, ImageAttachmentError> {
    load_image_blocks(file_references, trusted_root).await?.into_iter().map(|block| {
        match block {
            ContentBlock::Image { media_type, data } => Ok(nomifun_chat_model_broker::ChatContentPart::Image {
                media_type, data_base64: data,
            }),
            _ => Err(attachment_error("image preparation returned non-image content")),
        }
    }).collect()
}

/// Classify without opening a file, so missing vision authority fails before IO.
pub fn has_image_references(file_references: &[String]) -> Result<bool, ImageAttachmentError> {
    let mut found = false;
    for reference in file_references {
        found |= classify_extension(Path::new(reference))?.is_some();
    }
    Ok(found)
}
...
/// Prepare already-authorized immutable bytes without opening any path. The
/// host must verify the artifact inventory/digest before calling this blocking
/// decoder; the reference name selects the expected format, not an IO target.
pub fn prepare_image_resource(
    bytes: &[u8],
    reference_name: &str,
) -> Result<nomifun_chat_model_broker::ChatToolResultPart, ImageAttachmentError> {
    if bytes.len() as u64 > MAX_SOURCE_BYTES {
        return Err(attachment_error("image resource exceeds the source byte limit"));
    }
    let path = Path::new(reference_name);
    let format = classify_extension(path)?
        .ok_or_else(|| attachment_error("image resource must be PNG, JPEG or WebP"))?;
    let prepared = prepare_image(bytes, format, &display_name(path))?;
    Ok(nomifun_chat_model_broker::ChatToolResultPart::Image {
        media_type: prepared.media_type.into(),
        data_base64: STANDARD.encode(prepared.bytes),
    })
}
