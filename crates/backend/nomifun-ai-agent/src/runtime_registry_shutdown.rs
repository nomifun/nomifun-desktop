...
use super::{AgentRuntimeRegistry, InMemoryAgentRuntimeRegistry};
...
pub(super) fn start(registry: &InMemoryAgentRuntimeRegistry) -> crate::RuntimeTeardown {
...
async fn drain(registry: &InMemoryAgentRuntimeRegistry) -> Result<(), String> {
    ...
    let mut results = stream::iter(ids)
        .map(|id| {
            ...
            async move {
                ...
                registry.terminate_registered_runtime(&id, None).await
            }
        })
        .buffer_unordered(16);
