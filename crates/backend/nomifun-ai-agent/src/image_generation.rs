...
use nomifun_api_types::{derive_tasks_and_traits, HealthStatus, ModelHealthStatus, ModelTask};
...
impl ImageGenerationToolDiscovery for CatalogImageGenerationToolDiscovery {
    async fn discover_tool(&self) -> Result<Option<Box<dyn Tool>>, AppError> {
        Ok(discover_image_generation_capability(
            ...
            self.provider_model_repo.clone(),
...
pub async fn discover_image_generation_capability(
    ...
    for provider in providers {
        ...
        for row in rows {
            if !row.enabled
                || !row_supports_image_generation(&provider.platform, &row.model, &row.tasks)
                || row_is_unhealthy(row.health.as_deref())
            {
                continue;
            }
            let model = ModelRef {
...
fn row_supports_image_generation(platform: &str, model: &str, raw_tasks: &str) -> bool {
    let tasks = serde_json::from_str::<Vec<ModelTask>>(raw_tasks).unwrap_or_default();
    if tasks.is_empty() {
        derive_tasks_and_traits(platform, model)
            .0
            .contains(&ModelTask::ImageGeneration)
    } else {
        tasks.contains(&ModelTask::ImageGeneration)
    }
}

fn row_is_unhealthy(raw_health: Option<&str>) -> bool {
    raw_health
        .and_then(|raw| serde_json::from_str::<ModelHealthStatus>(raw).ok())
...
mod tests {
    ...
    use nomifun_db::{
        CreateProviderParams, DbError, IClientPreferenceRepository, NewProviderModel,
