...
#[async_trait]
pub trait AgentWorkspaceReader: Send + Sync {
    async fn read_text(
        &self,
...
pub async fn load_agents_md(
    reader: &dyn AgentWorkspaceReader,
    working_directory: &str,
    policy: AgentsMdPolicy,
    cancellation: CancellationToken,
) -> Result<AgentsMdContext, AgentEngineError> {
...
pub(crate) fn normalize_workspace_directory(value: &str) -> Result<String, AgentEngineError> {
    if value.len() > 4096 {
        return Err(AgentEngineError::WorkspaceContext(
...
mod tests {
    ...
    #[async_trait]
    impl AgentWorkspaceReader for MemoryWorkspace {
        async fn read_text(
            &self,
            workspace_relative_path: &str,
    ...
    async fn rejects_workspace_escape_without_calling_the_owner() {
        ...
        assert!(matches!(
            ...
            .await,
            Err(AgentEngineError::WorkspaceContext(_))
        ));
