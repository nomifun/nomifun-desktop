...
use crate::{
    AgentEngineError, AgentEngineEvent, AgentEventSink, AgentInputCitation, AgentPlan,
    AgentRequirementOrigin, AgentToolResult, AgentWorkStatus, EngineBinding,
};

pub(crate) const TOOL_NAME: &str = "resume_task";

#[derive(Clone, Debug, Serialize)]
pub struct AgentPriorTask {
...
struct HistoricalCriterion {
    ...
    disposition: crate::AgentCriterionDisposition,
    rationale: String,
    scope_change: Option<AgentInputCitation>,
}
...
impl AgentPriorTask {
    ...
    pub fn from_closed_turn(
    ...
    ) -> Result<Option<Self>, AgentEngineError> {
        let fail = |reason: &str| AgentEngineError::ReplayContract(reason.into());
        ...
        if operation.is_empty()
            ...
            || events
                .iter()
                .filter(|event| matches!(event, AgentEngineEvent::TurnStarted { .. }))
                .count()
                != 1
        {
        ...
        let terminal = match events.last() {
            Some(AgentEngineEvent::TurnCompleted { .. }) => "completed",
            Some(AgentEngineEvent::TurnCancelled { .. }) => "cancelled",
        ...
        let Some(plan) = events
            ...
            .find_map(|event| match event {
                AgentEngineEvent::PlanUpdated { plan } => Some(plan),
                _ => None,
            })
        ...
        let historical_account = events.iter().rev().find_map(|event| match event {
            AgentEngineEvent::CompletionReported { report } => Some(
                report
                    .criteria
                    .iter()
...
pub(crate) async fn resume(
    candidate: Option<&AgentPriorTask>,
    call: &ChatToolCall,
    plan: &mut AgentPlan,
    ...
    let reject = |reason: String| AgentToolResult::text(call.call_id.clone(), reason, true);
        ...
        .map(|old| {
            ...
            requirement.origin =
                Some(
                    old.origin
                        .clone()
                        .unwrap_or_else(|| AgentRequirementOrigin {
