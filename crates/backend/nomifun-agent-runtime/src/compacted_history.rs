...
pub enum AgentCompactedItem {
    AcceptedInput { index: usize },
    Message { message: ChatMessage },
}

fn invalid(message: impl std::fmt::Display) -> AgentEngineError {
    AgentEngineError::ReplayContract(format!("compaction replacement: {message}"))
}
...
pub(crate) fn capture(
    ...
    call_ids: &[ToolCallId],
) -> Result<Vec<AgentCompactedItem>, AgentEngineError> {
    ...
    for (position, message) in messages.iter().enumerate() {
        let item = if let Some(index) = positions.get(&position) {
            AgentCompactedItem::AcceptedInput { index: *index }
        } else {
            AgentCompactedItem::Message {
...
pub(crate) fn restore(
    ...
    for item in items {
        match item {
            AgentCompactedItem::AcceptedInput { index } => {
                if *index != next_input {
            ...
            AgentCompactedItem::Message { message } => {
    ...
    if call_ids.is_empty() {
        if items
            .iter()
            .any(|item| matches!(item, AgentCompactedItem::Message { .. }))
        {
...
fn instruction_reads(messages: &[ChatMessage]) -> BTreeSet<ToolCallId> {
    messages
        ...
        .filter_map(|part| {
            ...
            (call_id.as_ref().starts_with("agent-instructions:")
                || (name == "read_file"
                    && arguments
                        .0
                        .get("path")
...
) -> Result<ChatMessage, AgentEngineError> {
...
fn validate_portable(message: &ChatMessage) -> Result<(), AgentEngineError> {
