//! Compatibility names for the shared engine-neutral Broker port. The Runtime
//! core retains its public API without owning another provider adapter.
pub use nomifun_chat_model_broker::{
    BrokerEngineModelPort as BrokerAgentModelPort, EngineModelPort as AgentModelPort,
    EngineModelStream as AgentModelStream,
};
...
mod tests {
    ...
    async fn adapter_rejects_broker_without_native_cancellation() {
        let adapter = BrokerAgentModelPort::new(Arc::new(LegacyOnlyBroker));
    ...
    impl ChatBrokerPort for CancellableBroker {
        async fn open_chat_stream(
            ...
            panic!("the Runtime must not fall back to the uncancellable broker port");
        }
    ...
    async fn adapter_propagates_cancellation_during_broker_open() {
        ...
        let adapter = BrokerAgentModelPort::new(Arc::new(CancellableBroker(entered.clone())));
