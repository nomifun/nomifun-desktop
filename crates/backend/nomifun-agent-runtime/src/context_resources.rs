...
pub use nomifun_engine_core::{
    EngineContextContent as AgentContextContent,
    EngineContextResource as AgentContextResource,
};

pub(crate) fn index(
    resources: &BTreeMap<String, AgentContextResource>,
    image_input: bool,
...
pub(crate) fn read(
    ...
    let invalid = |reason: &str| AgentToolResult::text(call.call_id.clone(), reason, true);
    ...
    let text = match &resource.content {
        AgentContextContent::Text { text } => text,
        AgentContextContent::Image {
            media_type,
            data_base64,
        } => {
    ...
    loop {
        ...
        if encoded_bytes <= MAX_ENCODED_PAGE_BYTES {
            return AgentToolResult::text(call.call_id.clone(), page, false);
        }
