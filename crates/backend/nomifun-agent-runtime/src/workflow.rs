...
pub(crate) const LONG_HORIZON_EXECUTION_INSTRUCTIONS: &str = concat!(
    ...
    "An expected failing test within an already active in_progress step can be inspected and repaired under that plan; update the plan if the approach changes. Do not call report_completion while a plan step is open. ",
    ...
    "Use dedicated read-only file or VCS tools for final inspection. Then close the plan and cite the latest usable call ID in report_completion. ",
