...
#[async_trait]
pub trait AgentLiveContextPort: Send + Sync + std::fmt::Debug {
...
pub(crate) async fn read(
    ...
    let data = tokio::select! {
        ...
        value = tokio::time::timeout(std::time::Duration::from_secs(10), port.read(causality, generation)) => {
            value.map_err(|_| AgentEngineError::ContextAssembly("live context timed out".into()))??
        }
    };
