...
use nomifun_api_types::{
    ExportMiniAppBackupRequest, ImportMiniAppBackupRequest, MiniAppKindDto, MiniAppLifecycleDto,
};
use nomifun_db::{
...
use nomifun_miniapp_platform::{
    ...
    MiniAppM1ApplicationService, MiniAppPlatformError, MiniAppPlatformResult,
    MiniAppReleaseFileBytes, MiniAppReleasePublishRequest, MiniAppReleaseStore,
    MiniAppPrivateDatabasePort, MiniAppServiceHostState, MiniAppServiceRuntimeBinding,
    MiniAppServiceSpecInput, MiniAppServiceStoragePort, MiniAppServiceStorageRequest,
    MiniAppSourceStore, MiniAppStaticBundleBuilder, MiniAppStaticBundleInput,
    MiniAppStaticServiceInput, MiniAppStoredRelease, MiniAppBackupStorage,
...
struct BackupRuntime {
    storage: Arc<SqliteMiniAppManagedStorage>,
}

#[async_trait]
impl MiniAppServiceRuntimeBinding for BackupRuntime {
    ...
    async fn start(&self, _spec: ResolvedMiniAppServiceSpec) -> MiniAppPlatformResult<()> {
        Ok(())
    }
...
async fn service_whole_app_backup_roundtrips_files_private_sqlite_and_migration_ledger() {
    ...
    let source_store = Arc::new(MiniAppSourceStore::new(root.path().join("source")).unwrap());
    let release_store = Arc::new(MiniAppReleaseStore::new(root.path().join("release")).unwrap());
    let storage = Arc::new(
        SqliteMiniAppManagedStorage::new(root.path().join("managed"), database.pool().clone())
            .unwrap(),
    );
    let application = MiniAppM1ApplicationService::new_with_stores(
        repository.clone(),
        source_store,
        release_store.clone(),
    )
    ...
    let published = release_store
        .publish(MiniAppReleasePublishRequest::service(
            nomifun_miniapp_platform::MiniAppSourceScope::new(
                &owner,
                &miniapp_id,
                &project_id,
            )
    ...
    let ledger = MiniAppServiceStoragePort::apply_additive_migrations(
            storage.as_ref(),
            &owner,
            &miniapp,
            &resolved.descriptor,
    ...
    let query = storage
        ...
        .await
        .unwrap();
    let query: MiniAppDatabaseQueryResult = serde_json::from_value(query.0).unwrap();
    assert_eq!(
        query.rows,
...
fn release_files(artifact: &MiniAppReleaseArtifactV1) -> Vec<MiniAppReleaseFileBytes> {
    artifact
        .files
        .iter()
        .map(|file| {
            let bytes = if file.normalized_relative_path == "ui/index.html" {
                ...
                nomifun_miniapp_platform::materialize_surface_entrypoint(source).unwrap()
            } else {
...
fn artifact_row(
    ...
    published: &MiniAppStoredRelease,
