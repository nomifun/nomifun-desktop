...
use crate::{
    MiniAppReleaseFileBytes, MiniAppSourceFile, MiniAppSourceFileDigest,
    MiniAppSourceSnapshot, MiniAppStoredRelease, MINIAPP_SOURCE_STORE_FORMAT_VERSION,
};
...
pub struct MiniAppShareBundleLimits {
...
impl Default for MiniAppShareBundleLimits {
    fn default() -> Self {
        Self {
...
impl MiniAppShareBundleLimits {
    fn validate(self) -> Result<Self, MiniAppShareBundleError> {
        ...
        {
            return Err(MiniAppShareBundleError::InvalidLimits);
        }
        Ok(self)
    }
}
...
#[derive(Debug, Error)]
pub enum MiniAppShareBundleError {
    #[error("MiniApp Share Bundle limits are invalid")]
    InvalidLimits,
    #[error("MiniApp Share Bundle input is invalid: {0}")]
    InvalidInput(String),
    #[error("MiniApp Share Bundle path is invalid: {path} ({reason})")]
    InvalidPath { path: String, reason: String },
    #[error("MiniApp Share Bundle destination already exists: {0}")]
    DestinationExists(PathBuf),
    #[error("MiniApp Share Bundle inventory is invalid: {0}")]
    InvalidInventory(String),
    #[error("MiniApp Share Bundle digest or byte inventory was modified: {0}")]
    Tampered(String),
    #[error("MiniApp Share Bundle exceeds a size limit: {0}")]
    Oversize(String),
    #[error("MiniApp Share Bundle filesystem operation failed for {path}: {source}")]
    Io {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("MiniApp Share Bundle canonical serialization failed: {0}")]
    Canonical(String),
}

pub struct MiniAppShareSourceExport<'a> {
    pub snapshot: &'a MiniAppSourceSnapshot,
    pub source_archive_artifact_id: ArtifactId,
    pub dependency_lock_artifact_id: ArtifactId,
}

pub struct MiniAppShareBundleExport<'a> {
    pub bundle_id: MiniAppShareBundleId,
    pub source_miniapp_id: Option<MiniAppId>,
    pub release: &'a MiniAppStoredRelease,
    pub source: Option<MiniAppShareSourceExport<'a>>,
    pub test_provenance: Option<MiniAppImportedTestProvenance>,
}

#[derive(Clone, Debug, PartialEq)]
pub struct MiniAppImportedRelease {
...
pub struct MiniAppImportedSource {
...
#[derive(Clone, Debug, PartialEq)]
pub struct MiniAppImportedShareBundle {
    pub bundle: MiniAppShareBundleV1,
    pub release: MiniAppImportedRelease,
    pub source: Option<MiniAppImportedSource>,
}

#[derive(Clone, Debug, PartialEq)]
pub enum MiniAppShareImportEntry {
    ShareBundle(MiniAppImportedShareBundle),
    PrebuiltRelease(MiniAppImportedRelease),
}

#[derive(Clone, Debug)]
pub struct MiniAppShareBundleFilesystem {
    limits: MiniAppShareBundleLimits,
}

impl Default for MiniAppShareBundleFilesystem {
    fn default() -> Self {
        Self {
            limits: MiniAppShareBundleLimits::default(),
        }
    }
}

impl MiniAppShareBundleFilesystem {
    pub fn new(limits: MiniAppShareBundleLimits) -> Result<Self, MiniAppShareBundleError> {
        Ok(Self {
    ...
    pub fn export(
        &self,
        request: MiniAppShareBundleExport<'_>,
        destination: impl AsRef<Path>,
    ) -> Result<MiniAppShareBundleV1, MiniAppShareBundleError> {
        let prepared = prepare_export(request, self.limits)?;
        let destination = destination.as_ref();
        let parent = destination.parent().ok_or_else(|| {
            MiniAppShareBundleError::InvalidInput(
        ...
        match fs::symlink_metadata(destination) {
            Ok(_) => {
                return Err(MiniAppShareBundleError::DestinationExists(
    ...
    ) -> Result<MiniAppImportedShareBundle, MiniAppShareBundleError> {
    ...
    pub fn import_prebuilt_release(
        &self,
        root: impl AsRef<Path>,
    ) -> Result<MiniAppImportedRelease, MiniAppShareBundleError> {
        let root = root.as_ref();
        let observed = scan_tree(root, self.limits)?;
        if observed.files.contains(BUNDLE_FILE) {
            return Err(MiniAppShareBundleError::InvalidInventory(
    ...
    pub fn import(
        &self,
        root: impl AsRef<Path>,
    ) -> Result<MiniAppShareImportEntry, MiniAppShareBundleError> {
        let root = root.as_ref();
        let observed = scan_tree(root, self.limits)?;
        ...
        match (has_bundle, has_artifact) {
            (true, false) => {
                import_share_bundle_with_inventory(root, observed, self.limits)
                    .map(MiniAppShareImportEntry::ShareBundle)
            }
            (false, true) => import_release(root, "", &observed, self.limits)
                .map(MiniAppShareImportEntry::PrebuiltRelease),
            _ => Err(MiniAppShareBundleError::InvalidInventory(
...
) -> Result<usize, MiniAppShareBundleError> {
...
struct PreparedExport {
    ...
    release: MiniAppImportedRelease,
...
fn prepare_export(
    request: MiniAppShareBundleExport<'_>,
    limits: MiniAppShareBundleLimits,
) -> Result<PreparedExport, MiniAppShareBundleError> {
    ...
    let (source_contract, source) = match request.source {
        Some(source) => {
            let source_miniapp_id = request.source_miniapp_id.as_ref().ok_or_else(|| {
                MiniAppShareBundleError::InvalidInput(
                    "a Share Bundle carrying Source must identify its source MiniApp".into(),
                )
            })?;
            if &source.snapshot.project.miniapp_id != source_miniapp_id {
                return Err(MiniAppShareBundleError::InvalidInput(
                    "source MiniApp identity does not match the Source snapshot".into(),
                ));
            }
    ...
    .map_err(|error| MiniAppShareBundleError::InvalidInput(error.to_string()))?;
    Ok(PreparedExport {
        bundle,
        release,
        source,
...
fn validate_release_payload(
    ...
    mut files: Vec<MiniAppReleaseFileBytes>,
    limits: MiniAppShareBundleLimits,
) -> Result<MiniAppImportedRelease, MiniAppShareBundleError> {
    artifact
        .validate()
    ...
    if artifact.files.len() > limits.max_release_file_count {
        return Err(MiniAppShareBundleError::Oversize(
    ...
    if manifest_bytes != expected_manifest {
        return Err(MiniAppShareBundleError::Tampered(
    ...
    for (actual, expected) in files.iter().zip(&artifact.files) {
        ...
        if !collisions.insert(portable_collision_key(&actual.normalized_relative_path)?) {
            return Err(MiniAppShareBundleError::InvalidPath {
        ...
        {
            return Err(MiniAppShareBundleError::Tampered(format!(
...
fn validate_source_payload(
    mut files: Vec<MiniAppSourceFile>,
    dependency_lock: Vec<u8>,
    expected_snapshot_digest: DigestHex,
    expected_lock_digest: DigestHex,
    limits: MiniAppShareBundleLimits,
) -> Result<PreparedSource, MiniAppShareBundleError> {
    ...
    for file in &files {
        ...
        digest_files.push(MiniAppSourceFileDigest {
...
) -> Result<(), MiniAppShareBundleError> {
...
fn write_files<'a>(
    ...
    for (relative, bytes) in files {
        ...
        let parent = target.parent().ok_or_else(|| {
            MiniAppShareBundleError::InvalidInventory("file has no parent directory".into())
        })?;
...
fn import_share_bundle_with_inventory(
    ...
    bundle
        .validate()
        .map_err(|error| MiniAppShareBundleError::Tampered(error.to_string()))?;
...
fn import_release(
    ...
    for file in &artifact.files {
        ...
        files.push(MiniAppReleaseFileBytes::new(
...
fn import_source(
    ...
    observed: &ObservedTree,
    limits: MiniAppShareBundleLimits,
) -> Result<MiniAppImportedSource, MiniAppShareBundleError> {
    let source_root = root.join(SOURCE_DIRECTORY);
    ...
    let imported = MiniAppImportedSource {
...
fn source_file_inventory(files: &[MiniAppSourceFile]) -> BTreeSet<String> {
...
fn require_exact_inventory(
    ...
    if &observed.files != expected_files {
        ...
        let missing = expected_files
            ...
            .cloned()
            .collect::<Vec<_>>();
        return Err(MiniAppShareBundleError::InvalidInventory(format!(
...
fn scan_tree(
    root: &Path,
    limits: MiniAppShareBundleLimits,
) -> Result<ObservedTree, MiniAppShareBundleError> {
...
fn scan_directory(
    ...
    for entry in fs::read_dir(current).map_err(|error| io_error(current, error))? {
        ...
        let relative = path.strip_prefix(root).map_err(|_| {
            MiniAppShareBundleError::InvalidInventory("bundle entry escaped its root".into())
        })?;
...
fn source_snapshot_digest(
    files: &[MiniAppSourceFileDigest],
) -> Result<DigestHex, MiniAppShareBundleError> {
...
fn read_canonical<T: DeserializeOwned + Serialize>(
    path: &Path,
    limit: u64,
) -> Result<T, MiniAppShareBundleError> {
...
fn read_regular_bounded(
    path: &Path,
    limit: u64,
) -> Result<Vec<u8>, MiniAppShareBundleError> {
    let metadata = fs::symlink_metadata(path).map_err(|error| io_error(path, error))?;
...
fn enforce_file_size(
    ...
    if observed > limit {
        Err(MiniAppShareBundleError::Oversize(format!(
...
fn canonical_bytes<T: Serialize>(value: &T) -> Result<Vec<u8>, MiniAppShareBundleError> {
    canonical_json_bytes(value)
        .map_err(|error| MiniAppShareBundleError::Canonical(error.to_string()))
}
...
fn join_relative(root: &Path, relative: &str) -> Result<PathBuf, MiniAppShareBundleError> {
...
fn normalize_filesystem_path(path: &Path) -> Result<String, MiniAppShareBundleError> {
    ...
    for component in path.components() {
        match component {
            Component::Normal(value) => {
                let value = value.to_str().ok_or_else(|| MiniAppShareBundleError::InvalidPath {
                    path: path.display().to_string(),
...
fn validate_portable_relative_path(path: &str) -> Result<(), MiniAppShareBundleError> {
    if path.is_empty()
...
fn portable_collision_key(path: &str) -> Result<String, MiniAppShareBundleError> {
...
fn ensure_normal_destination(path: &Path) -> Result<(), MiniAppShareBundleError> {
...
fn ensure_regular_directory(path: &Path) -> Result<(), MiniAppShareBundleError> {
    let metadata = fs::symlink_metadata(path).map_err(|error| io_error(path, error))?;
...
fn create_relative_directories(
    ...
    let relative = parent.strip_prefix(root).map_err(|_| {
        MiniAppShareBundleError::InvalidInventory("file parent escaped its root".into())
    })?;
...
fn write_new_synced(path: &Path, bytes: &[u8]) -> Result<(), MiniAppShareBundleError> {
...
fn sync_tree_directories(root: &Path) -> Result<(), MiniAppShareBundleError> {
...
fn validate_removal_tree(root: &Path) -> Result<(), MiniAppShareBundleError> {
    ensure_regular_directory(root)?;
    for entry in fs::read_dir(root).map_err(|error| io_error(root, error))? {
        let entry = entry.map_err(|error| io_error(root, error))?;
        let path = entry.path();
...
fn sync_directory_if_supported(path: &Path) -> Result<(), MiniAppShareBundleError> {
    #[cfg(unix)]
    {
...
fn io_error(path: impl Into<PathBuf>, source: io::Error) -> MiniAppShareBundleError {
    MiniAppShareBundleError::Io {
        path: path.into(),
        source,
    }
}
...
impl StagingGuard {
    fn allocate(parent: &Path) -> Result<Self, MiniAppShareBundleError> {
