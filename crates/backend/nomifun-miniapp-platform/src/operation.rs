...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum MiniAppOperationState {
    Running,
    Succeeded,
    Failed,
    Canceled,
...
pub struct DurableMiniAppOperation {
    pub operation_id: OperationId,
    pub revision: u64,
    pub miniapp_id: MiniAppId,
    pub kind: MiniAppOperationKind,
    pub state: MiniAppOperationState,
    pub cancelable: bool,
...
impl DurableMiniAppOperation {
    pub fn running(
        ...
        let value = Self {
            ...
            miniapp_id,
            kind,
            state: MiniAppOperationState::Running,
            cancelable,
            progress_percent: if kind == MiniAppOperationKind::PermanentDelete {
                None
            } else {
                Some(0)
            },
    ...
    pub fn succeed(
        ...
        self.revision += 1;
        self.state = MiniAppOperationState::Succeeded;
        self.progress_percent = if self.kind == MiniAppOperationKind::PermanentDelete {
            None
        } else {
            Some(100)
        };
    ...
    pub fn fail(
        ...
        self.require_running(expected_revision)?;
        self.revision += 1;
        self.state = MiniAppOperationState::Failed;
        self.completed_at_ms = Some(completed_at_ms);
        self.last_error = Some(error);
        self.validate()
    }

    pub fn cancel(&mut self, expected_revision: u64, now_ms: i64) -> MiniAppPlatformResult<()> {
        self.require_running(expected_revision)?;
        if !self.cancelable {
            return Err(MiniAppPlatformError::OperationNotCancelable(
                self.operation_id.0.clone(),
            ));
        }
        self.revision += 1;
        self.state = MiniAppOperationState::Canceled;
        self.completed_at_ms = Some(now_ms);
        self.validate()
    }

    fn require_running(&self, expected_revision: u64) -> MiniAppPlatformResult<()> {
        if self.revision != expected_revision || self.state != MiniAppOperationState::Running {
    ...
    pub fn validate(&self) -> MiniAppPlatformResult<()> {
        ...
        if self.kind == MiniAppOperationKind::PermanentDelete && self.progress_percent.is_some() {
            return Err(MiniAppPlatformError::InvalidState(
                "permanent delete must not persist percentage progress".into(),
            ));
        }
        match self.state {
            MiniAppOperationState::Running => {
            ...
            MiniAppOperationState::Succeeded => {
                if self.completed_at_ms.is_none()
                    ...
                    || (self.kind != MiniAppOperationKind::PermanentDelete
            ...
            MiniAppOperationState::Canceled => {
    ...
    pub fn to_dto(&self) -> DurableOperationSummaryDto {
        DurableOperationSummaryDto {
            ...
            kind: match self.kind {
                MiniAppOperationKind::Build => DurableOperationKindDto::Build,
                MiniAppOperationKind::Import => DurableOperationKindDto::Import,
                MiniAppOperationKind::Export => DurableOperationKindDto::Export,
            ...
            state: match self.state {
                MiniAppOperationState::Running => DurableOperationStateDto::Running,
                MiniAppOperationState::Succeeded => DurableOperationStateDto::Succeeded,
                MiniAppOperationState::Failed => DurableOperationStateDto::Failed,
                MiniAppOperationState::Canceled => DurableOperationStateDto::Canceled,
            },
