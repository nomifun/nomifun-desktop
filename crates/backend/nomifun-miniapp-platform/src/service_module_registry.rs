...
use crate::{
    MiniAppPlatformError, MiniAppPlatformResult, MiniAppServiceLaunch,
    MiniAppServiceModuleResolver,
};
...
#[derive(Clone, Debug)]
pub struct MiniAppServiceModuleRegistry {
    root: Arc<PathBuf>,
    modules: Arc<RwLock<BTreeMap<ModuleKey, RegisteredModule>>>,
}

impl MiniAppServiceModuleRegistry {
    /// Creates a registry rooted at an existing, non-symlink directory.
    pub fn new(root: impl Into<PathBuf>) -> MiniAppPlatformResult<Self> {
        let root = root.into();
        let canonical_root = canonical_directory(&root, "MiniApp Service module registry root")?;
        Ok(Self {
    ...
    async fn resolve_exact(
        ...
        let registered = {
            ...
            {
                Some(Err(MiniAppPlatformError::InvalidState(
                    "MiniApp Service release is registered for another MiniApp".into(),
                )))
            } else {
                None
            }
        ...
        let registered = match registered {
            ...
            None => {
                return Err(MiniAppPlatformError::NotFound(format!(
                    "MiniApp Service module for {} release {}",
                    miniapp_id.as_ref(),
        ...
        if observed.path != registered.path {
            return Err(MiniAppPlatformError::InvalidState(
                "registered MiniApp Service module path is no longer canonical".into(),
            ));
        }
        if observed.digest != registered.digest {
            return Err(MiniAppPlatformError::Runtime(
                "registered MiniApp Service module digest no longer matches its path".into(),
            ));
        }
        if observed.digest != *expected_module_digest {
            return Err(MiniAppPlatformError::Runtime(
                "MiniApp Service module digest does not match the launch spec".into(),
            ));
        }
        Ok(observed.path)
    }
...
#[async_trait]
impl MiniAppServiceModuleResolver for MiniAppServiceModuleRegistry {
...
fn canonical_directory(path: &Path, label: &str) -> MiniAppPlatformResult<PathBuf> {
    if !path.is_absolute() {
    ...
    let metadata = fs::symlink_metadata(path).map_err(|error| {
        MiniAppPlatformError::Runtime(format!("{label} cannot be inspected: {error}"))
    })?;
    ...
    fs::canonicalize(path).map_err(|error| {
        MiniAppPlatformError::Runtime(format!("{label} cannot be canonicalized: {error}"))
    })
}
...
fn validate_module_path(
    root: &Path,
    path: PathBuf,
) -> MiniAppPlatformResult<ValidatedModule> {
    if !path.is_absolute() {
        return Err(MiniAppPlatformError::InvalidState(
    ...
    let metadata = fs::symlink_metadata(&path).map_err(|error| {
        if error.kind() == std::io::ErrorKind::NotFound {
            MiniAppPlatformError::NotFound(format!(
                "MiniApp Service module {}",
                path.display()
            ))
        } else {
            MiniAppPlatformError::Runtime(format!(
                "MiniApp Service module cannot be inspected: {error}"
            ))
        }
    })?;
    if metadata.file_type().is_symlink() {
        return Err(MiniAppPlatformError::InvalidState(
            "MiniApp Service module path must not be a symlink".into(),
        ));
    }
    if !metadata.is_file() {
        return Err(MiniAppPlatformError::InvalidState(
            "MiniApp Service module path must be a regular file".into(),
        ));
    }

    let canonical = fs::canonicalize(&path).map_err(|error| {
        MiniAppPlatformError::Runtime(format!(
            "MiniApp Service module cannot be canonicalized: {error}"
        ))
    })?;
    if !canonical.starts_with(root) {
        return Err(MiniAppPlatformError::InvalidState(
            "MiniApp Service module path escaped the registry root".into(),
        ));
    }
    ...
    {
        return Err(MiniAppPlatformError::InvalidState(format!(
            "MiniApp Service module path must end with {SERVICE_ENTRYPOINT}"
        )));
    }

    let bytes = fs::read(&canonical).map_err(|error| {
        MiniAppPlatformError::Runtime(format!(
            "MiniApp Service module cannot be read: {error}"
        ))
    })?;
    Ok(ValidatedModule {
        path: canonical,
...
fn validate_identity(
    ...
    if miniapp_id.as_ref().is_empty() || miniapp_id.as_ref().trim() != miniapp_id.as_ref() {
        return Err(MiniAppPlatformError::InvalidState(
            "MiniApp Service module registry MiniAppId must be non-empty and trimmed".into(),
        ));
    }
...
mod tests {
    ...
    async fn registers_and_resolves_exact_release_pair() {
        ...
        let registry = MiniAppServiceModuleRegistry::new(temp.path()).unwrap();
    ...
    async fn rejects_missing_wrong_miniapp_and_digest_mismatch() {
        ...
        let missing = registry
            ...
            .await
            .unwrap_err();
        assert!(matches!(missing, MiniAppPlatformError::NotFound(_)));
        ...
        let wrong_miniapp = registry
            ...
            .await
            .unwrap_err();
        assert!(matches!(wrong_miniapp, MiniAppPlatformError::InvalidState(_)));
        ...
        let changed = registry
            ...
            .await
            .unwrap_err();
        assert!(matches!(changed, MiniAppPlatformError::Runtime(_)));
    ...
    async fn rejects_non_file_and_outside_root_paths() {
        ...
        let directory_error = registry
            ...
            .await
            .unwrap_err();
        assert!(matches!(directory_error, MiniAppPlatformError::InvalidState(_)));
        ...
        let outside_error = registry
            ...
            .await
            .unwrap_err();
        assert!(matches!(outside_error, MiniAppPlatformError::InvalidState(_)));
    }
    ...
    async fn rejects_module_symlinks() {
        ...
        let error = registry
            ...
            .await
            .unwrap_err();
        assert!(matches!(error, MiniAppPlatformError::InvalidState(_)));
    }
