...
async fn repository_contract_keeps_release_slots_and_catalog_atomic() {
    let repository: Arc<dyn MiniAppRepository> = Arc::new(InMemoryMiniAppRepository::new());
    let first = create_root(&repository, "miniapp-a", MiniAppKind::UiOnly).await;
    let second = create_root(&repository, "miniapp-b", MiniAppKind::UiOnly).await;
    let second_expectation = MiniAppMutationExpectation::from_snapshot(&second);
    ...
    assert!(
        ...
        "a mutation to another MiniApp must not invalidate this owner CAS"
    );
    ...
    let first = lifecycle_repository(
        &repository,
        &first,
        MiniAppLifecycleCommand::Enable,
        T0 + 30,
    )
    .await;
    assert_eq!(
        first
            .catalog
            .as_ref()
            .expect("enabled MiniApp publishes Catalog")
            .catalog_digest,
        digest("catalog-a-v1")
    );
    ...
    let rolled_back = repository
        .commit_release(CommitMiniAppRelease {
            expected: MiniAppMutationExpectation::from_snapshot(&first),
            command: MiniAppReleaseCommand::Rollback(rollback),
            now_ms: T0 + 60,
        })
        .await
        .unwrap();
...
async fn repository_contract_enforces_lifecycle_without_intermediate_disabled_commit() {
    let repository: Arc<dyn MiniAppRepository> = Arc::new(InMemoryMiniAppRepository::new());
    let created = create_root(&repository, "miniapp-lifecycle", MiniAppKind::UiOnly).await;
    let enable_without_active = repository
        .commit_lifecycle(CommitMiniAppLifecycle {
            expected: MiniAppMutationExpectation::from_snapshot(&created),
            command: MiniAppLifecycleCommand::Enable,
            now_ms: T0 + 1,
        })
        .await;
    assert!(matches!(
    ...
    let mut running = DurableMiniAppOperation::running(
        OperationId::from("lifecycle-build"),
        published.root.product.miniapp_id.clone(),
        MiniAppOperationKind::Build,
        true,
        T0 + 21,
    )
    .unwrap();
    ...
    let blocked = repository
        .commit_lifecycle(CommitMiniAppLifecycle {
            expected: MiniAppMutationExpectation::from_snapshot(&published),
            command: MiniAppLifecycleCommand::Enable,
            now_ms: T0 + 22,
        })
        .await;
    assert!(matches!(blocked, Err(MiniAppPlatformError::OwnerBusy(_))));
    ...
    assert!(matches!(
        ...
        Err(MiniAppPlatformError::LifecycleConflict(_))
            | Err(MiniAppPlatformError::InvalidState(_))
    ));
    ...
    let restored = lifecycle_repository(
        &repository,
        &trashed,
        MiniAppLifecycleCommand::Restore,
        T0 + 50,
    )
    .await;
    assert_eq!(
...
async fn repository_contract_keeps_delete_intent_and_operation_outside_owner_root() {
    let repository: Arc<dyn MiniAppRepository> = Arc::new(InMemoryMiniAppRepository::new());
    let created = create_root(&repository, "miniapp-delete", MiniAppKind::UiOnly).await;
    let ready = complete_ready(
        &repository,
        &created,
    ...
    let deleting = repository
        .begin_delete(BeginMiniAppDelete {
            expected: MiniAppMutationExpectation::from_snapshot(&trashed),
    ...
    assert!(matches!(
        ...
        Err(MiniAppPlatformError::OperationNotCancelable(_))
    ));
    ...
    assert!(matches!(
        ...
        Err(MiniAppPlatformError::NotFound(_))
    ));
    assert_eq!(
        repository
            ...
            .unwrap()
            .state,
        MiniAppOperationState::Succeeded
    );
}
...
async fn repository_contract_rejects_cross_shape_release_and_legacy_state() {
    let repository: Arc<dyn MiniAppRepository> = Arc::new(InMemoryMiniAppRepository::new());
    let ui = create_root(&repository, "miniapp-ui", MiniAppKind::UiOnly).await;
    ...
    let result = repository
        .complete_ready_release(CompleteReadyReleaseCommit {
            expected: MiniAppMutationExpectation::from_snapshot(&ui),
        ...
        .await;
    assert!(matches!(result, Err(MiniAppPlatformError::InvalidState(_))));

    let service = create_root(&repository, "miniapp-service", MiniAppKind::Service).await;
    ...
    let result = repository
        .complete_ready_release(CompleteReadyReleaseCommit {
            expected: MiniAppMutationExpectation::from_snapshot(&service),
            release: stored_release(
...
async fn application_service_allows_strict_ui_change_for_service_miniapp() {
    ...
    let service = MiniAppApplicationService::new(MiniAppApplicationDependencies {
        repository: repository.clone(),
        runtime: runtime.clone(),
        managed_data: Arc::new(NoopMiniAppManagedData),
        mutations: Arc::new(MiniAppOwnerMutationCoordinator::new()),
    });
    let miniapp_id = MiniAppId::from("miniapp-service-auto");
    let created = service
        .create(CreateMiniApp {
            expected_library_revision: 0,
            miniapp_id: miniapp_id.clone(),
            project_id: MiniAppProjectId::from("project-service-auto"),
            display_name: "Service Auto".into(),
            description: None,
            kind: MiniAppKind::Service,
            storage: storage(&miniapp_id, MiniAppKind::Service),
            now_ms: T0,
        })
        .await
        .unwrap();
    ...
    let first = service
        ...
        .unwrap();
    let first = service
        .change_lifecycle(ChangeMiniAppLifecycle {
            expected: MiniAppMutationExpectation::from_snapshot(&first),
            command: MiniAppLifecycleCommand::Enable,
    ...
    let proof = MiniAppUiOnlyAutoPublishProof {
        ...
        current_ui_tree_digest: current.artifact.manifest.payload.ui.ui_tree_digest.clone(),
        target_ui_tree_digest: target.artifact.manifest.payload.ui.ui_tree_digest.clone(),
    ...
    let forged = service
        .publish(PublishMiniApp {
            expected: MiniAppMutationExpectation::from_snapshot(&second),
        ...
        .await;
    assert!(matches!(forged, Err(MiniAppPlatformError::InvalidState(_))));
    let authorized = service
        .set_auto_publish(SetMiniAppAutoPublish {
            expected: MiniAppMutationExpectation::from_snapshot(&second),
    ...
    let published = service
        .publish(PublishMiniApp {
            expected: MiniAppMutationExpectation::from_snapshot(&authorized),
            authorization: MiniAppPublishAuthorization::AutoUiOnly {
                authorization,
                proof: Box::new(proof),
            },
...
#[async_trait]
impl MiniAppRuntimePort for RecordingRuntime {
...
async fn complete_ready(
    ...
    complete_ready_with_expectation(
        repository,
        MiniAppMutationExpectation::from_snapshot(snapshot),
        artifact,
        operation_id,
        now_ms,
    )
...
async fn complete_ready_with_expectation(
    repository: &Arc<dyn MiniAppRepository>,
    expectation: MiniAppMutationExpectation,
    artifact: MiniAppReleaseArtifactV1,
    operation_id: &str,
    now_ms: i64,
) -> MiniAppRepositorySnapshot {
...
fn complete_import_operation(
    mut operation: DurableMiniAppOperation,
    artifact: &MiniAppReleaseArtifactV1,
    now_ms: i64,
) -> DurableMiniAppOperation {
    operation
...
async fn publish_repository_result(
    ...
    repository
        .commit_release(CommitMiniAppRelease {
            expected: MiniAppMutationExpectation::from_snapshot(snapshot),
            command: MiniAppReleaseCommand::Publish(MiniAppPublishRequest {
...
fn artifact(seed: &str, with_service: bool) -> MiniAppReleaseArtifactV1 {
    ...
    let manifest = MiniAppReleaseV1Manifest {
        ...
        ui: nomifun_agent_contracts::MiniAppUiReleaseDescriptor {
...
fn storage(miniapp_id: &MiniAppId, kind: MiniAppKind) -> MiniAppServiceStorageDescriptor {
    MiniAppServiceStorageDescriptor {
        ...
        files_dir: (kind == MiniAppKind::Service).then(|| MiniAppFilesDirDescriptor {
        ...
        private_database: (kind == MiniAppKind::Service).then(|| {
