...
Plugins cannot replace Agent Session pages or receive Session-control authority. Do not declare agent_view or an AgentSession UI contribution, and do not use window.nomi.agentSession. Ordinary plugin pages use only the existing Service and storage APIs.
