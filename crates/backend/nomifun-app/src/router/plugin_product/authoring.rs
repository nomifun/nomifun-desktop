...
Only when the user asks for a replacement Agent session page, include source_manifest.agent_view with nonempty name and description. This publishes a UI-only capability; it does not grant Session access automatically. The user must explicitly select this release from the Agent session page. Use window.nomi.agentSession.observe({after_seq:0,limit:100}), .turn({content:text},stableIntentKey), .cancel(), and .subscribe(async event => { ... }) through the existing SDK. subscribe returns an unsubscribe function. resync_required means discard transient fragments and reconcile durable paginated history with observe; it is not token replay. Never automatically resend a turn after a timeout/reconnect or start one on page load. In preview or when opened without a Session grant, explain how to select the page instead of fabricating conversation data. No separate service_source is required for this UI. Preserve trusted host recovery controls; never try to access the parent or host database.
...
For recoverable edits use window.nomi.storage.read(key), returning {value,revision}, and .compareAndSwap(key,expectedRevision,value), returning {applied,revision}. null expectedRevision means the key has never existed; deleted keys retain a revision. Omit value (or pass null) to delete conditionally. On conflict or timeout, keep local input and ask the user to reconcile; never blindly overwrite using the returned revision. Agent page drafts must be keyed by the Session ID obtained from authorized observe(), never persist transcripts as a second Session authority, save the immutable input/idempotency key before sending, and restore an uncertain request without automatically sending it. New SDK methods require a rebuilt/published release; preview storage is always temporary.
