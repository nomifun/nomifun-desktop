...
use nomifun_api_types::{
    ImportMiniAppArtifactRequest, ImportMiniAppBackupRequest, ImportMiniAppShareRequest,
    MiniAppShareContentDto, ShareMiniAppRequest,
};
use nomifun_miniapp_platform::{MiniAppShareBundleFilesystem, MiniAppWholeAppBackupFilesystem};
use std::{
    fs,
    io::Read,
...
pub(super) async fn inspect(
    ...
    if let Some(content) = request.content {
        ...
        let source = PathBuf::from(
            request
                .source_path
                .ok_or_else(|| invalid("Choose a MiniApp file"))?,
        );
        ...
        if metadata.file_type().is_symlink() {
            return Err(invalid("Choose the original MiniApp file"));
        }
        if metadata.is_file()
            && matches!(
                source
...
fn inspect_directory(path: &FsPath, draft: &mut Draft) -> Result<(), AppError> {
    ...
    if path.join("bundle.json").is_file() {
        let imported = filesystem
            ...
            .map_err(|_| invalid("This MiniApp package is damaged or incomplete"))?;
        ...
        draft.import = Some(ImportSource {
            ...
            digest: json["bundle_digest"]
                .as_str()
                .ok_or_else(|| invalid("Invalid MiniApp package"))?
                .into(),
            release_digest: artifact["artifact_digest"]
                .as_str()
                .ok_or_else(|| invalid("Invalid MiniApp release"))?
                .into(),
        ...
        let validated = MiniAppWholeAppBackupFilesystem::default()
            .import(path)
        ...
        let product: serde_json::Value =
            serde_json::from_slice(&fs::read(path.join("product.json")).map_err(internal)?)
                .map_err(internal)?;
        draft.name = product["display_name"].as_str().unwrap_or("MiniApp").into();
        ...
        let imported = filesystem
            ...
            .map_err(|_| invalid("Choose a MiniApp sharing package, backup, or HTML file"))?;
...
impl MiniAppProductService {
    pub(super) fn cleanup_import_stage(&self, draft: &Draft) -> Result<(), AppError> {
        ...
        if !target.starts_with(&base) || target == base {
            return Err(invalid("Invalid MiniApp import staging path"));
        }
    ...
    pub(super) async fn commit_import(
        ...
        match source.kind.as_str() {
            ...
            "backup" => self
                ...
                .await
                .map_err(application_error),
            _ => Err(invalid("Unsupported MiniApp import")),
        }
    }
}
...
pub(super) async fn export_file(
    ...
    let result = async {
        if request.backup {
            service
                ...
                .export_backup(
                    user.id.as_str(),
                    nomifun_api_types::ExportMiniAppBackupRequest {
                        miniapp_id: id.clone(),
                        expected_product_revision: workshop.miniapp.product_revision,
                        expected_lifecycle: nomifun_api_types::MiniAppLifecycleDto::Disabled,
                        expected_pointer_revision: workshop.miniapp.releases.pointer_revision,
            ...
            let release = workshop
                ...
                .active
                .as_ref()
                .ok_or_else(|| invalid("Save the MiniApp before sharing"))?;
            service
                .application
                .export_share(
                    user.id.as_str(),
    ...
    if stage.exists() {
        if let Err(error) = fs::remove_dir_all(&stage) {
            tracing::warn!(%error,"MiniApp export staging cleanup failed");
        }
    }
    result?;
...
fn extract_archive(source: &FsPath, destination: &FsPath) -> Result<(), AppError> {
    ...
    let mut archive = zip::ZipArchive::new(file)
        .map_err(|_| invalid("This file is not a MiniApp sharing package"))?;
    ...
    if file_size > limit {
        return Err(invalid("This MiniApp package exceeds the supported size"));
    }
    if archive.len() > if backup { 32768 } else { 4096 } {
        return Err(invalid("This MiniApp package has too many files"));
    }
    ...
    for i in 0..archive.len() {
        ...
        {
            return Err(invalid("Unsupported path in MiniApp package"));
        }
        let path = entry
            .enclosed_name()
            .ok_or_else(|| invalid("Invalid path in MiniApp package"))?
            .to_path_buf();
        if path
            .components()
        ...
        {
            return Err(invalid("Ambiguous path in MiniApp package"));
        }
        for part in path.components() {
            let text = part.as_os_str().to_string_lossy();
            if text.ends_with(['.', ' ']) {
                return Err(invalid("Unsupported file name in MiniApp package"));
            }
        }
        ...
        if total > limit {
            return Err(invalid("The unpacked MiniApp exceeds the supported size"));
        }
        fs::create_dir_all(
            target
                .parent()
        ...
        if copied != expected {
            return Err(invalid("The MiniApp package is incomplete"));
        }
    }
    Ok(())
}
