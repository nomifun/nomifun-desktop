...
//! Use `--model-smoke` for the canonical Session → single official Runtime path.
...
fn coding_pre_execution_rejection(message: &Value) -> bool {
    ...
    {
        // the Runtime's effect_gate returns these before invoking any owner.
        return true;
    }
...
#[derive(Clone, Copy)]
struct AgentToolExpectation {
    name: &'static str,
    validate_args: fn(&Value, &Path) -> bool,
    output_contains: Option<&'static str>,
}

const CODING_CREATE_TOOLS: &[AgentToolExpectation] = &[AgentToolExpectation {
    name: "Write",
    validate_args: validate_write_args,
    output_contains: None,
}];
const CODING_PATCH_TOOLS: &[AgentToolExpectation] = &[
...
const CODING_EXEC_TOOLS: &[AgentToolExpectation] = &[AgentToolExpectation {
    name: "exec_command",
    validate_args: validate_exec_command_args,
    output_contains: Some("git version"),
}];
const CODING_INSPECT_TOOLS: &[AgentToolExpectation] = &[
    AgentToolExpectation {
        name: "Grep",
...
const CODING_COMMIT_TOOLS: &[AgentToolExpectation] = &[
    AgentToolExpectation {
        name: "vcs.stage",
...
fn inspect_coding_evidence(
    messages: &[Value],
    marker: &str,
    expected_tools: &[AgentToolExpectation],
    workspace: &Path,
) -> AgentEvidence {
    ...
    if tool_messages.len() != expected_tools.len() {
        return AgentEvidence::Incomplete(if tool_messages.len() < expected_tools.len() {
    ...
    for (message, expected) in tool_messages.iter().zip(expected_tools) {
        if message.get("presentation_intent").and_then(Value::as_str) != Some("left") {
            return AgentEvidence::Incomplete("CODING_TOOL_PROJECTION_INVALID");
        }
        let Some(projection) = message.get("projection") else {
            return AgentEvidence::Incomplete("CODING_TOOL_PROJECTION_INVALID");
        };
        if projection.get("name").and_then(Value::as_str) != Some(expected.name) {
            return AgentEvidence::Incomplete("CODING_TOOL_SET_MISMATCH");
        }
        if projection.get("status").and_then(Value::as_str) != Some("completed") {
            return AgentEvidence::Incomplete("CODING_TOOL_NOT_COMPLETED");
        }
        if projection
            .get("call_id")
            .and_then(Value::as_str)
        ...
        {
            return AgentEvidence::Incomplete("CODING_TOOL_CALL_ID_INVALID");
        }
        let Some(args) = projection.get("args") else {
            return AgentEvidence::Incomplete("CODING_TOOL_ARGUMENTS_INVALID");
        };
        if projection.get("input") != Some(args) {
            return AgentEvidence::Incomplete("CODING_TOOL_INPUT_MISMATCH");
        }
        ...
        if expected.output_contains.is_some_and(|needle| {
            ...
            return AgentEvidence::Incomplete("CODING_TOOL_OUTPUT_INVALID");
        }
        let Some(projection_turn_id) = projection
            ...
            return AgentEvidence::Incomplete("CODING_TURN_ID_MISSING");
        };
        if turn_id
            .replace(projection_turn_id)
            .is_some_and(|expected_turn_id| expected_turn_id != projection_turn_id)
        {
            return AgentEvidence::Incomplete("CODING_TURN_ID_MISMATCH");
        }
    }
    ...
    let [assistant_text] = assistant_text.as_slice() else {
        return AgentEvidence::Incomplete(if assistant_text.is_empty() {
    ...
    if assistant_text.get("content").and_then(Value::as_str).map(str::trim) != Some(marker) {
        return AgentEvidence::Incomplete("CODING_ASSISTANT_TEXT_INVALID");
    }
...
async fn wait_for_coding_stage(
    ...
    loop {
        ...
        if ready && matches!(latest_evidence, AgentEvidence::Complete) {
            if coding_engine {
        ...
        if ready {
            ...
            if since.elapsed() >= Duration::from_secs(2) {
                if let AgentEvidence::Incomplete(code) = latest_evidence {
            ...
            let (code, status) = if ready {
                (
                    match latest_evidence {
                        AgentEvidence::Complete => "CODING_SESSION_NOT_READY",
                        AgentEvidence::Incomplete(code) => code,
                    },
...
async fn run_product_chain(
    ...
    let (minimal_session, _) = create_session(router, &minimal_id, &provider_id, model, json!([])).await?;
    ...
    let (preset_id, _source_binding) =
        create_agent_preset(router, &provider_id, model, CODING_CAPABILITIES).await?;
    ...
    let (session_id, binding) =
        create_session(router, &preset_id, &provider_id, model, coding_resource_selections()).await?;
...
fn coding_engine_evidence_messages(messages: &[Value]) -> Result<Vec<Value>, SmokeFailure> {
    ...
    for (index, message) in messages.iter().enumerate() {
        ...
        let projection = &message["projection"];
        // These are the Runtime's mandatory planning/completion controls, not
...
const ENGINE_CONTINUE_TOOLS: &[AgentToolExpectation] = &[AgentToolExpectation {
    name: "read_file", validate_args: validate_engine_read, output_contains: Some("beta"),
}];
#[derive(Clone, Copy)]
enum LiveSmokeMode { Product, Model, Compaction, BeforeTool }
...
async fn run_live_compaction_chain(router: &Router, key: &str, model: &str, root: &Path) -> Result<(), SmokeFailure> {
    ...
    let (session, _) = create_session(router, &preset, &provider, model, json!([])).await?;
    assert_session_runtime(router, &session).await?;
    let count = nomifun_agent_runtime::AgentContextBudget::default().max_history_messages + 32;
...
async fn run_live_provider_smoke(mode: LiveSmokeMode) -> Result<(), SmokeFailure> {
    ...
    let compaction_evidence = if matches!(mode, LiveSmokeMode::Compaction) {
...
async fn read_compaction_evidence(root: &Path) -> Result<(i64, i64), SmokeFailure> {
    ...
    let counts = nomifun_db::sqlx::query_as::<_, (i64, i64)>(
        "SELECT COALESCE(SUM(json_extract(e.event_json,'$.event')='compaction_started'),0), \
         ...
         WHERE json_extract(c.extra,'$.runtime_build_binding.family_id')='nomifun.nomi'"
...
async fn unified_runtime_compaction_reaches_live_stepfun_without_discarding_history() {
...
mod evidence_tests {
    ...
    fn coding_recovery_requires_proven_pre_execution_rejection_and_later_success() {
        ...
        let invalid_process = json!({"projection":{"name":"exec_command","status":"error",
            ...
            "output":"Capability Kernel rejected Agent Runtime Tool (INVALID_PAYLOAD): hidden owner detail"}});
    ...
    fn canonical_coding_arguments_and_host_reads_are_separate_evidence() {
        ...
        assert!(matches!(inspect_coding_evidence(&projected, ENGINE_CONTINUE_MARKER, ENGINE_CONTINUE_TOOLS, root.path()), AgentEvidence::Complete));
        let host_only = coding_engine_evidence_messages(&[host_read, text]).unwrap();
        assert!(matches!(inspect_coding_evidence(&host_only, ENGINE_CONTINUE_MARKER, ENGINE_CONTINUE_TOOLS, root.path()), AgentEvidence::Incomplete("CODING_TOOL_EVIDENCE_MISSING")));
    }
    ...
    fn strict_coding_evidence_rejects_shell_substitution_and_marker_suffixes() {
        ...
        assert!(matches!(
            ...
            AgentEvidence::Incomplete("CODING_TOOL_SET_MISMATCH")
        ));
        ...
        assert!(matches!(
            ...
            AgentEvidence::Incomplete("CODING_ASSISTANT_TEXT_INVALID")
        ));
    }
