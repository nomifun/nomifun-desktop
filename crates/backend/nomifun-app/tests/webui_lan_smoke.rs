...
//! (`DesktopServer::start_nomi_core` → `start_lan`) against a throwaway data dir, so the
...
async fn webui_lan_start_smoke() {
    ...
    let started =
        nomifun_app::DesktopServer::start_nomi_core(&cli, &merged_path, Some(spa_dir), None, None).await;
    let (server, _keep) = match started {
        Ok(pair) => pair,
        Err(e) => panic!("DesktopServer::start_nomi_core failed: {e:#}"),
    };
...
async fn webui_lan_spa_deep_link_serves_app_shell() {
    ...
    let (server, _keep) =
        nomifun_app::DesktopServer::start_nomi_core(&cli, &merged_path, Some(spa_dir), None, None)
            .await
            .expect("DesktopServer::start_nomi_core failed");
...
async fn webui_lan_embedded_asset_source_needs_no_filesystem_shell() {
    ...
    let (server, _keep) =
        nomifun_app::DesktopServer::start_nomi_core(&cli, &merged_path, None, None, Some(assets))
            .await
            .expect("DesktopServer::start_nomi_core failed");
    let status = server.start_lan().await;
    assert!(status.running, "start_lan failed: {:?}", status.error);
...
async fn webui_lan_without_app_shell_fails_instead_of_serving_partial_qr_flow() {
    ...
    let (server, _keep) =
        nomifun_app::DesktopServer::start_nomi_core(&cli, &merged_path, None, None, None)
            .await
            .expect("DesktopServer::start_nomi_core failed");
...
async fn webui_lan_dev_proxy_serves_live_frontend() {
    ...
    let (server, _keep) = nomifun_app::DesktopServer::start_nomi_core(
        &cli,
        &merged_path,
        None,
        Some(dev_url),
