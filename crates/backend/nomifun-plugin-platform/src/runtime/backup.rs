...
pub struct PluginRuntimeBackupFile {
    pub relative_path: String,
    pub bytes: Vec<u8>,
}

impl PluginRuntimeBackupFile {
...
#[derive(Clone, Debug, PartialEq)]
pub struct PluginRuntimeBackupStorage {
    pub kv: Vec<Value>,
    pub files: Vec<PluginRuntimeBackupFile>,
...
pub struct PluginRuntimeWholeAppBackupExport<'a> {
    ...
    pub releases: &'a BTreeMap<String, PluginRuntimeBackupRelease>,
    pub source: Option<&'a PluginRuntimeBackupSource>,
    pub storage: &'a PluginRuntimeBackupStorage,
}
...
pub struct PluginRuntimeWholeAppBackupImport {
    ...
    pub releases: BTreeMap<String, PluginRuntimeBackupRelease>,
    pub source: Option<PluginRuntimeBackupSource>,
    pub storage: PluginRuntimeBackupStorage,
}

#[derive(Debug, Error)]
pub enum PluginRuntimeWholeAppBackupError {
    #[error("Plugin Whole-App Backup input is invalid: {0}")]
    InvalidInput(String),
    #[error("Plugin Whole-App Backup path is invalid: {path} ({reason})")]
    InvalidPath { path: String, reason: String },
    #[error("Plugin Whole-App Backup destination already exists: {0}")]
    DestinationExists(PathBuf),
    #[error("Plugin Whole-App Backup inventory is invalid: {0}")]
    InvalidInventory(String),
    #[error("Plugin Whole-App Backup digest or content was modified: {0}")]
    Tampered(String),
    #[error("Plugin Whole-App Backup exceeds a size limit: {0}")]
    Oversize(String),
    #[error("Plugin Whole-App Backup filesystem operation failed for {path}: {source}")]
    Io {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("Plugin Whole-App Backup canonical serialization failed: {0}")]
    Canonical(String),
}

#[derive(Clone, Copy, Debug, Default)]
pub struct PluginRuntimeWholeAppBackupFilesystem;

impl PluginRuntimeWholeAppBackupFilesystem {
    pub fn new() -> Self {
        Self
    }
    ...
    pub fn export(
        ...
        match fs::symlink_metadata(destination) {
            Ok(_) => {
                return Err(PluginRuntimeWholeAppBackupError::DestinationExists(
    ...
    ) -> Result<PluginRuntimeWholeAppBackupImport, PluginRuntimeWholeAppBackupError> {
...
fn prepare_export(
    request: PluginRuntimeWholeAppBackupExport<'_>,
) -> Result<PreparedBackup, PluginRuntimeWholeAppBackupError> {
    ...
    if request.releases.len() > MAX_RELEASE_SLOTS {
        return Err(PluginRuntimeWholeAppBackupError::Oversize(format!(
...
fn prepare_release(
    release: PluginRuntimeBackupRelease,
) -> Result<PluginRuntimeBackupRelease, PluginRuntimeWholeAppBackupError> {
    release
        .artifact
        .validate()
    ...
    if release.manifest_bytes != expected_manifest {
        return Err(PluginRuntimeWholeAppBackupError::Tampered(
    ...
    for (actual, expected) in files.iter().zip(&release.artifact.files) {
        ...
        {
            return Err(PluginRuntimeWholeAppBackupError::Tampered(format!(
                "Release file {} does not match its Artifact inventory",
                expected.normalized_relative_path
            )));
        }
    }
    Ok(PluginRuntimeBackupRelease {
...
) -> Result<PluginRuntimeBackupSource, PluginRuntimeWholeAppBackupError> {
...
fn write_file_tree(
    ...
    for file in files {
        ...
        let parent = target.parent().ok_or_else(|| {
            PluginRuntimeWholeAppBackupError::InvalidInventory("backup file has no parent".into())
        })?;
...
fn import_backup(root: &Path) -> Result<PluginRuntimeWholeAppBackupImport, PluginRuntimeWholeAppBackupError> {
    ...
    metadata
        .validate()
        .map_err(|error| PluginRuntimeWholeAppBackupError::Tampered(error.to_string()))?;
    ...
    for slot in discover_release_slots(&observed)? {
        ...
        for file in &artifact.files {
            ...
            files.push(PluginRuntimeBackupFile::new(
        ...
        releases.insert(
            slot,
            prepare_release(PluginRuntimeBackupRelease {
                artifact,
                manifest_bytes,
                files,
            })?,
    ...
    if has_database != has_ledger {
        return Err(PluginRuntimeWholeAppBackupError::InvalidInventory(
            "Private Database bytes and migration ledger must be present or absent together"
                .into(),
        ));
    }
    ...
    let migration_ledger = has_ledger
        .then(|| {
            read_canonical::<PluginRuntimeMigrationLedger>(
                &storage_root.join(MIGRATION_LEDGER_FILE),
                MAX_JSON_BYTES,
            )
        })
    ...
    Ok(PluginRuntimeWholeAppBackupImport {
        metadata,
        product,
        project,
        config,
...
fn import_source(
...
) -> Result<Option<PluginRuntimeBackupSource>, PluginRuntimeWholeAppBackupError> {
    ...
    let source = prepare_source(PluginRuntimeBackupSource {
...
fn import_storage_files(
    storage_root: &Path,
    observed: &ObservedTree,
) -> Result<Vec<PluginRuntimeBackupFile>, PluginRuntimeWholeAppBackupError> {
    ...
    for path in observed.files.iter().filter(|path| path.starts_with(&prefix)) {
        let relative = path.strip_prefix(&prefix).ok_or_else(|| {
            PluginRuntimeWholeAppBackupError::InvalidInventory(
...
fn discover_release_slots(
    observed: &ObservedTree,
) -> Result<Vec<String>, PluginRuntimeWholeAppBackupError> {
...
fn file_records(files: &[PluginRuntimeBackupFile]) -> Vec<BackupFileRecord> {
    files
        .iter()
...
fn prepare_files(
    mut files: Vec<PluginRuntimeBackupFile>,
    label: &str,
    max_count: usize,
    max_total_bytes: u64,
    allow_empty: bool,
    ...
    for file in &files {
        ...
        if previous.is_some_and(|previous| previous == file.relative_path) {
            return Err(PluginRuntimeWholeAppBackupError::InvalidPath {
...
fn validate_file_records(
    ...
    for record in records {
        ...
        if previous.is_some_and(|previous| previous >= record.normalized_relative_path.as_str()) {
            return Err(PluginRuntimeWholeAppBackupError::InvalidInventory(format!(
...
fn validate_object(value: &Value, label: &str) -> Result<(), PluginRuntimeWholeAppBackupError> {
    if value.is_object() {
        Ok(())
    } else {
        Err(PluginRuntimeWholeAppBackupError::InvalidInput(format!(
...
fn credential_slot_keys(
    value: &Value,
) -> Result<BTreeSet<String>, PluginRuntimeWholeAppBackupError> {
    let slots = value.as_array().ok_or_else(|| {
        PluginRuntimeWholeAppBackupError::InvalidInput(
...
fn scan_tree(root: &Path) -> Result<ObservedTree, PluginRuntimeWholeAppBackupError> {
    ensure_regular_directory(root)?;
...
fn read_canonical<T: DeserializeOwned + Serialize>(
    path: &Path,
    limit: u64,
) -> Result<T, PluginRuntimeWholeAppBackupError> {
    let bytes = read_regular_bounded(path, limit)?;
    parse_canonical_bytes(&bytes, limit).map_err(|error| match error {
        PluginRuntimeWholeAppBackupError::InvalidInput(message) => {
            PluginRuntimeWholeAppBackupError::Tampered(format!("{}: {message}", path.display()))
        }
        other => other,
    })
}
...
fn read_regular_bounded(
    path: &Path,
    limit: u64,
) -> Result<Vec<u8>, PluginRuntimeWholeAppBackupError> {
...
fn enforce_size(
    ...
    if observed > limit {
        Err(PluginRuntimeWholeAppBackupError::Oversize(format!(
...
fn canonical_bytes<T: Serialize>(
    ...
    canonical_json_bytes(value)
        .map_err(|error| PluginRuntimeWholeAppBackupError::Canonical(error.to_string()))
}

fn validate_nonempty(value: &str, field: &str) -> Result<(), PluginRuntimeWholeAppBackupError> {
...
fn validate_digest(value: &str, field: &str) -> Result<(), PluginRuntimeWholeAppBackupError> {
    if value.len() == 64
        && value
            .bytes()
...
fn validate_machine_key(value: &str, field: &str) -> Result<(), PluginRuntimeWholeAppBackupError> {
...
fn validate_release_slot(slot: &str) -> Result<(), PluginRuntimeWholeAppBackupError> {
...
fn join_relative(root: &Path, relative: &str) -> Result<PathBuf, PluginRuntimeWholeAppBackupError> {
...
fn normalize_filesystem_path(path: &Path) -> Result<String, PluginRuntimeWholeAppBackupError> {
        ...
        match component {
            Component::Normal(value) => {
                let value = value.to_str().ok_or_else(|| {
                    PluginRuntimeWholeAppBackupError::InvalidPath {
                        path: path.display().to_string(),
...
fn portable_collision_key(
    path: &str,
) -> Result<String, PluginRuntimeWholeAppBackupError> {
...
fn ensure_normal_destination(path: &Path) -> Result<(), PluginRuntimeWholeAppBackupError> {
...
fn ensure_regular_directory(path: &Path) -> Result<(), PluginRuntimeWholeAppBackupError> {
    let metadata = fs::symlink_metadata(path).map_err(|error| io_error(path, error))?;
...
fn create_relative_directories(
    ...
    let relative = parent.strip_prefix(root).map_err(|_| {
        PluginRuntimeWholeAppBackupError::InvalidInventory("file parent escaped its root".into())
    })?;
...
fn sync_tree_directories(root: &Path) -> Result<(), PluginRuntimeWholeAppBackupError> {
...
fn validate_removal_tree(root: &Path) -> Result<(), PluginRuntimeWholeAppBackupError> {
    ensure_regular_directory(root)?;
    for entry in fs::read_dir(root).map_err(|error| io_error(root, error))? {
        let entry = entry.map_err(|error| io_error(root, error))?;
        let path = entry.path();
...
fn sync_directory_if_supported(path: &Path) -> Result<(), PluginRuntimeWholeAppBackupError> {
    #[cfg(unix)]
    {
...
fn io_error(path: impl Into<PathBuf>, source: io::Error) -> PluginRuntimeWholeAppBackupError {
    PluginRuntimeWholeAppBackupError::Io {
        path: path.into(),
        source,
    }
}
...
impl StagingGuard {
    fn allocate(parent: &Path) -> Result<Self, PluginRuntimeWholeAppBackupError> {
...
mod tests {
        ...
        fn new() -> Self {
            Self {
                ...
                storage: PluginRuntimeBackupStorage {
                    ...
                    files: vec![PluginRuntimeBackupFile::new(
                        "state/current.bin",
                        b"state".to_vec(),
                    )],
    ...
    fn whole_app_backup_roundtrips_and_never_overwrites() {
        ...
        assert!(matches!(
            PluginRuntimeWholeAppBackupFilesystem::new().export(
                fixture.request(),
                &destination
            ),
            Err(PluginRuntimeWholeAppBackupError::DestinationExists(_))
        ));
    }
    ...
    fn whole_app_backup_rejects_tampered_bytes() {
        ...
        assert!(matches!(
            PluginRuntimeWholeAppBackupFilesystem::new().import(&destination),
            Err(PluginRuntimeWholeAppBackupError::Tampered(_))
        ));
    }
    ...
    fn whole_app_backup_rejects_extra_files() {
        ...
        assert!(matches!(
            PluginRuntimeWholeAppBackupFilesystem::new().import(&destination),
            Err(PluginRuntimeWholeAppBackupError::InvalidInventory(_))
        ));
    }
