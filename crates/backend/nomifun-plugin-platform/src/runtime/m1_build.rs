...
pub struct PluginRuntimeStaticBundleFile {
    pub normalized_relative_path: String,
    pub bytes: Vec<u8>,
}

impl PluginRuntimeStaticBundleFile {
    pub fn new(
...
pub struct PluginRuntimeStaticBundleInput {
    pub artifact_id: ArtifactId,
    pub display: LocalizedMetadata,
    pub ui_index_html: Vec<u8>,
    pub ui_assets: Vec<PluginRuntimeStaticBundleFile>,
    pub service: Option<PluginRuntimeStaticServiceInput>,
...
#[derive(Debug, Error)]
pub enum PluginRuntimeStaticBundleBuildError {
    ...
    #[error("UI-only Plugin cannot include Service source")]
    UiOnlyServiceSource,
    #[error("Plugin Service source must be valid UTF-8")]
    InvalidServiceSourceEncoding,
    #[error("Plugin Service source contains a NUL byte")]
...
pub struct PluginRuntimeStaticBundleBuilder;

impl PluginRuntimeStaticBundleBuilder {
...
pub fn build_miniapp_static_bundle(
    ...
    if !ui_index_html.is_empty() {
        let ui_index_html = materialize_surface_entrypoint(&ui_index_html)?;
        push_file(&mut files, &mut collision_keys, "ui/index.html".to_owned(), ui_index_html)?;
    }
...
pub fn materialize_surface_entrypoint(
    source: &[u8],
) -> Result<Vec<u8>, PluginRuntimeStaticBundleBuildError> {
    if source.is_empty() {
        return Ok(Vec::new());
    }
...
pub fn validate_service_source(
    ...
    std::str::from_utf8(source)
        .map_err(|_| PluginRuntimeStaticBundleBuildError::InvalidServiceSourceEncoding)?;
...
pub fn materialize_service_release(
    input: PluginRuntimeStaticServiceInput,
    ...
    let PluginRuntimeStaticServiceInput {
        main_mjs,
        lifecycle,
        uses_files,
...
pub fn validate_no_custom_scripts(
    ...
    let value: Value = serde_json::from_slice(package_json)
        .map_err(|error| PluginRuntimeStaticBundleBuildError::InvalidPackageJson(
            error.to_string(),
        ))?;
    let object = value.as_object().ok_or_else(|| {
        PluginRuntimeStaticBundleBuildError::InvalidPackageJson(
    ...
    match scripts {
        ...
        Value::Object(_) => Err(
            PluginRuntimeStaticBundleBuildError::CustomScriptsForbidden,
        ),
        _ => Err(PluginRuntimeStaticBundleBuildError::InvalidPackageJson(
...
fn validate_ui_asset_path(
    ...
    if path == "ui/index.html" {
        return Err(PluginRuntimeStaticBundleBuildError::InvalidPath {
            path: path.to_owned(),
...
fn push_file(
    ...
    if !collision_keys.insert(collision_key) {
        return Err(PluginRuntimeStaticBundleBuildError::PathCollision { path });
    }
    if bytes.is_empty() {
        return Err(PluginRuntimeStaticBundleBuildError::EmptyFile { path });
    }
    let size_bytes = u64::try_from(bytes.len()).map_err(|_| {
        PluginRuntimeStaticBundleBuildError::FileSizeOverflow {
            path: path.clone(),
        }
    })?;
