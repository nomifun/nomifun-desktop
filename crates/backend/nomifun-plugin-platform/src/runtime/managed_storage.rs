...
use crate::runtime::{
    PluginRuntimeBackupFile, PluginRuntimeBackupStorage, PluginRuntimeCallCancellation,
    PluginRuntimeDatabaseExecuteResult, PluginRuntimeDatabaseQueryResult, PluginRuntimeDatabaseStatement,
    PluginRuntimeFilesPort, PluginRuntimeHostKvPort, PluginRuntimeMigrationLedger, PluginRuntimePlatformError,
    PluginRuntimePlatformResult, PluginRuntimePrivateDatabasePort,
    PluginRuntimeServiceStoragePort, PluginRuntimeServiceStorageRequest, PluginRuntimeServiceStorageResolution,
    PluginRuntimeServiceTestKvSnapshotEntry, PluginRuntimeServiceTestStorageResolution,
    rebind_migration_ledger_for_target, service_test_kv_digest, validate_service_test_id,
};
use crate::runtime::PluginRuntimeMigrationLedgerEntry;
...
impl std::fmt::Debug for SqlitePluginRuntimeManagedStorage {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SqlitePluginRuntimeManagedStorage")
            .field("root", &self.root)
            .finish_non_exhaustive()
    }
}

impl SqlitePluginRuntimeManagedStorage {
    pub fn new(
                    ...
                    expected_revision: expected_revision
                        .map(|value| {
                            i64::try_from(value).map_err(|_| {
                                PluginRuntimePlatformError::KvRevisionOverflow
                            })
                        })
                        .transpose()?,
        ...
        let response = match operation {
                ...
                let value = current
                    ...
                    .map(|row| {
                        serde_json::from_str(&row.value_json).map_err(|error| {
                            PluginRuntimePlatformError::Database(format!(
                                "Plugin KV value is invalid JSON: {error}"
                            ))
                        })
                    })
                    .transpose()?;
            ...
            KvOperation::Set { value } => {
                let value_json = serde_json::to_string(&value).map_err(|error| {
                    PluginRuntimePlatformError::Database(format!(
                        "Plugin KV value cannot be serialized: {error}"
                    ))
                })?;
    ...
    where
        T: Send + 'static,
        F: FnOnce(Connection, Arc<SqliteAuthorizationState>) -> Result<T, PluginRuntimePlatformError>
            + Send
            + 'static,
    {
...
#[async_trait]
impl PluginRuntimeServiceStoragePort for SqlitePluginRuntimeManagedStorage {
    async fn resolve_service_storage(
        ...
        let (private_database, migration_ledger, database_path) =
            if uses_private_database {
                ...
                let path = fs::canonicalize(&path).map_err(|error| {
                    PluginRuntimePlatformError::Runtime(format!(
                        "cannot canonicalize Plugin private database: {error}"
                    ))
                })?;
        ...
        Ok(PluginRuntimeServiceStorageResolution {
            descriptor,
            migration_ledger,
        })
    }
        ...
        let database_path = registration
            ...
            .ok_or_else(|| {
                PluginRuntimePlatformError::InvalidState(
                    "Plugin migrations require a Private Database".into(),
                )
            })?;
            ...
            with_authorizer_mode(&internal_mode, true, true, true, || {
                ...
                for migration in &migrations_owned {
                    ...
                    let entry = next
                        ...
                        .ok_or_else(|| {
                            PluginRuntimePlatformError::Database(
        ...
        let (private_database, migration_ledger) =
            if let Some(database) = resolution.descriptor.private_database.as_ref() {
                ...
                let bytes = fs::read(&temporary_path).map_err(|error| {
                    PluginRuntimePlatformError::Runtime(format!(
                        "cannot read Plugin backup database snapshot: {error}"
                    ))
                })?;
    ...
    async fn import_backup_storage(
        ...
        if uses_private_database {
            ...
            let parent = target_path.parent().ok_or_else(|| {
                PluginRuntimePlatformError::InvalidState(
                    "Plugin backup database target has no parent".into(),
                )
            })?;
        ...
        self.with_database(registration, move |connection, _| {
                ...
                .map_err(|error| canceled_database_error(error, &cancellation))?
            {
                if result.len() >= MAX_DATABASE_RESULT_ROWS {
                    return Err(PluginRuntimePlatformError::Database(
            ...
            ensure_not_canceled(&cancellation)?;
            Ok(PluginRuntimeDatabaseQueryResult { rows: result })
        })
        .await
    }
        ...
        self.with_database(registration, move |connection, authorization| {
            ...
            with_authorizer_mode(&authorization, false, false, true, || {
                ...
                let affected = connection
                    ...
                    .map_err(|error| canceled_database_error(error, &cancellation))?;
                ensure_not_canceled(&cancellation)?;
                Ok(PluginRuntimeDatabaseExecuteResult {
            ...
            let result = with_authorizer_mode(&authorization, false, true, true, || {
                ...
                for statement in &statements {
                    ...
                    let affected = transaction
                        .execute(&statement.sql, params_from_iter(values.iter()))
                        .map_err(|error| canceled_database_error(error, &cancellation))?;
                    output.push(PluginRuntimeDatabaseExecuteResult {
                        affected_rows: affected as u64,
                    });
                }
                ensure_not_canceled(&cancellation)?;
...
fn service_test_kv_namespace(test_id: &str) -> PluginRuntimePlatformResult<String> {
    validate_service_test_id(test_id)?;
...
async fn copy_service_test_kv(
    ...
    for row in rows {
        ...
        {
            return Err(PluginRuntimePlatformError::Database(
                "Plugin KV row violates its tombstone contract".into(),
            ));
        }
        ...
        digest_entries.push(PluginRuntimeServiceTestKvSnapshotEntry {
            key: row.key.clone(),
            value: StrictJsonValue(value),
            revision: u64::try_from(row.revision)
                .map_err(|_| PluginRuntimePlatformError::KvRevisionOverflow)?,
...
fn ensure_directory(path: &Path) -> PluginRuntimePlatformResult<()> {
    ...
    fs::create_dir_all(path).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot create Plugin managed storage directory {}: {error}",
            path.display()
        ))
    })?;
    let metadata = fs::symlink_metadata(path).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot inspect Plugin managed storage directory {}: {error}",
            path.display()
        ))
    })?;
    if is_reparse_or_symlink(&metadata) || !metadata.is_dir() {
...
fn ensure_database_path(path: &Path) -> PluginRuntimePlatformResult<()> {
    if let Ok(metadata) = fs::symlink_metadata(path) {
        if is_reparse_or_symlink(&metadata) || !metadata.is_file() {
            return Err(PluginRuntimePlatformError::InvalidState(format!(
                "Plugin private database path is not a regular file: {}",
                path.display()
            )));
        }
    }
...
fn write_new_regular_file(path: &Path, bytes: &[u8]) -> PluginRuntimePlatformResult<()> {
    let mut file = std::fs::OpenOptions::new()
        ...
        .map_err(|error| {
            PluginRuntimePlatformError::Runtime(format!(
                "cannot create Plugin storage staging file {}: {error}",
                path.display()
            ))
        })?;
    use std::io::Write;
    file.write_all(bytes).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot write Plugin storage staging file {}: {error}",
            path.display()
        ))
    })?;
    file.sync_all().map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot sync Plugin storage staging file {}: {error}",
            path.display()
        ))
    })
}
...
fn read_backup_files(
    root: &Path,
    managed_root: &Path,
) -> PluginRuntimePlatformResult<Vec<PluginRuntimeBackupFile>> {
    let canonical_root = fs::canonicalize(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot canonicalize Plugin backup Files root {}: {error}",
            root.display()
        ))
    })?;
...
fn collect_backup_files(
    root: &Path,
    current: &Path,
    output: &mut Vec<PluginRuntimeBackupFile>,
) -> PluginRuntimePlatformResult<()> {
    for entry in fs::read_dir(current).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot read Plugin backup Files directory {}: {error}",
            current.display()
        ))
    })? {
        let entry = entry.map_err(|error| {
            PluginRuntimePlatformError::Runtime(format!(
                "cannot inspect Plugin backup Files entry: {error}"
            ))
        })?;
        let path = entry.path();
        let metadata = fs::symlink_metadata(&path).map_err(|error| {
        ...
        if is_reparse_or_symlink(&metadata) {
            return Err(PluginRuntimePlatformError::InvalidState(
                "Plugin backup Files cannot contain symlinks or reparse points".into(),
            ));
        }
        let relative = path.strip_prefix(root).map_err(|_| {
            PluginRuntimePlatformError::InvalidState(
                "Plugin backup Files entry escaped its root".into(),
            )
        })?;
        let relative = relative
            ...
            .map(|component| match component {
                std::path::Component::Normal(value) => value.to_str().ok_or_else(|| {
                    PluginRuntimePlatformError::InvalidState(
                        "Plugin backup Files path must be UTF-8".into(),
                    )
                }),
                _ => Err(PluginRuntimePlatformError::InvalidState(
                    "Plugin backup Files path contains a non-normal component".into(),
                )),
            })
        ...
        if metadata.is_dir() {
            ...
            output.push(PluginRuntimeBackupFile::new(
                relative,
                fs::read(&path).map_err(|error| {
                    PluginRuntimePlatformError::Runtime(format!(
                        "cannot read Plugin backup File {}: {error}",
                        path.display()
                    ))
                })?,
            ));
            ...
            return Err(PluginRuntimePlatformError::InvalidState(
                "Plugin backup Files cannot contain special entries".into(),
            ));
        }
    }
    Ok(())
...
fn write_backup_files(
    root: &Path,
    files: &[PluginRuntimeBackupFile],
    managed_root: &Path,
) -> PluginRuntimePlatformResult<()> {
    let canonical_root = fs::canonicalize(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot canonicalize Plugin restore Files root {}: {error}",
            root.display()
        ))
    })?;
    ensure_within(managed_root, &canonical_root)?;
    ...
    for file in files {
        ...
        {
            return Err(PluginRuntimePlatformError::InvalidState(
                "Plugin backup Files contain an invalid or duplicate path".into(),
            ));
        }
        let target = file
            .relative_path
        ...
        if !target.starts_with(&canonical_root) {
            return Err(PluginRuntimePlatformError::InvalidState(
                "Plugin backup File escaped its target root".into(),
            ));
        }
        let parent = target.parent().ok_or_else(|| {
            PluginRuntimePlatformError::InvalidState(
                "Plugin backup File has no parent directory".into(),
            )
        })?;
...
fn restore_backup_files_atomically(
    ...
    let parent = canonical_root.parent().ok_or_else(|| {
        PluginRuntimePlatformError::InvalidState(
            "Plugin restore Files root has no parent directory".into(),
        )
    })?;
    ensure_directory(parent)?;
    ...
    fs::create_dir(&staging).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot create Plugin restore Files staging directory {}: {error}",
            staging.display()
        ))
    })?;
    ...
    let result = (|| {
        ...
        fs::rename(&canonical_root, &quarantine).map_err(|error| {
            PluginRuntimePlatformError::Runtime(format!(
                "cannot quarantine existing Plugin Files directory: {error}"
            ))
        })?;
        if let Err(error) = fs::rename(&staging, &canonical_root) {
            ...
            if rollback.is_err() {
                return Err(PluginRuntimePlatformError::Runtime(format!(
                    "cannot install Plugin Files restore ({error}); rollback also failed"
                )));
            }
            return Err(PluginRuntimePlatformError::Runtime(format!(
                "cannot install Plugin Files restore: {error}"
            )));
        }
        ...
        if fs::symlink_metadata(&staging).is_ok() {
            let _ = validate_removal_tree(&staging).and_then(|()| {
                fs::remove_dir_all(&staging).map_err(|error| {
                    PluginRuntimePlatformError::Runtime(format!(
                        "cannot clean Plugin Files restore staging: {error}"
                    ))
                })
            });
        }
...
fn remove_private_database_files(root: &Path, path: &Path) -> PluginRuntimePlatformResult<()> {
    let parent = path.parent().ok_or_else(|| {
        PluginRuntimePlatformError::InvalidState(
            "Plugin private database cleanup target has no parent".into(),
        )
    })?;
    ...
    let file_name = path.file_name().ok_or_else(|| {
        PluginRuntimePlatformError::InvalidState(
            "Plugin private database cleanup target has no file name".into(),
        )
    })?;
    ...
    for suffix in ["", "-wal", "-shm", "-journal"] {
        ...
        match fs::symlink_metadata(&target) {
            Ok(metadata) if is_reparse_or_symlink(&metadata) => {
                return Err(PluginRuntimePlatformError::InvalidState(format!(
                    "Plugin private database cleanup encountered a symlink or reparse point: {}",
                    target.display()
                )));
            }
            Ok(metadata) if metadata.is_file() => {
                fs::remove_file(&target).map_err(|error| {
                    PluginRuntimePlatformError::Runtime(format!(
                        "cannot remove Plugin private database file {}: {error}",
                        target.display()
                    ))
                })?
            }
            Ok(_) => {
                return Err(PluginRuntimePlatformError::InvalidState(format!(
                    "Plugin private database cleanup target is not a regular file: {}",
                    target.display()
                )));
            }
            ...
            Err(error) => {
                return Err(PluginRuntimePlatformError::Runtime(format!(
                    "cannot inspect Plugin private database file {}: {error}",
                    target.display()
                )));
            }
        }
...
fn replace_private_database_atomically(
    ...
    for suffix in ["", "-wal", "-shm", "-journal"] {
        ...
        if is_reparse_or_symlink(&metadata) || !metadata.is_file() {
            return Err(PluginRuntimePlatformError::InvalidState(format!(
                "Plugin private database replacement target is not a regular file: {}",
                original.display()
            )));
        }
        ...
        if let Err(error) = fs::rename(&original, &quarantine) {
            ...
            return Err(PluginRuntimePlatformError::Runtime(format!(
                "cannot quarantine Plugin private database file {}: {error}",
                original.display()
            )));
        }
    ...
    if let Err(error) = fs::rename(temporary_path, &target) {
        ...
        if rollback_failed {
            return Err(PluginRuntimePlatformError::Runtime(format!(
                "cannot install Plugin private database restore ({error}); rollback also failed"
            )));
        }
        return Err(PluginRuntimePlatformError::Runtime(format!(
            "cannot install Plugin private database restore: {error}"
        )));
    }
...
fn sync_storage_tree(root: &Path) -> PluginRuntimePlatformResult<()> {
...
fn collect_storage_directories(
    ...
    let metadata = fs::symlink_metadata(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot inspect Plugin storage staging directory {}: {error}",
            root.display()
        ))
    })?;
    if is_reparse_or_symlink(&metadata) || !metadata.is_dir() {
        return Err(PluginRuntimePlatformError::InvalidState(format!(
            "Plugin storage staging path is not a regular directory: {}",
            root.display()
        )));
    }
    ...
    for entry in fs::read_dir(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot read Plugin storage staging directory {}: {error}",
            root.display()
        ))
    })? {
        let entry = entry.map_err(|error| {
            PluginRuntimePlatformError::Runtime(format!(
                "cannot inspect Plugin storage staging entry: {error}"
            ))
        })?;
        let path = entry.path();
        let metadata = fs::symlink_metadata(&path).map_err(|error| {
        ...
        if is_reparse_or_symlink(&metadata) {
            return Err(PluginRuntimePlatformError::InvalidState(
                "Plugin storage staging cannot contain symlinks or reparse points".into(),
            ));
        }
        if metadata.is_dir() {
            ...
            return Err(PluginRuntimePlatformError::InvalidState(
                "Plugin storage staging cannot contain special files".into(),
            ));
        }
    }
    Ok(())
...
fn sync_directory_if_supported(path: &Path) -> PluginRuntimePlatformResult<()> {
    #[cfg(unix)]
    {
        match fs::File::open(path).and_then(|file| file.sync_all()) {
            ...
            Err(error) => Err(PluginRuntimePlatformError::Runtime(format!(
                "cannot sync Plugin storage directory {}: {error}",
                path.display()
            ))),
        }
    }
...
fn remove_managed_directory(root: &Path, path: &Path) -> PluginRuntimePlatformResult<()> {
    let parent = path.parent().ok_or_else(|| {
        PluginRuntimePlatformError::InvalidState(
            "Plugin filesDir cleanup target has no parent".into(),
        )
    })?;
    let Some(canonical_parent) = canonical_existing_directory_chain(root, parent)? else {
        return Ok(());
    ...
    let metadata = match fs::symlink_metadata(path) {
        ...
        Err(error) => {
            return Err(PluginRuntimePlatformError::Runtime(format!(
                "cannot inspect Plugin filesDir during cleanup: {error}"
            )));
        }
    };
    if is_reparse_or_symlink(&metadata) || !metadata.is_dir() {
        return Err(PluginRuntimePlatformError::InvalidState(
            "Plugin filesDir is not a regular managed directory".into(),
        ));
    }
    let canonical = fs::canonicalize(path).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot canonicalize Plugin filesDir during cleanup: {error}"
        ))
    })?;
    if canonical.parent() != Some(canonical_parent.as_path()) {
        return Err(PluginRuntimePlatformError::InvalidState(
            "Plugin filesDir escaped its owner boundary".into(),
        ));
    }
    validate_removal_tree(&canonical)?;
    fs::remove_dir_all(&canonical).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot remove Plugin filesDir {}: {error}",
            canonical.display()
        ))
    })
}
...
fn canonical_existing_directory_chain(
    root: &Path,
    target: &Path,
) -> PluginRuntimePlatformResult<Option<PathBuf>> {
    let relative = target.strip_prefix(root).map_err(|_| {
        PluginRuntimePlatformError::InvalidState(
            "Plugin managed storage path escaped its root".into(),
        )
    })?;
    let root_metadata = fs::symlink_metadata(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot inspect Plugin managed storage root: {error}"
        ))
    })?;
    if is_reparse_or_symlink(&root_metadata) || !root_metadata.is_dir() {
        return Err(PluginRuntimePlatformError::InvalidState(
            "Plugin managed storage root is not a regular directory".into(),
        ));
    }
    let canonical_root = fs::canonicalize(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
    ...
    for component in relative.components() {
        let std::path::Component::Normal(value) = component else {
            return Err(PluginRuntimePlatformError::InvalidState(
                "Plugin managed storage path contains a non-normal component".into(),
            ));
        };
        current.push(value);
        ...
        if is_reparse_or_symlink(&metadata) || !metadata.is_dir() {
            return Err(PluginRuntimePlatformError::InvalidState(format!(
                "Plugin managed storage directory is not regular: {}",
                current.display()
            )));
        }
    }
    let canonical_target = fs::canonicalize(target).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot canonicalize Plugin managed storage directory {}: {error}",
            target.display()
        ))
    })?;
...
fn validate_removal_tree(root: &Path) -> PluginRuntimePlatformResult<()> {
    let metadata = fs::symlink_metadata(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot inspect Plugin removal target {}: {error}",
            root.display()
        ))
    })?;
    if is_reparse_or_symlink(&metadata) || !metadata.is_dir() {
        return Err(PluginRuntimePlatformError::InvalidState(format!(
            "Plugin removal target is not a regular directory: {}",
            root.display()
        )));
    }
    for entry in fs::read_dir(root).map_err(|error| {
        PluginRuntimePlatformError::Runtime(format!(
            "cannot read Plugin removal target {}: {error}",
            root.display()
        ))
    })? {
        let entry = entry.map_err(|error| {
            PluginRuntimePlatformError::Runtime(format!(
                "cannot inspect Plugin removal entry under {}: {error}",
                root.display()
            ))
        })?;
        let path = entry.path();
        let metadata = fs::symlink_metadata(&path).map_err(|error| {
            PluginRuntimePlatformError::Runtime(format!(
                "cannot inspect Plugin removal entry {}: {error}",
                path.display()
            ))
        })?;
        if is_reparse_or_symlink(&metadata) {
            return Err(PluginRuntimePlatformError::InvalidState(format!(
                "Plugin removal target contains a symlink or reparse point: {}",
                path.display()
            )));
        }
        if metadata.is_dir() {
            ...
            return Err(PluginRuntimePlatformError::InvalidState(format!(
                "Plugin removal target contains a special file: {}",
                path.display()
            )));
        }
    }
...
fn ensure_within(root: &Path, child: &Path) -> PluginRuntimePlatformResult<()> {
    if child.starts_with(root) {
        Ok(())
    } else {
        Err(PluginRuntimePlatformError::InvalidState(
            "Plugin managed storage path escaped its owner root".into(),
        ))
    }
}

fn validate_path_component(value: &str, field: &str) -> PluginRuntimePlatformResult<()> {
    if value.is_empty()
        || value == "."
        || value == ".."
        || value.contains('/')
...
fn validate_visible_key(value: &str, maximum: usize) -> PluginRuntimePlatformResult<()> {
    if value.is_empty()
        || value.len() > maximum
        || !value.bytes().all(|byte| byte.is_ascii_graphic())
    {
...
fn positive_revision(value: i64) -> PluginRuntimePlatformResult<u64> {
    u64::try_from(value)
        .ok()
        .filter(|value| *value > 0)
        .ok_or_else(|| PluginRuntimePlatformError::Database("KV revision is invalid".into()))
}
...
fn open_private_database(
    path: &Path,
) -> PluginRuntimePlatformResult<(Connection, Arc<SqliteAuthorizationState>)> {
    ensure_database_path(path)?;
...
fn with_authorizer_mode<T>(
    ...
    allow_dml: bool,
    operation: impl FnOnce() -> PluginRuntimePlatformResult<T>,
) -> PluginRuntimePlatformResult<T> {
    authorization
        .allow_schema
    ...
    match action {
        MiniAppAdditiveMigrationAction::CreateTable {
            ...
            let mut definitions = columns
                .iter()
                .map(|column| -> PluginRuntimePlatformResult<String> {
...
fn validate_sql_type(value: &str) -> PluginRuntimePlatformResult<()> {
...
fn validate_default_literal(value: &str) -> PluginRuntimePlatformResult<()> {
    let value = value.trim();
    let upper = value.to_ascii_uppercase();
    ...
    Err(PluginRuntimePlatformError::InvalidDatabaseRequest(
...
fn parameter_values(parameters: &StrictJsonValue) -> PluginRuntimePlatformResult<Vec<SqlValue>> {
    let values = parameters.0.as_array().ok_or_else(|| {
        PluginRuntimePlatformError::InvalidDatabaseRequest(
    ...
    values
        ...
        .map(|value| match value {
            ...
            JsonValue::Array(_) | JsonValue::Object(_) => serde_json::to_string(value)
                .map(SqlValue::Text)
                .map_err(|error| PluginRuntimePlatformError::InvalidDatabaseRequest(error.to_string())),
        })
        .collect()
}

fn row_to_json(row: &rusqlite::Row<'_>) -> PluginRuntimePlatformResult<JsonValue> {
...
fn database_error(error: impl std::fmt::Display) -> PluginRuntimePlatformError {
    PluginRuntimePlatformError::Database(error.to_string())
}
...
fn canceled_database_error(
    ...
    cancellation: &PluginRuntimeCallCancellation,
) -> PluginRuntimePlatformError {
    if cancellation.is_canceled() {
        PluginRuntimePlatformError::Canceled
    } else {
        PluginRuntimePlatformError::Database(error.to_string())
    }
}
