...
use nomifun_api_types::{
    BuildPluginRuntimeRequest, CreatePluginRuntimeProjectRequest, DeletePluginRuntimeRequest,
    DurableOperationKindDto, DurableOperationStateDto, PluginRuntimeKindDto,
    PluginRuntimeLifecycleDto, PluginRuntimeProjectSourceStateDto, RetryPluginRuntimeDeleteRequest,
...
fn source_project_root(
    store_root: &Path,
    owner: &str,
    workshop: &nomifun_api_types::PluginRuntimeWorkshopDto,
) -> PathBuf {
    store_root
        .join("source")
        .join("sources")
...
async fn source_mutation_recovery_aborts_old_head_and_finalizes_new_head() {
    ...
    let created = service
        ...
        .await
        .unwrap();
    let source_store = PluginRuntimeSourceStore::new(store_root.path().join("source")).unwrap();
...
async fn ui_only_build_commits_ready_atomically_and_exposes_created_at() {
    ...
    assert_eq!(built.source_state, PluginRuntimeProjectSourceStateDto::Editable);
