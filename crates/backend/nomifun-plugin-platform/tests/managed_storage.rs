...
use nomifun_plugin_platform::runtime::{
    ...
    PluginRuntimeServiceStoragePort, PluginRuntimeServiceStorageRequest, SqlitePluginRuntimeManagedStorage,
};
use serde_json::json;
...
async fn production_storage_is_owner_scoped_and_persists_private_db() {
    ...
    let inserted = storage
        ...
        .await
        .unwrap();
    let inserted: nomifun_plugin_platform::runtime::PluginRuntimeDatabaseExecuteResult =
    ...
    let absent = storage
        ...
        .await
        .unwrap();
    let absent: PluginRuntimeDatabaseQueryResult = serde_json::from_value(absent.0).unwrap();
    ...
    let queried = storage
        ...
        .await
        .unwrap();
    let queried: PluginRuntimeDatabaseQueryResult = serde_json::from_value(queried.0).unwrap();
    ...
    let restarted = SqlitePluginRuntimeManagedStorage::new(
    ...
    let recovered = SqlitePluginRuntimeManagedStorage::new(
        root.path().to_path_buf(),
        _database.pool().clone(),
    )
    .unwrap();
