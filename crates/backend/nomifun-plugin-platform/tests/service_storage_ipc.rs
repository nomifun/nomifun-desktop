...
use nomifun_plugin_platform::runtime::{
    FixedPluginRuntimeServiceModuleResolver, InMemoryPluginRuntimeManagedStorage,
    PluginRuntimeCallCancellation, PluginRuntimeServiceInvocation, PluginRuntimeServiceProcessFactory,
    PluginRuntimeServiceProcessLimits, PluginRuntimeServiceStoragePort, NodePluginRuntimeServiceProcessFactory,
    PluginRuntimeServiceStorageRequest, PluginRuntimeServiceStorageResolution, PluginRuntimePlatformError,
...
async fn real_node_service_round_trips_host_storage_without_exposing_db_path() {
    ...
    let process = NodePluginRuntimeServiceProcessFactory::new(
        node.clone(),
    ...
    .unwrap()
    .start(nomifun_plugin_platform::runtime::PluginRuntimeServiceLaunch {
...
#[derive(Clone)]
struct SlowStorage {
    inner: Arc<InMemoryPluginRuntimeManagedStorage>,
    started: Arc<tokio::sync::Notify>,
    active: Arc<AtomicBool>,
}

#[async_trait]
impl PluginRuntimeServiceStoragePort for SlowStorage {
...
async fn slow_storage_request_does_not_block_other_calls_and_cancel_reaches_host() {
    ...
    tokio::fs::write(&module, CONCURRENT_STORAGE_SERVICE)
        .await
        .unwrap();
    let inner = Arc::new(InMemoryPluginRuntimeManagedStorage::new());
    ...
    let slow_cancellation = PluginRuntimeCallCancellation::default();
