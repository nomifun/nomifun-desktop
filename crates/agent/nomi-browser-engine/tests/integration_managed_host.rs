...
use nomi_browser_engine::{
    BrowserEngine, BrowserViewerFrame, BrowserViewerImageFormat, EngineConfig, FirewallConfig,
    LaneEngineConfig, ManagedBrowserHost,
};
...
fn viewer_screencast_fixture_html() -> &'static str {
    r#"<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <title>NomiFun viewer screencast fixture</title>
  <style>
    html, body {
      width: 100%;
      height: 100%;
...
<body data-nomifun-viewer="same-target">
  <div id="pulse">NOMIFUN_VIEWER_SAME_TARGET</div>
  <script>
    let phase = 0;
    const pulse = document.getElementById("pulse");
    setInterval(() => {
      phase = (phase + 29) % 360;
      document.body.style.background = `hsl(${phase} 90% 42%)`;
      document.body.dataset.phase = String(phase);
      pulse.textContent = `NOMIFUN_VIEWER_SAME_TARGET_${phase}`;
    }, 40);
  </script>
</body>
</html>"#
...
fn jpeg_dimensions(image: &[u8]) -> Option<(u16, u16)> {
    if image.len() < 4 || !image.starts_with(&[0xff, 0xd8]) {
        return None;
    }
    ...
    while cursor < image.len() {
        ...
        while cursor < image.len() && image[cursor] == 0xff {
            cursor += 1;
        }
        ...
        let segment_len =
            u16::from_be_bytes([*image.get(cursor)?, *image.get(cursor + 1)?]) as usize;
        if segment_len < 2 || cursor.checked_add(segment_len)? > image.len() {
            return None;
        }
        if matches!(
            marker,
            ...
            let height = u16::from_be_bytes([image[cursor + 3], image[cursor + 4]]);
            let width = u16::from_be_bytes([image[cursor + 5], image[cursor + 6]]);
...
fn assert_viewer_screencast_frame(
    frame: &BrowserViewerFrame,
    expected_target_id: &str,
) -> (u16, u16) {
    assert_eq!(frame.format, BrowserViewerImageFormat::Jpeg);
    assert_eq!(frame.target_id.as_deref(), Some(expected_target_id));
    assert!(
        frame.image.len() > 512,
        "viewer screencast must return a non-trivial JPEG frame"
    );
    assert!(
        frame.image.ends_with(&[0xff, 0xd9]),
        "viewer screencast JPEG must contain a complete end marker"
    );
    let dimensions =
        jpeg_dimensions(&frame.image).expect("viewer screencast must return a parseable JPEG");
    assert!(
        dimensions.0 <= 1_600 && dimensions.1 <= 1_200,
        "viewer screencast exceeded the 1600x1200 frame bound: {dimensions:?}"
    );
    assert!(
        frame
            .css_viewport_width
    ...
    assert_eq!(
        frame.device_pixel_ratio, 1.0,
        "screencast coordinates must use authoritative CSS dimensions"
    );
    dimensions
}
...
/// Real-Chromium acceptance for the persistent viewer frame path.
///
/// The fixture continuously repaints a full-page sentinel. The test starts a
/// lane-scoped `Page.startScreencast`, verifies that the JPEG is bound to the
/// lane's actual active target, then deliberately stops consuming while many
/// compositor updates occur. The next read must remain fast and eventually
/// expose a newer frame, which is the public observable consequence of the
/// latest-only watch buffer and eager CDP acknowledgement. It also covers
/// explicit/idempotent stop, restart, Lane close, Host shutdown, endpoint
/// closure and managed process-tree cleanup.
#[tokio::test]
#[ignore = "requires configured/bundled Chromium; set NOMIFUN_CHROME_BINARY and run alone with --ignored"]
async fn managed_host_viewer_screencast_real_chromium_acceptance() {
    let mut fixture = LocalFixture::start().await;
    let temp = tempfile::tempdir().unwrap();
    let profile_name = "viewer-screencast-profile";
    #[cfg(windows)]
    let profile = temp.path().join(profile_name);
    let host = Arc::new(
    ...
    let host_pid = host
        .process_id()
        .expect("managed viewer Host must report its root pid");
    #[cfg(windows)]
    let debug_port = {
        let port = read_windows_debug_port(&profile);
        tokio::net::TcpStream::connect(("127.0.0.1", port))
            .await
            .expect("viewer Host DevTools endpoint must be live before shutdown");
        port
    };

    let lane_id = "viewer-screencast";
    let lane = host
        .open_lane(lane_id, LaneEngineConfig::default())
        .await
        .unwrap();
    let navigation = {
        let lane = Arc::clone(&lane);
        let url = fixture.url("/viewer-screencast");
        tokio::spawn(async move { lane.navigate(&url, false).await })
    };
    let request = fixture.next_request().await;
    assert_eq!(request.path, "/viewer-screencast");
    request
        .response
        .send(FixtureResponse::html(viewer_screencast_fixture_html()))
        .unwrap();
    navigation.await.unwrap().unwrap();
    assert!(
        lane.rendered_html()
            .await
            .unwrap()
            .contains("NOMIFUN_VIEWER_SAME_TARGET")
    );

    let active_tabs = lane
        .tabs()
        .await
        .unwrap()
        .into_iter()
    ...
    assert_eq!(
        active_tabs.len(),
        1,
        "viewer Lane must have exactly one authoritative active target"
    );
    let active_target_id = active_tabs[0].target_id.clone();

    let first_started = Instant::now();
    let first = tokio::time::timeout(Duration::from_secs(5), lane.viewer_screencast_frame())
        .await
        .expect("viewer screencast must produce its first frame within five seconds")
        .unwrap();
    let first_elapsed = first_started.elapsed();
    let first_dimensions = assert_viewer_screencast_frame(&first, &active_target_id);

    // Do not poll while the fixture repaints roughly 25 times. A persistent
    // producer must keep acknowledging Chromium and replace one latest value,
    // rather than requiring a consumer read for each compositor frame.
    tokio::time::sleep(Duration::from_secs(1)).await;
    let post_pause_started = Instant::now();
    let mut latest =
        tokio::time::timeout(Duration::from_secs(1), lane.viewer_screencast_frame())
            .await
            .expect("latest viewer frame must remain immediately readable after a slow consumer")
            .unwrap();
    let post_pause_elapsed = post_pause_started.elapsed();
    assert!(
        post_pause_elapsed < Duration::from_millis(500),
        "latest-only viewer read was unexpectedly backpressured: {post_pause_elapsed:?}"
    );

    let changed_deadline = Instant::now() + Duration::from_secs(2);
    while latest.image == first.image && Instant::now() < changed_deadline {
        tokio::time::sleep(Duration::from_millis(100)).await;
        latest = tokio::time::timeout(Duration::from_secs(1), lane.viewer_screencast_frame())
            .await
            .expect("persistent viewer screencast must remain responsive")
            .unwrap();
    }
    assert_ne!(
        latest.image, first.image,
        "the continuously repainting fixture never replaced the latest viewer frame"
    );
    let latest_dimensions = assert_viewer_screencast_frame(&latest, &active_target_id);
    assert_eq!(
        latest_dimensions, first_dimensions,
        "viewer frame viewport changed without a target or viewport transition"
    );
    let html_during_stream =
        tokio::time::timeout(Duration::from_secs(2), lane.rendered_html())
            .await
            .expect("viewer screencast must not occupy the Lane correctness gate")
            .unwrap();
    assert!(html_during_stream.contains("NOMIFUN_VIEWER_SAME_TARGET"));

    let stop_started = Instant::now();
    tokio::time::timeout(Duration::from_secs(2), lane.stop_viewer_screencast())
        .await
        .expect("viewer screencast stop must be bounded")
        .unwrap();
    let stop_elapsed = stop_started.elapsed();
    tokio::time::timeout(Duration::from_secs(2), lane.stop_viewer_screencast())
        .await
        .expect("repeated viewer screencast stop must be bounded")
        .unwrap();

    let restart_started = Instant::now();
    let restarted =
        tokio::time::timeout(Duration::from_secs(5), lane.viewer_screencast_frame())
            .await
            .expect("viewer screencast must restart after an explicit stop")
            .unwrap();
    let restart_elapsed = restart_started.elapsed();
    let restarted_dimensions = assert_viewer_screencast_frame(&restarted, &active_target_id);
    assert_eq!(restarted_dimensions, first_dimensions);
    lane.stop_viewer_screencast().await.unwrap();

    let mut system = sysinfo::System::new();
    let before_lane_close = sample_process_tree(&mut system, host_pid);
    assert!(before_lane_close.pids.contains(&host_pid));
    let close_started = Instant::now();
    host.close_lane(lane_id).await.unwrap();
    let close_elapsed = close_started.elapsed();
    let closed_lane_frame =
        tokio::time::timeout(Duration::from_secs(2), lane.viewer_screencast_frame())
            .await
            .expect("closed viewer Lane must fail instead of hanging");
    assert!(
        closed_lane_frame.is_err(),
        "closed viewer Lane unexpectedly restarted a screencast"
    );
    assert_eq!(
        host.process_id(),
        Some(host_pid),
        "closing the viewer Lane must not bypass explicit Host shutdown"
    );
    ...
    assert!(
        shutdown_elapsed < Duration::from_secs(5),
        "managed viewer Host must shut down within five seconds; elapsed={shutdown_elapsed:?}"
    );
    assert!(host.process_id().is_none());
    #[cfg(windows)]
    assert!(
        wait_for_debug_endpoint_closed(debug_port, Duration::from_secs(5)).await,
        "managed viewer debugging endpoint remained reachable"
    );

    let sampled_pids = before_lane_close
        .pids
        .iter()
        .chain(&before_host_shutdown.pids)
        .copied()
    ...
    assert!(
        residual.is_empty(),
        "managed viewer Chromium process residue after shutdown: {residual:?}"
    );

    println!(
        "managed-host viewer screencast acceptance: host_pid={} target_bound=true \
         first_frame_ms={} post_pause_read_ms={} stop_ms={} restart_ms={} close_lane_ms={} \
         shutdown_ms={} frame_size={}x{} process_tree_before_close={} \
         process_tree_before_shutdown={} debug_endpoint_closed=true residual_pids={:?}",
        host_pid,
        first_elapsed.as_millis(),
        post_pause_elapsed.as_millis(),
