...
impl InnerWebView {
  ...
  fn init_webview(
    ...
    // Webview handlers
    unsafe { Self::attach_handlers(hwnd, &webview, &mut attributes, &mut token, env, is_child)? };
  ...
  unsafe fn attach_handlers(
    ...
    // Top-level behavior is unchanged. A child HWND belongs to its host: eagerly
    // destroying it here invalidates download operations before the host's
    // WindowCloseRequested handler can settle them. The host must explicitly
    // close child views after its native lifecycle barrier.
    if !is_child {
