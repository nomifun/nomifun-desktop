...
export default function MiniAppDraftPreview({
  html,
  title,
  onStatus,
}: {
  ...
  useEffect(() => {
    ...
    const receive = (event: MessageEvent) => {
      if (
        ...
        event.data?.type !== 'nomifun-miniapp-preview' ||
    ...
    <>
      {status === 'loading' && (
        <span className={styles.muted} role='status'>
          <Spin size={12} /> {t('miniApps.product.checking')}
        </span>
      )}
      {status === 'error' && (
        <Alert type='warning' content={t('miniApps.product.previewError')} />
      )}
      <iframe
        ref={frame}
        key={token}
