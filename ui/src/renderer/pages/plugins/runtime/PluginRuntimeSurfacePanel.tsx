...
import { pluginRuntimeSurfaceAssetPath, shortPluginRuntimeIdentity } from './model';
import styles from './PluginRuntimeSurface.module.css';

interface PluginRuntimeSurfacePanelProps {
  compact?: boolean;
...
function createBridgeNonce(): string {
  ...
  if (!cryptoObject || typeof cryptoObject.getRandomValues !== 'function') {
    throw new Error('WebCrypto getRandomValues is required for PluginRuntime Bridge handshake');
  }
...
function parseKvRequest(value: unknown): PluginRuntimeBridgeKvRequest | null {
...
const PluginRuntimeSurfacePanel: React.FC<PluginRuntimeSurfacePanelProps> = ({
  compact = false,
  descriptor,
  displayName,
  reloading,
  ...
  const openBridge = useCallback(
    (frame: Window, nonce: string) => {
      ...
      hostPort.onmessage = (event: MessageEvent<unknown>) => {
        ...
        void ipcBridge.pluginRuntimes.bridge
          .invoke({
  ...
  const beginBridgeHandshake = useCallback(() => {
    ...
    try {
      ...
      console.error('[pluginRuntimes] failed to create Surface Bridge nonce', error);
      setFrameFailed(true);
      return;
    }
    ...
    try {
      ...
      console.error('[pluginRuntimes] failed to start Surface Bridge handshake', error);
      clearHandshake();
      bridgeLoadRef.current.handshakeNonce = null;
    }
  }, [
  ...
  return (
    <section
      ...
      aria-labelledby={compact ? undefined : 'plugin-surface-title'}
      aria-label={compact ? displayName : undefined}
      aria-describedby={compact ? undefined : 'plugin-surface-hint'}
    ...
    >
      {!compact && <header className={styles.surfaceHeader}>
        <div className={styles.surfaceHeading}>
          <h3 id='plugin-surface-title' className={styles.sectionTitle}>
            {t('pluginRuntime.surface.title')}
          </h3>
          <p id='plugin-surface-hint' className={styles.sectionHint}>
            {t('pluginRuntime.surface.fence', {
              epoch: descriptor.active_release_epoch,
              digest: shortPluginRuntimeIdentity(
                descriptor.expected_release_digest,
                10
              ),
            })}
        ...
        <div className={styles.surfaceActions}>
          <Button
            ...
            aria-label={`${t(compact ? 'pluginRuntime.product.retry' : 'pluginRuntime.actions.reloadSurface')}: ${displayName}`}
            loading={reloading}
            disabled={reloading || closing}
            onClick={handleReload}
          >
            {t(compact ? 'pluginRuntime.product.retry' : 'pluginRuntime.actions.reloadSurface')}
          </Button>
          <Button
            size='small'
            icon={<CloseOne theme='outline' size='14' />}
            aria-label={`${t('pluginRuntime.actions.closeSurface')}: ${displayName}`}
            loading={closing}
            disabled={reloading || closing}
            onClick={handleClose}
          >
            {t('pluginRuntime.actions.closeSurface')}
          </Button>
        </div>
      </header>}
        ...
        {!source || frameFailed ? (
          ...
          >
            <span className={styles.stateTitle}>
              {t(compact ? 'pluginRuntime.product.openFailed' : 'pluginRuntime.errors.surfaceFrameTitle')}
            </span>
            <span className={styles.stateBody}>
              {t(compact ? 'pluginRuntime.product.operationFailed' : 'pluginRuntime.errors.surfaceFrameBody')}
            </span>
            <Button
              size='small'
              icon={<Refresh theme='outline' size='14' />}
          ...
          <>
            {frameLoading && (
              <div className={styles.surfaceLoading} role='status'>
                <Spin size={24} />
                <span>{t('pluginRuntime.states.loadingSurface')}</span>
              </div>
            )}
            <iframe
              ref={iframeRef}
              ...
              style={compact ? { minHeight: 'calc(100vh - 190px)' } : undefined}
              src={source}
              sandbox='allow-scripts allow-forms'
              referrerPolicy='no-referrer'
              title={t('pluginRuntime.surface.frameTitle', { name: displayName })}
...
export default PluginRuntimeSurfacePanel;
