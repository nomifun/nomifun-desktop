/**
 * @license
 * Copyright 2025-2026 NomiFun (nomifun.com)
 * SPDX-License-Identifier: Apache-2.0
 */

import { describe, expect, test } from 'bun:test';

import { cloneTemplateRunAggregate } from '../domain';
import { IDS, createTemplateRunFixture } from '../domain/testFixtures';
import type { CreativeTemplateHttpRequest } from './templateApi';
import { createCreativeTemplateRunApi } from './templateRunApi';

describe('Creative Template run API client', () => {
  test('uses exact list, create, detail, and CAS save contracts', async () => {
    const requested = createTemplateRunFixture();
    const queued = cloneTemplateRunAggregate(requested);
    queued.revision = 2;
    queued.record.status = 'queued';
    queued.record.taskIds = [IDS.task];
    queued.record.queuedAt = 2_100;
    const calls: Array<{ method: string; path: string; body?: unknown }> = [];
    const request: CreativeTemplateHttpRequest = async (method, path, body) => {
      calls.push({ method, path, ...(body === undefined ? {} : { body }) });
      if (method === 'GET' && path.includes('?templateId=')) return { runs: [requested] };
      return { run: method === 'PUT' ? queued : requested };
    };
    const api = createCreativeTemplateRunApi(request);

    expect(await api.listRuns(IDS.template)).toEqual([requested]);
    expect(await api.createRun({
      templateRunId: IDS.request,
      templateId: IDS.template,
      templateRevision: 1,
      inputs: requested.request.inputs,
      referenceAssetIds: [],
    })).toEqual(requested);
    expect(await api.getRun(IDS.request)).toEqual(requested);
    expect(await api.saveRun(IDS.request, { expectedRevision: '1', run: queued })).toEqual(queued);

    expect(calls).toEqual([
      {
        method: 'GET',
        path: `/api/creative-studio/template-runs?templateId=${IDS.template}`,
      },
      {
        method: 'POST',
        path: '/api/creative-studio/template-runs',
        body: {
          request: {
            templateRunId: IDS.request,
            templateId: IDS.template,
            templateRevision: 1,
            inputs: requested.request.inputs,
            referenceAssetIds: [],
          },
        },
      },
      { method: 'GET', path: `/api/creative-studio/template-runs/${IDS.request}` },
      {
        method: 'PUT',
        path: `/api/creative-studio/template-runs/${IDS.request}`,
        body: { expectedRevision: '1', run: queued },
      },
    ]);
  });

  test('fails closed on unknown fields and response identity drift', async () => {
    const run = createTemplateRunFixture();
    const unknown = createCreativeTemplateRunApi(async () => ({
      run: { ...run, legacyState: 'unsafe' },
    }));
    try {
      await unknown.getRun(IDS.request);
      throw new Error('expected unknown-field rejection');
    } catch (error) {
      expect(error).toMatchObject({ code: 'unknown-field', path: '$.run.legacyState' });
    }

    const drifted = cloneTemplateRunAggregate(run);
    drifted.request.id = IDS.idempotency;
    drifted.request.idempotencyKey = IDS.idempotency;
    drifted.record.requestId = IDS.idempotency;
    const identity = createCreativeTemplateRunApi(async () => ({ run: drifted }));
    try {
      await identity.getRun(IDS.request);
      throw new Error('expected identity rejection');
    } catch (error) {
      expect(error).toMatchObject({
        code: 'identity-mismatch',
        path: '$.run.request.id',
      });
    }
  });

  test('validates writes before issuing an HTTP request', async () => {
    const run = createTemplateRunFixture();
    let calls = 0;
    const api = createCreativeTemplateRunApi(async () => {
      calls += 1;
      return { run };
    });
    try {
      await api.createRun({
        templateRunId: IDS.request,
        templateId: IDS.template,
        templateRevision: 1,
        inputs: run.request.inputs,
        referenceAssetIds: [IDS.asset, IDS.asset],
      });
      throw new Error('expected duplicate-id rejection');
    } catch (error) {
      expect(error).toMatchObject({
        code: 'duplicate-id',
        path: '$.request.referenceAssetIds',
      });
    }
    expect(calls).toBe(0);
  });
});
