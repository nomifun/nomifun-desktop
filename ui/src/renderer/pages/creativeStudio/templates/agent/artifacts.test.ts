...
const artifact = (
  overrides: Partial<CreativeTemplateDraftArtifact['draft']> = {}
): CreativeTemplateDraftArtifact => ({
  kind: CREATIVE_TEMPLATE_DRAFT_ARTIFACT_KIND,
  summary: '生成一个可直接编辑的模板草稿',
  draft: {
    mode: 'single-image',
    name: '电商主图',
...
const expectFailure = (text: string): CreativeStudioContractError => {
  try {
    parseCreativeTemplateDraftArtifact(text);
    throw new Error('Expected template draft artifact contract failure');
  } catch (error) {
...
describe('Creative Template Agent artifact parser', () => {
  ...
  test('leaves ordinary prose and artifacts owned by other products untouched', () => {
    expect(parseCreativeTemplateDraftArtifact('先从一个简单模板开始。')).toBeNull();
    expect(
      parseCreativeTemplateDraftArtifact(
  ...
  test('rejects unpaired UTF-16 surrogates and oversized JSON', () => {
    ...
    const oversized =
      ...
      JSON.stringify(artifact().draft) +
      ',"padding":"' +
      'x'.repeat(MAX_CREATIVE_TEMPLATE_DRAFT_JSON_BYTES) +
      '"}\n```';
