...
export const CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND =
  'nomifun.creative-studio.workflow-draft/v1' as const;
export const MAX_CREATIVE_WORKFLOW_DRAFT_JSON_BYTES = 262_144;

export type CreativeWorkflowDraftMode = 'single-image' | 'multi-image-series';

export interface CreativeWorkflowDraft {
  mode: CreativeWorkflowDraftMode;
  name: string;
  description: string;
  category: string;
...
export interface CreativeWorkflowDraftArtifact {
  kind: typeof CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND;
  summary: string;
  draft: CreativeWorkflowDraft;
}
...
const parseDraft = (value: unknown): CreativeWorkflowDraft => {
...
const artifactFromValue = (value: unknown): CreativeWorkflowDraftArtifact | null => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const artifact = value as UnknownRecord;
  if (artifact.kind !== CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND) return null;
 ...
 */
export function parseCreativeWorkflowDraftArtifact(
  text: string
): CreativeWorkflowDraftArtifact | null {
  ...
  if (!hasCanonicalOpening || !candidate.endsWith(closing)) {
    if (
      text.includes('```') &&
      text.includes(CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND)
    ) {
      return fail('$', 'one final lowercase json workflow-draft fence');
    }
    return null;
  }
  ...
  if (
    jsonText.length > MAX_CREATIVE_WORKFLOW_DRAFT_JSON_BYTES ||
    new TextEncoder().encode(jsonText).byteLength >
      MAX_CREATIVE_WORKFLOW_DRAFT_JSON_BYTES
  ) {
    return fail(
      '$',
      `workflow-draft JSON <= ${MAX_CREATIVE_WORKFLOW_DRAFT_JSON_BYTES} UTF-8 bytes`
    );
  }
  ...
  try {
    ...
    if (jsonText.includes(CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND)) {
      return fail('$', 'well-formed workflow-draft JSON');
    }
    return null;
  }
  ...
  try {
    ...
    if (
      decodedKind === CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND ||
      jsonText.includes(CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND)
    ) {
  ...
  if (decodedKind !== CREATIVE_WORKFLOW_DRAFT_ARTIFACT_KIND) return null;
