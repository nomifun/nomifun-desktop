...
function broken(path: string, message: string): WorkflowValueResult<WorkflowStep[]> {
...
export function topologicallySortWorkflowSteps(
...
): WorkflowValueResult<WorkflowStep[]> {
  ...
  for (const step of workflow.steps) {
    ...
    for (const dependencyId of step.dependsOn) {
      if (!byId.has(dependencyId)) {
        return broken(`$.steps[${workflow.steps.indexOf(step)}].dependsOn`, 'dependency does not exist');
      }
      followers.set(dependencyId, [...(followers.get(dependencyId) ?? []), step.id]);
    }
  }
  const queue = workflow.steps.filter((step) => indegree.get(step.id) === 0).map((step) => step.id);
  ...
  if (ordered.length !== workflow.steps.length) {
    return {
      ok: false,
      error: { code: 'cycle-detected', path: '$.steps', message: 'workflow step graph must be acyclic' },
    };
  }
  return { ok: true, value: ordered };
}

export function validateWorkflowGraph(workflow: WorkflowDefinitionV1): WorkflowValidationError | null {
  const variables = new Map(workflow.variables.map((variable) => [variable.id, variable]));
  const templates = new Map(workflow.templates.map((template) => [template.id, template]));
  const steps = new Map(workflow.steps.map((step) => [step.id, step]));

  for (const [templateIndex, template] of workflow.templates.entries()) {
  ...
  for (const [stepIndex, step] of workflow.steps.entries()) {
    ...
    if (step.kind === 'render-template' || step.kind === 'draft-prompts') {
      ...
      if (step.kind === 'draft-prompts' && workflow.output.kind !== 'multi-image-series') {
        return {
          code: 'invalid-value',
