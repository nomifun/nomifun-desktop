...
import { createWorkflowTextAssetReader } from './textAssetReader';

describe('workflow planner text asset reader', () => {
  test('reads only bounded text/plain responses from the canonical asset URL', async () => {
    const calls: Array<{ input: string; init?: RequestInit }> = [];
    const reader = createWorkflowTextAssetReader(
  ...
  test('rejects non-text and oversized planner assets', async () => {
    const wrongType = createWorkflowTextAssetReader(
      () => '/asset',
    ...
    const oversized = createWorkflowTextAssetReader(
      () => '/asset',
