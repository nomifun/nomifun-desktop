...
const SUCCESS_MARKER = 'workflow-model-select-interaction:ok';
...
describe('Workflow Agent draft modal interaction', () => {
  test('clicks a real model option without leaking DOM globals into the UI suite', async () => {
    ...
    const casePath = fileURLToPath(
      new URL('./WorkflowAgentDraftModal.interaction.case.tsx', import.meta.url)
    );
