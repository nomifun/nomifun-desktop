        ...
        <div ref={dropdownRef} className={styles.wsDropdown} style={dropdownStyle}>
          ...
          {filteredRecent.map((path) => {
            ...
            return (
              ...
              >
                <FolderIcon size={13} />
                {/* Two lines: basename for quick scanning + full path so same-named
                    folders under different parents stay distinguishable. */}
                <div className='min-w-0 flex-1 flex flex-col'>
                  <span className={styles.wsDropdownItemName}>{name}</span>
                  <span className='truncate text-11px leading-14px text-t-tertiary'>{path}</span>
                </div>
                {isActive && (
                  <svg
                    width='12'
