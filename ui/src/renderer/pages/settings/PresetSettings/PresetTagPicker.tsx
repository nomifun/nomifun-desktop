/**
 * PresetTagPicker — A chip-toggle group for one tag dimension in the edit
...
export type PresetTagPickerHandle = {
      ...
      <div className='flex flex-wrap items-center gap-8px'>
            ...
            <div className='inline-flex flex-col gap-2px'>
              ...
              {showAddHint && (
                <span className='text-11px leading-16px text-[var(--color-text-3)] whitespace-nowrap'>
                  {t('settings.presetTagAddHint', { defaultValue: 'Press Enter to finish adding' })}
                </span>
              )}
            </div>
          ) : (
        ...
        {tags.length === 0 && readOnly && (
          <span className='text-12px text-[var(--color-text-3)]'>
            {t('settings.presetTagNone', { defaultValue: 'None' })}
          </span>
        )}
      </div>
    </div>
...
PresetTagPicker.displayName = 'PresetTagPicker';

export default PresetTagPicker;
