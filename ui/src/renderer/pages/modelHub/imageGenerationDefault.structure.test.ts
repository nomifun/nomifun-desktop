...
describe('default image-generation model', () => {
  ...
  test('the image section wires the exact image-generation task selector', () => {
    ...
    const panel = readSource('./CreationModelsPanel.tsx');

    expect(
      imageSection.includes("defaultModelPreferenceKey='models.default.imageGeneration'")
    ).toBe(true);
    expect(panel.includes("task='image_generation'")).toBe(true);
    expect(panel.includes("useModelsForTask('image_generation')")).toBe(true);
    expect(panel.includes('disabled={noCandidates || isSavingDefault}')).toBe(true);
    expect(panel.includes("t('settings.modelHub.creation.defaultNoModels')")).toBe(true);
  });
