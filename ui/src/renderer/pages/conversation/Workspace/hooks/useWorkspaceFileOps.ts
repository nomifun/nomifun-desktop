...
import type { MessageApi, RenameModalState, DeleteModalState, SelectedFile } from '../types';
import { getPathSeparator, replacePathInList, updateTreeForRename } from '../utils/treeHelpers';

interface UseWorkspaceFileOpsOptions {
  workspace: string;
  /**
   * Selection sinks supplied by the active source (replaces the former
   * `${eventPrefix}.selected.file(.append)` emits). Optional: sources without a
   * selection channel (terminal) omit them and these calls become no-ops.
   */
...
export function useWorkspaceFileOps(options: UseWorkspaceFileOpsOptions) {
  ...
  const handleAddToChat = useCallback(
    ...
    [closeContextMenu, ensureNodeSelected, onAppendFiles, messageApi, t]
  );
