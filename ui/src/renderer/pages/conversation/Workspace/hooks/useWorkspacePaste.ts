...
interface UseWorkspacePasteOptions {
  /**
   * Stable upload-tracking identity (the conversation id string for WebUI HTTP
...
export function useWorkspacePaste(options: UseWorkspacePasteOptions) {
        ...
        try {
          for (let i = 0; i < fileList.length; i++) {
            ...
            const tracker = trackUpload(file.size, {
              source: 'workspace',
              name: file.name,
              conversationId: trackingKey,
              onAbort: () => controller.abort(),
            });
            try {
              await uploadFileViaHttp(file, trackingKey, tracker.onProgress, undefined, {
  ...
  }, [trackingKey, handleSelectHostFiles, messageApi, refreshWorkspace, t]);
