...
const StarOfficeMonitorCard: React.FC<StarOfficeMonitorCardProps> = ({ conversation_id }) => {
  const { t } = useTranslation();
  // 直接从本表面的 PreviewProvider 取 openPreview。此组件经由 ChatLayout 的 headerExtra
  // 渲染（始终位于 ChatLayout 内的 provider 之下），因此可安全直接消费预览上下文，
  // 无需再由父级 ChatConversation 透传 onOpenUrl（那会让消费点落到 ChatLayout 之外）。
  // Consume openPreview directly from this surface's PreviewProvider. This card renders
  // via ChatLayout's headerExtra (always under the in-ChatLayout provider), so it can
  // safely consume the preview context directly instead of receiving an onOpenUrl prop
  // from the parent ChatConversation (which would put the consumer outside ChatLayout).
  const { openPreview } = usePreviewContext();
  const onOpenUrl = useCallback(
    (url: string, metadata?: PreviewMetadata) => {
  ...
  const statusBadgeColor = useMemo(() => {
    ...
    if (detectState === 'checking') return 'rgb(var(--primary-6))';
