...
export type BrowserSnapshot = { conversation_id: string; run: { revision: number; input_state: 'user_ready' | 'agent_running'; input_gate_failed: boolean }; runtime: { runtime_generation: number; revision: number; active_tab_id: string | null; tabs: BrowserTab[]; downloads: BrowserDownload[] } | null };
...
export type BrowserShortcut = { conversation_id: string; target: BrowserTarget; action: BrowserShortcutAction };
...
export function newerSnapshot(current: BrowserSnapshot | null, incoming: BrowserSnapshot): BrowserSnapshot {
  ...
  return { conversation_id: incoming.conversation_id, run, runtime };
}
export interface BrowserClient {
  listenShortcuts(id: string, listener: (shortcut: BrowserShortcut) => void): Promise<() => void>;
  closeWorkspace(id: string, runtimeGeneration: number): Promise<void>;
  ensure(id: string): Promise<BrowserSnapshot>;
  command(id: string, command: BrowserCommand): Promise<BrowserSnapshot>;
  attach(id: string, bounds: SurfaceBounds, onEvent: (event: BrowserViewEvent) => void): Promise<number>;
...
export const browserClient: BrowserClient = {
  async listenShortcuts(id, listener) {
    ...
    return listen<BrowserShortcut>('browser-workspace-shortcut', event => { if (event.payload.conversation_id === id) listener(event.payload); });
  },
  async closeWorkspace(id, runtimeGeneration) { await httpRequest('DELETE', `/api/conversations/${encodeURIComponent(id)}/browser`, { runtime_generation: runtimeGeneration }); },
  ensure: id => httpRequest('POST', `/api/conversations/${encodeURIComponent(id)}/browser`, {}),
  command: (id, command) => httpRequest('POST', `/api/conversations/${encodeURIComponent(id)}/browser/commands`, command),
  async attach(id, bounds, onEvent) {
    const { invoke, Channel } = await import('@tauri-apps/api/core');
    const events = new Channel<BrowserViewEvent>();
    events.onmessage = onEvent;
    return invoke<number>('browser_surface_attach', { conversationId: id, bounds, events });
  },
