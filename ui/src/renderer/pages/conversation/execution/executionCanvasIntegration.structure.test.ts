...
describe('conversation execution canvas integration', () => {
  test('keeps execution progress native to the collaboration pane', () => {
    ...
    expect(edgeSource.includes("sourceHandle: 'step-output'")).toBe(true);
    expect(edgeSource.includes("targetHandle: 'step-input'")).toBe(true);
    ...
    expect(canvasSource.includes("style: { pointerEvents: 'all', zIndex: 4 }")).toBe(true);
    expect(canvasSource.includes('resolveExecutionCanvasRelationState')).toBe(true);
