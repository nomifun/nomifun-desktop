...
export function usePresetAssistantInfo(conversation: TChatConversation | undefined): {
  ...
  const remoteAgentId =
    conversation?.type === 'remote' ? (conversation.extra as { remoteAgentId?: number })?.remoteAgentId : undefined;
  const { data: remoteAgent, isLoading: isLoadingRemoteAgent } = useSWR(
    remoteAgentId != null ? `remote-agent.get.${remoteAgentId}` : null,
    () => (remoteAgentId != null ? ipcBridge.remoteAgent.get.invoke({ id: remoteAgentId }) : null)
  );
