...
export const usePreviewLauncher = () => {
  ...
  // Non-throwing: surfaces without a PreviewProvider (e.g. the knowledge inbox
  // diff view) get `null` instead of a render-time crash. `canPreview` lets
  // those callers hide the "open in panel" affordance.
  const openPreview = usePreviewContextOptional()?.openPreview;
  ...
  const launchPreview = useCallback(
    async ({
      ...
      // No preview surface available (no PreviewProvider) — degrade to a no-op.
