...
const EmojiPicker: React.FC<EmojiPickerProps> = ({ value, onChange, children, placement = 'bl' }) => {
  ...
  const pickerContent = (
    <div className='w-280px'>
      {/* Category Tabs */}
      <div className='flex items-center gap-2px px-8px py-6px border-b border-b-solid border-[var(--color-border-2)] overflow-x-auto'>
        {categoryKeys.map((key) => (
          <button
            key={key}
            className={`flex-shrink-0 w-28px h-28px flex items-center justify-center rounded-md text-16px cursor-pointer border-none bg-transparent hover:bg-fill-2 transition-colors ${activeCategory === key ? '!bg-primary-1 text-primary-6' : ''}`}
