...
/** NomiModal 组件 Props */
export interface NomiModalProps extends Omit<ModalProps, 'title' | 'footer'> {
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <NomiModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </NomiModal>
 *
 * // 预设尺寸 / Preset size
 * <NomiModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </NomiModal>
 *
...
const NomiModal: React.FC<NomiModalProps> = ({
  children,
  size,
  header,
  footer,
      ...
      return {
        render: () => (
          <div className='flex justify-end gap-8px'>
            ...
            <Button onClick={onCancel} className='px-16px min-w-72px' style={{ borderRadius: 8 }}>
              {cancelLabel}
            </Button>
            <Button
              type='primary'
              // Button.onClick is (e: Event) => void; Modal.onOk is (e?: MouseEvent) => Promise|void.
              // Forward the original MouseEvent when present and discard the return value.
              onClick={(e) => {
                props.onOk?.(e instanceof MouseEvent ? e : undefined);
              }}
              loading={props.confirmLoading}
              className='px-16px min-w-72px'
...
NomiModal.displayName = 'NomiModal';

export default NomiModal;
