 ...
 */
const NomiCollapseItem: React.FC<NomiCollapseItemProps> = ({ children }) => <>{children}</>;
NomiCollapseItem.displayName = 'NomiCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <NomiCollapse defaultActiveKey={['1']}>
 *   <NomiCollapse.Item name="1" header="面板1">
 *     内容1
 *   </NomiCollapse.Item>
 *   <NomiCollapse.Item name="2" header="面板2">
 *     内容2
 *   </NomiCollapse.Item>
 * </NomiCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <NomiCollapse accordion defaultActiveKey="1">
 *   <NomiCollapse.Item name="1" header="面板1">内容1</NomiCollapse.Item>
 *   <NomiCollapse.Item name="2" header="面板2">内容2</NomiCollapse.Item>
 * </NomiCollapse>
 *
 * // 自定义图标 / Custom icon
 * <NomiCollapse
 ...
 * ```
 */
const NomiCollapseComponent: React.FC<NomiCollapseProps> & { Item: typeof NomiCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<NomiCollapseItemProps> => {
      return React.isValidElement(child) && child.type === NomiCollapseItem;
    });
  }, [children]);
...
NomiCollapseComponent.Item = NomiCollapseItem;

export default NomiCollapseComponent;
