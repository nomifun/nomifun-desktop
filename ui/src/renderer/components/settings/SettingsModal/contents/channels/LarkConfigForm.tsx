...
const LarkConfigForm: React.FC<LarkConfigFormProps> = ({
  ...
  const {
    ...
    rejectPairing,
    revokeUser,
  } = useChannelPairing('lark', channelTarget);
  ...
  // is live (connected) gets its credentials locked so they can't be changed out
  // from under it; create mode (pluginStatus null) and a not-yet-connected row
  // stay editable. (Previously keyed off the GLOBAL per-platform authorized-user
  // list, which wrongly froze a second companion's create form once ANY bot of
  // the platform had a user — the "platform monopolized by one companion" bug.)
