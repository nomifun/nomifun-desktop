...
const MattermostConfigForm: React.FC<MattermostConfigFormProps> = ({ pluginStatus, channelTarget, onStatusChange }) => {
  ...
  const loadPendingPairings = useCallback(async () => {
    ...
    try {
      ...
      if (pairings) setPendingPairings(pairings.filter((p) => p.platformType === 'mattermost' && (!channelTarget?.channelId || p.channelId === channelTarget.channelId)));
    } finally {
  ...
  const loadAuthorizedUsers = useCallback(async () => {
    ...
    try {
      ...
      if (users) setAuthorizedUsers(users.filter((u) => u.platformType === 'mattermost' && (!channelTarget?.channelId || u.channelId === channelTarget.channelId)));
    } finally {
  ...
  const handleAutoEnable = async () => {
    ...
    if (plugins) {
      const row = channelTarget ? (channelTarget.channelId ? plugins.find((p) => p.id === channelTarget.channelId) : plugins.find((p) => p.type === 'mattermost' && p.companionId === channelTarget.companionId)) : plugins.find((p) => p.type === 'mattermost');
  ...
  const handleTestConnection = async () => {
    ...
    setTestLoading(true);
    try {
      const result = await channel.testPlugin.invoke({ plugin_id: 'mattermost', token: token.trim(), extra_config: { server_url: serverUrl.trim() } });
      if (result.success) {
