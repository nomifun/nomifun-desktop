...
- **Status-only browser management** — the **Browser** page reports conversations, runtimes, Lanes, tabs, URLs, identity mode, capacity, queue position, pressure, resource estimates, and failures. It deliberately does not embed a page preview or expose page input, preventing accidental interruption of agent work.
...
- **External managed browser, authoritative cleanup** — Primary work runs in a visible managed Chromium window while NomiFun retains lifecycle authority. Users can close one Lane, a conversation's Lanes, or all managed browsers; runtime, attempt, remote-transport, and app shutdown also revoke ownership and explicitly clean up managed Chromium process trees.
...
| ✉️ **Email** | `www.nomifun.com/contact` <sub>(provisional — being finalized)</sub> |
